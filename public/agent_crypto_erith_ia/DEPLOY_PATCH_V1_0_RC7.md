# Déploiement minimal — Agent-Crypto @erith.IA V1.0-RC7

## Fichiers à uploader

```text
public/agent_crypto_erith_ia/web/index.html
public/agent_crypto_erith_ia/web/style.css
public/agent_crypto_erith_ia/web/app.js
public/agent_crypto_erith_ia/README.md
public/agent_crypto_erith_ia/MANIFEST.json
public/agent_crypto_erith_ia/docs/CHANGELOG_V1_0_RC7.md
public/agent_crypto_erith_ia/DEPLOY_PATCH_V1_0_RC7.md
```

## Test

```text
Ctrl + F5
Lancer Livecheck
Regarder la carte Sources réussies
Regarder le bloc Anti-hallucination
```

## Résultat attendu

```text
Sources réussies : 6/7
7/7 interrogées · 1 échec
```

## Commit conseillé

```text
clarify source status labels rc7
```
