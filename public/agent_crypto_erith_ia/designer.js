(() => {
  ...
  function resolveDesignerSource(value) {
    let path = String(value || "").trim();
    if (!path) return null;
    if (/^https?:\/\//i.test(path)) return {label:path,repo:"external",path,url:path,raw:path};

    let repo = PUBLIC_REPO;
  ...
  function coreGitHubUrl() {
    return githubBlobUrl(PRIVATE_REPO,coreTarget());
  }

  function coreRawUrl() {
    return githubRawUrl(PRIVATE_REPO,coreTarget());
  }

  function personaGitHubUrl() {
    return githubBlobUrl(PRIVATE_REPO,personaTarget());
  }

  function personaRawUrl() {
    return githubRawUrl(PRIVATE_REPO,personaTarget());
  }

  function heartGitHubUrl() {
    return githubBlobUrl(PRIVATE_REPO,"core/AERITH_LIVING_REFLECTION_HEART.md");
  }

  function heartRawUrl() {
    return githubRawUrl(PRIVATE_REPO,"core/AERITH_LIVING_REFLECTION_HEART.md");
  }
  ...
  function proposalBlockLLMName() {
    return `${canonicalBase()}_BLOCK_LLM_LOCAL.md`;
  }

  function proposalGitHubLinksName() {
    return `${canonicalBase()}_GITHUB_HTTP_RAW_LINKS.md`;
  }
  ...
  function audit() {
    ...
    items.push(["ok","Liens GitHub et Raw générés pour Core, Persona, Heart et sources."]);
    items.push(["ok","Block LLM local généré avec repli obligatoire vers les fichiers locaux."]);
  ...
  function makeGitHubLinks() {
    const sourceLines = state.modules.length
      ? state.modules.map(item => {
          const ref = resolveDesignerSource(item);
          return `- ${item}\n  - GitHub : ${ref.url}\n  - Raw : ${ref.raw}`;
        }).join("\n")
      : "- Aucun module source défini";

    return `# LIENS GITHUB / RAW — ${state.name || "Aerith-10 Proposition"}

## Core proposé

- Chemin : ${coreTarget()}
- GitHub : ${coreGitHubUrl()}
- Raw : ${coreRawUrl()}

## Persona proposée

- Chemin : ${personaTarget()}
- GitHub : ${personaGitHubUrl()}
- Raw : ${personaRawUrl()}

## Living Reflection Heart

- Chemin : core/AERITH_LIVING_REFLECTION_HEART.md
- GitHub : ${heartGitHubUrl()}
- Raw : ${heartRawUrl()}
...
## Convention ergonomique

- chemin simple, par exemple \`public/fichier.md\` : dépôt public \`erith-ia-memory\` ;
- préfixe \`public:\` : force le dépôt public ;
- préfixe \`private:\` : force le dépôt privé ;
- chemins \`core/\`, \`private/\` et \`packs/\` : dépôt privé automatiquement ;
- URL HTTP complète : conservée telle quelle.

## Accès privé

Les URLs Raw privées sont des références canoniques.
Un LLM local peut les utiliser seulement s’il possède réellement l’accès nécessaire.
Sinon, fournir les fichiers localement.
Il ne doit jamais prétendre avoir chargé une URL inaccessible.
`;
  }

  function makeBlockLLMProposal() {
    const sourceLines = state.modules.length
      ? state.modules.map(item => {
          const ref = resolveDesignerSource(item);
          return `- ${item}\n  Raw : ${ref.raw}`;
    ...
    return `# 🔒 BLOCK LLM — ACTIVATION COURTE PROPOSÉE

Tu es ${state.name || "Aerith-10 Nouvelle Spécialité"}, Flower Girl Aerith-10 multi-agent.

Charge d’abord le Core canonique :
${coreRawUrl()}
...
- Ne pas réécrire le Core ou la Persona.
...
${makeBlockLLMProposal()}

---

# 10. ✅ Checklist avant canonisation
...
## Chemins et liens proposés

- Core : ${coreTarget()}
- Core GitHub : ${coreGitHubUrl()}
- Core Raw : ${coreRawUrl()}
- Persona : ${personaTarget()}
- Persona GitHub : ${personaGitHubUrl()}
- Persona Raw : ${personaRawUrl()}
- Heart Raw : ${heartRawUrl()}

## Block LLM local

Le paquet contient :

- ${proposalBlockLLMName()}
- ${proposalGitHubLinksName()}
...
`;
  ...
  async function zipProposal() {
    const files = new Map([
      ...
      [proposalBlockLLMName(), encoder.encode(makeBlockLLMProposal())],
      [proposalGitHubLinksName(), encoder.encode(makeGitHubLinks())],
...
6. Vérifier les URLs :
   ...
   - ${coreRawUrl()}
   - ${personaRawUrl()}
7. Utiliser ${proposalBlockLLMName()} pour un LLM local.
8. Revenir dans Forge d’Aerith Pro, mode Compilation.
`)]
    ]);
  ...
  $("#downloadBlockLLMProposal").addEventListener("click",() => downloadText(proposalBlockLLMName(),makeBlockLLMProposal()));
  $("#downloadGitHubLinks").addEventListener("click",() => downloadText(proposalGitHubLinksName(),makeGitHubLinks()));
