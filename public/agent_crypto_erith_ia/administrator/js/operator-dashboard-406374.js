/* Agent-Crypto Administrator — 40.6.374
   OPERATOR DASHBOARD · THREE-LINE HOVER STATUS
   Three-line hover status: mounted hover readouts are preserved and rendered as compact centered three-line operator cards for REDIVIDER and Math Core. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406374)return;

  const BUILD="40.6.374";
  const ROOT_ID="agentCryptoOperatorDashboard406374";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406374";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.374.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406374");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406374");
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406374">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406374">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406374">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406374">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406374">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406374">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406374" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406374">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406374">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406374" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406374" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406374" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406374" aria-describedby="acOperatorRedividerConfirmDesc406374">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406374">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406374">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406374">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406374">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406374">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406374" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406374" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406374")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406374")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406374");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406374");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406374");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406374");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406374");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406374");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406374");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406374");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406374");
  ...
  globalThis.AgentCryptoOperatorDashboard406374=Object.freeze({
    build:BUILD,
    mount,
