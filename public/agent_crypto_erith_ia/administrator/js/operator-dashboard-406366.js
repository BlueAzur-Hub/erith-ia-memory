/* Agent-Crypto Administrator — 40.6.366
   OPERATOR DASHBOARD · REDIVIDER READY RESET
   REDIVIDER now shows full 100% readiness when its stop owner is available, and cancelling confirmation returns the control to neutral until pointer leave. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406366)return;

  const BUILD="40.6.366";
  const ROOT_ID="agentCryptoOperatorDashboard406366";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406366";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.366.")};
    }
  ...
  function openStopConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406366");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406366");
...
#${ROOT_ID} .kill-mini-ring{
  ...
  background:conic-gradient(from -90deg,
    rgba(167,31,45,.96) 0 calc(var(--redivider-ready) * 1%),
    rgba(85,47,51,.25) 0 100%);
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406366">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406366">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406366">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406366">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406366">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406366">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406366" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406366">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406366">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406366" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406366" aria-describedby="acOperatorRedividerConfirmDesc406366">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406366">REDIVIDER</h2>
    ...
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406366">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406366">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406366" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406366" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406366")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406366")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406366");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406366");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406366");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406366");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406366");
  ...
  globalThis.AgentCryptoOperatorDashboard406366=Object.freeze({
    build:BUILD,
    mount,
    ...
    css_authority:"SINGLE_CLEAN_REWRITE_REDIVIDER_READY_RESET"
  });
})();
