/* Agent-Crypto Administrator — 40.6.368
   OPERATOR DASHBOARD · REDIVIDER TEMPERED GLOW
   State logic from 40.6.367 is preserved, but hover/confirm luminance is deliberately restrained to the darker REDIVIDER visual references. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406368)return;

  const BUILD="40.6.368";
  const ROOT_ID="agentCryptoOperatorDashboard406368";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406368";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.368.")};
    }
  ...
  function openStopConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406368");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406368");
...
#${ROOT_ID}:not([data-kill-hover-suppressed="true"]) .kill-mini-ring:hover{
  ...
  background:conic-gradient(from -90deg,
    rgba(132,30,43,.84) 0 calc(var(--redivider-ready) * 1%),
    rgba(63,44,48,.20) 0 100%);
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406368">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406368">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406368">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406368">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406368">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406368">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406368" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406368">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406368">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406368" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406368" aria-describedby="acOperatorRedividerConfirmDesc406368">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406368">REDIVIDER</h2>
    ...
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406368">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406368">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406368" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406368" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406368")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406368")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406368");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406368");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406368");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406368");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406368");
  ...
  globalThis.AgentCryptoOperatorDashboard406368=Object.freeze({
    build:BUILD,
    mount,
    ...
    css_authority:"SINGLE_CLEAN_REWRITE_REDIVIDER_TEMPERED_GLOW"
  });
})();
