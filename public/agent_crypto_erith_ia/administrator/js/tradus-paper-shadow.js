...
(() => {
  ...
  function onObservation(e){process(e?.detail,"tradus_event");}
  ...
  const api=Object.freeze({build:BUILD,process_observation:process,read:()=>clone(state),summary:()=>summary(),mount,export_journal:exportJournal,reset,self_test:selfTest,assumptions:Object.freeze({start_capital_eur:START_CAPITAL,ticket_eur:TICKET_EUR,fee_rate_per_fill:FEE_RATE}),paper_only:true,real_orders:false,credentials:false,wallet:false,recurring_timer:false,network_owner:false,headless_auto:true,presentation_auto_mount:false});
  globalThis.AgentCryptoTradusPaperShadow=api;load();
  // 40.6.405 — lifecycle Paper automatique, panneau non résident tant qu’il n’est pas demandé.
  if(typeof document!=="undefined"){document.addEventListener(EVENT_NAME,onObservation);}
})();
