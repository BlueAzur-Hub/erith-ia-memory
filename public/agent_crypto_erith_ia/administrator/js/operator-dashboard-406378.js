/* Agent-Crypto Administrator — 40.6.378
   OPERATOR DASHBOARD · MATH VALUE CONTRAST
   Math value contrast: the operator Math Core keeps the canonical Atlas color on the ring, while score and label use a protected high-contrast foreground for Transformer Book / small-display readability. REDIVIDER and Math computation remain unchanged. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406378)return;

  const BUILD="40.6.378";
  const ROOT_ID="agentCryptoOperatorDashboard406378";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406378";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.378.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406378");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406378");
...
#${ROOT_ID} .math-stage{
  min-height:148px;display:grid;grid-template-rows:1fr auto;gap:10px;place-items:center;position:relative;
...
#${ROOT_ID} .math-hud{
  ...
  align-items:center;justify-items:center;gap:7px;position:relative;
}
...
#${ROOT_ID} .math-mini-ring{
  ...
  box-shadow:0 0 0 1px color-mix(in srgb,var(--math-tone) 18%,transparent),0 0 10px color-mix(in srgb,var(--math-tone) 22%,transparent);
}
...
#${ROOT_ID} .mini-math-core{
  ...
  background:
    radial-gradient(circle at 50% 46%,rgba(5,10,16,.96) 0 28%,rgba(6,15,20,.97) 58%,rgba(3,8,12,.99) 100%);
  border:1px solid rgba(111,182,187,.20);
  box-shadow:inset 0 0 18px rgba(0,0,0,.62);
}
#${ROOT_ID} .math-ring .value{
  position:relative;z-index:3;color:#F7FBFF;font-size:27px;line-height:1;font-weight:1000;
  -webkit-text-stroke:.45px rgba(0,0,0,.58);
  text-shadow:0 1px 2px rgba(0,0,0,.92),0 0 6px rgba(0,0,0,.72);
}
#${ROOT_ID} .ring-label,
#${ROOT_ID} .mini-math-label{
  position:relative;z-index:3;max-width:72px;color:#EAF2F5;font-size:6.5px;font-weight:1000;line-height:1.05;text-align:center;
  -webkit-text-stroke:.25px rgba(0,0,0,.54);
  text-shadow:0 1px 2px rgba(0,0,0,.88),0 0 4px rgba(0,0,0,.52);
}
#${ROOT_ID} .math-status{
...
#${ROOT_ID} .mini-score{
  position:relative;z-index:3;color:#F7FBFF;font-size:clamp(23px,1.55vw,27px);line-height:1;font-weight:1000;
  -webkit-text-stroke:.45px rgba(0,0,0,.60);
  text-shadow:0 1px 2px rgba(0,0,0,.94),0 0 6px rgba(0,0,0,.74);
}
#${ROOT_ID} .mini-open{
...
#${ROOT_ID}[data-math-mode="normal"] .math-hover-readout{
  left:16px;width:clamp(292px,23vw,372px);bottom:214px;
}
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406378">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406378">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406378">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406378">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406378">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406378">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406378" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406378">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406378">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406378" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406378" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406378" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406378" aria-describedby="acOperatorRedividerConfirmDesc406378">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406378">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406378">Confirmation de coupure</p>
    <div class="confirm-hud" aria-hidden="true">
      <span class="confirm-rail left"></span>
      <span class="confirm-ring"><span class="confirm-core"><strong>REDIVIDER</strong><span class="confirm-percent">100%</span><span class="confirm-glyphs"><span>◈</span><b>◉</b><span>◈</span></span></span></span>
      <span class="confirm-rail right"></span>
    </div>
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406378">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406378">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406378">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406378" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406378" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406378")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406378")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406378");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406378");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406378");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406378");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406378");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406378");
    if(mathHover)mathHover.textContent="MATH CORE · "+scoreText+"\n"+math.label+"\nAppuyez pour ouvrir";
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406378");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406378");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406378");
  ...
  globalThis.AgentCryptoOperatorDashboard406378=Object.freeze({
    build:BUILD,
    mount,
