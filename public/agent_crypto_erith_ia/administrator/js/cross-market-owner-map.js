...
(() => {
  ...
  function crypto(now){
    let rows=[];try{rows=globalThis.AgentCryptoEventReactionSource?.snapshot?.()?.records||[];}catch(_){}
    ...
    return {domain:"crypto",runtime_owner:"AgentCryptoEventReactionSource + canonical market broker",owner_available:!!globalThis.AgentCryptoEventReactionSource,data_timestamp:latest?.timestamp||null,data_age_ms:a,data_age_state:ageLabel(a),data_scope:"collector snapshot / crypto market",active:true};
  }
  function metals(now){
    const demand=globalThis.AtlasParallelMarketDemand||null;
  ...
  globalThis.AtlasCrossMarketOwnerMap=Object.freeze({build:BUILD,schema:SCHEMA,integrity,snapshot:integrity,read_only:true,new_source:false,new_storage_owner:false,storage_write:false,new_fetch:false,new_timer:false,new_observer:false,financial_signal:false,investment_recommendation:false,automatic_order:false});
})();