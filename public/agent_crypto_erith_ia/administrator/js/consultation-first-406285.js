/* Agent-Crypto Administrator — 40.6.285 P0 BOOT PROBE CAUSAL READINESS
...
(()=>{
  "use strict";
  if(globalThis.AgentCryptoConsultationFirst406285)return;

  const BUILD="40.6.285";
  const AETHER_SRC="./js/aether.js?v=40.6.285";
  ...
  function markSignals(signals){
    ...
    for(const [key,name] of mapping)if(signals[key])try{probe.markOnce(name,{build:BUILD,owner:"consultation-first-406285"});}catch(_){}
  }
  ...
  function loadAether(reason="consultation-ready"){
    ...
    const existing=document.querySelector('script[data-agent-crypto-aether-406285="1"]');
    ...
    script.src=AETHER_SRC;script.async=false;script.dataset.agentCryptoAether406285="1";
  ...
  globalThis.AgentCryptoConsultationFirst406285=Object.freeze({
