/* Agent-Crypto Administrator — 40.6.381
   OPERATOR DASHBOARD · QUIET CHROME + TACTICAL REDIVIDER
   Compact canonical Math: the floating/operator Math Core keeps the exact Atlas Math visual language but reduces only the NORMAL expanded footprint for better balance over the graph. MINI, REDIVIDER and Math computation remain unchanged. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406381)return;

  const BUILD="40.6.381";
  const ROOT_ID="agentCryptoOperatorDashboard406381";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406381";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.381.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406381");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406381");
...
/* 40.6.381 — QUIET CHROME + TACTICAL REDIVIDER
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406381">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406381">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406381">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406381">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406381">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406381">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406381" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406381">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406381">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406381" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406381" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406381" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406381" aria-describedby="acOperatorRedividerConfirmDesc406381">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406381">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406381">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406381">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406381">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406381">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406381" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406381" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406381")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406381")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406381");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406381");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406381");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406381");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406381");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406381");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406381");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406381");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406381");
  ...
  globalThis.AgentCryptoOperatorDashboard406381=Object.freeze({
    build:BUILD,
    mount,
