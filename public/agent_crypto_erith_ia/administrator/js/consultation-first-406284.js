/* Agent-Crypto Administrator — 40.6.284 P0 BOOK CONSULTATION FIRST
   Aether becomes a consumer of the consultation surface, never a parser prerequisite.
   Order: shell/menu/Chronos -> Market/Graph/Technical/Top5/Flow/Snapshot/Math -> Aether -> background.
   Explicit Aether demand remains available. Same application; no Book-lite fork; no business logic change. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoConsultationFirst406284)return;

  const BUILD="40.6.284";
  const AETHER_SRC="./js/aether.js?v=40.6.284";
  ...
  function signalState(){
    ...
    return {
      ...
      market_flow:Boolean(meaningful(flow)&&!/livecheck requis|aucune donnée chiffrée chargée/i.test(flow)),
  ...
  function markSignals(signals){
    ...
    for(const [key,name] of mapping)if(signals[key])try{probe.markOnce(name,{build:BUILD,owner:"consultation-first-406284"});}catch(_){}
  }
  ...
  function loadAether(reason="consultation-ready"){
    ...
    const existing=document.querySelector('script[data-agent-crypto-aether-406284="1"]');
    ...
    script.src=AETHER_SRC;script.async=false;script.dataset.agentCryptoAether406284="1";
  ...
  globalThis.AgentCryptoConsultationFirst406284=Object.freeze({
    ...
    aether_parser_blocking:false,explicit_aether_demand:true,recurring_timer:false,bounded_probe_timer:true,observer:false,business_logic_changed:false,market_core_changed:false,book_lite:false
  });
})();