(() => {
  ...
  /* ============================================================
     39.9.0R1 — MEMORY ARCHITECTURE FREEZE / PREFLIGHT
  ...
  const BUILD_3990R1 = "39.9.0R1";
  const ENGINE_3990R1 = "38.15.11";
  const TOKEN_3990R1 = `market-core-v2.0-alpha-build-${BUILD_3990R1}`;
  const ROOT_ID = "atlasArchitectureFreeze3990R1";
  ...
  function versionedLocalScripts() {
    ...
    const stale = rows.filter(row => !row.raw.includes(BUILD_3990R1));
    return { rows, stale };
  }
  ...
  function versionedAdminStyles() {
    ...
    const states = required.map(name => {
      ...
      return {
        ...
        current: !!row && row.raw.includes(BUILD_3990R1),
        raw: row?.raw || "absent"
      };
    });
    return states;
  ...
  function scriptOrder() {
    const expected = [
      ...
      "./js/app.js",
      "./js/layout-repair-39.9.0R1.js",
      "./js/architecture-freeze-39.9.0R1.js"
    ];
  ...
  function derive() {
    ...
    const layoutContract = globalThis.__AGENT_CRYPTO_LAYOUT_REPAIR_3990R1__ || null;
    ...
    const checks = [
      check("Build runtime", build === BUILD_3990R1, `ATLAS_BUILD=${build || "—"}`),
      check("Token runtime", token === TOKEN_3990R1, `ATLAS_ASSET_TOKEN=${token || "—"}`),
      ...
      check("Meta atlas-build", meta("atlas-build") === BUILD_3990R1, `meta=${meta("atlas-build") || "—"}`),
      check("Meta administrator-build", meta("administrator-build") === BUILD_3990R1, `meta=${meta("administrator-build") || "—"}`),
      check("Meta asset token", meta("atlas-asset-token") === TOKEN_3990R1, `meta=${meta("atlas-asset-token") || "—"}`),
      check("Engine lock", meta("atlas-engine-build") === ENGINE_3990R1, `engine=${meta("atlas-engine-build") || "—"}`),
      check("DOM build identity", bodyBuild === BUILD_3990R1 || rootBuild === BUILD_3990R1, `body=${bodyBuild || "—"} · html=${rootBuild || "—"}`),
      ...
      check("Layout repair 39.9.0R1", !!globalThis.atlasAdministratorLayoutRepair3990R1 && hasScriptSuffix("/js/layout-repair-39.9.0R1.js"), "module DOM dédié + script unique"),
      ...
      check("Rétrospective isolée", !!board && !!retro && !board.contains(retro), retro ? "Retrospective 39.6 hors grille Decision Board · aucun chevauchement de zone decision-memory" : "bloc rétrospectif absent"),
      check("Ancien Freeze non chargé", countScriptSuffix("/js/architecture-freeze-39.9.0.js") === 0, "39.9.0 historique non chargé par l’interface R1"),
      check("Freeze singleton", hasScriptSuffix("/js/architecture-freeze-39.9.0R1.js"), "script 39.9.0R1 unique")
    ];
  ...
  function ensureRoot() {
    ...
    root.setAttribute("aria-labelledby", "architectureFreezeTitle3990R1");
    root.innerHTML = `
      <div class="atlas-memory-intelligence-head">
        <div>
          <p class="eyebrow">ARCHITECTURE FREEZE · 39.9.0R1 · READ ONLY</p>
          <h5 id="architectureFreezeTitle3990R1">Contrôle global avant 40.0</h5>
          <p>Vérifie que les briques validées sont présentes, alignées et non contradictoires. Aucun correctif automatique.</p>
        </div>
        <span class="pill warn" id="architectureFreezeBadge3990R1">En attente</span>
      </div>
      <div class="atlas-memory-ledger-35">
        <article><span>Contrôles</span><b id="architectureFreezeCount3990R1">—</b><small>PASS réellement exécutés dans ce navigateur.</small></article>
        <article><span>Critiques</span><b id="architectureFreezeCritical3990R1">—</b><small>Un seul FAIL critique interdit le passage en 40.0.</small></article>
        <article><span>Limites</span><b id="architectureFreezeWarnings3990R1">—</b><small>Couverture ou données manquantes : visibles mais non maquillées en panne.</small></article>
        <article><span>Verdict</span><b id="architectureFreezeState3990R1">—</b><small>Préflight local ; validation Firefox opérateur encore requise.</small></article>
      </div>
      <div class="atlas-memory-intelligence-grid" id="architectureFreezeGrid3990R1"></div>
      <div class="atlas-memory-intelligence-actions">
        <button type="button" id="btnArchitectureFreezeRefresh3990R1">Relancer le contrôle</button>
        <button type="button" id="btnArchitectureFreezeExport3990R1">Exporter Freeze Audit .md</button>
      </div>
      <p id="architectureFreezeContract3990R1">Aucun verdict tant que le préflight n’a pas été exécuté.</p>`;

    anchor.insertAdjacentElement("afterend", root);
    byId("btnArchitectureFreezeRefresh3990R1")?.addEventListener("click", render);
    byId("btnArchitectureFreezeExport3990R1")?.addEventListener("click", exportMarkdown);
    return root;
  }
  ...
  function render() {
    ...
    setText("architectureFreezeCount3990R1", `${passed}/${data.checks.length} PASS`);
    setText("architectureFreezeCritical3990R1", data.criticalFails.length ? `${data.criticalFails.length} FAIL` : "0 FAIL");
    setText("architectureFreezeWarnings3990R1", data.warnings.length ? `${data.warnings.length} limite(s)` : "0");
    setText("architectureFreezeState3990R1", data.label);
    setText("architectureFreezeContract3990R1", data.pass
      ? "39.9.0R1 : architecture consolidée. Les limites de couverture restent visibles ; aucune n’est transformée en faux défaut structurel."
      : "FREEZE REFUSÉ : corriger les FAIL critiques avant toute préparation 40.0.");

    const badge = byId("architectureFreezeBadge3990R1");
    if (badge) {
      badge.textContent = data.label;
      badge.className = `pill ${data.pass ? (data.warnings.length ? "warn" : "ok") : "fail"}`;
    }

    const grid = byId("architectureFreezeGrid3990R1");
    if (grid) {
    ...
    root.dataset.build = BUILD_3990R1;
  ...
  function markdown(data = derive()) {
    const lines = [
      "# Agent-Crypto — Architecture Freeze 39.9.0R1", "",
  ...
  function exportMarkdown() {
    ...
    const name = `agent_crypto_architecture_freeze_39_9_0R1_${stamp}.md`;
  ...
  const freezeContract3990R1 = Object.freeze({
    build: BUILD_3990R1,
  ...
  globalThis.__AGENT_CRYPTO_ARCHITECTURE_FREEZE_3990R1__ = freezeContract3990R1;
  globalThis.atlasArchitectureFreeze3990R1 = Object.freeze({ derive, render, markdown });
