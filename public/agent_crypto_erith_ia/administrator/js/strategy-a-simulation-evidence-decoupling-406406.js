/* Agent-Crypto Administrator — 40.6.406 SIMULATION / EVIDENCE DECOUPLING
   Presentation + explicit-demand routing only.
   Goals:
   - expose Strategy A headless state outside Simulation;
...
(() => {
  ...
  if (globalThis.AgentCryptoSimulationEvidenceDecoupling406406) return;

  const BUILD = "40.6.406";
  const OWNER = "strategy-a-simulation-evidence-decoupling-406406.js";
  const STATUS_ID = "strategyAAutoHeadlessStatus406406";
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      #${STATUS_ID}{
        cursor:default;
        white-space:nowrap;
        border-color:rgba(111,225,195,.32)!important;
        background:rgba(30,111,91,.16)!important;
        color:#b8ffe8!important;
      }
      #${STATUS_ID}[data-state="WAIT"]{
        border-color:rgba(255,211,92,.32)!important;
        background:rgba(116,87,22,.16)!important;
        color:#ffe398!important;
      }
      #${STATUS_ID}[data-state="STOP"]{
        border-color:rgba(255,102,126,.38)!important;
        background:rgba(115,31,50,.20)!important;
        color:#ffb3c1!important;
      }
      #${STATUS_ID}[data-state="PENDING"]{
        border-color:rgba(120,171,206,.26)!important;
        background:rgba(34,65,86,.16)!important;
        color:#b7d3e4!important;
      }
      #${SUMMARY_ID}{
        margin:9px 0;
        padding:10px;
  ...
  function renderHeadlessStatus() {
    ...
    if (!node) {
      ...
      const engine = byId("administratorEngineBadge");
      if (engine?.parentNode === stack) engine.insertAdjacentElement("afterend", node);
  ...
  function render() {
    ensureStyle();
    const status = renderHeadlessStatus();
  ...
  globalThis.AgentCryptoSimulationEvidenceDecoupling406406 = Object.freeze({
    build: BUILD,
    owner: OWNER,
    render,
