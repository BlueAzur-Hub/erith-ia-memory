/* Agent-Crypto Administrator — 40.6.367
   OPERATOR DASHBOARD · REDIVIDER STATE LIGHTING
   The ring remains 100% full when ready; luminance now carries the state: dim=ready, bright=armed/hover, muted=stopped/unavailable. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406367)return;

  const BUILD="40.6.367";
  const ROOT_ID="agentCryptoOperatorDashboard406367";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406367";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.367.")};
    }
  ...
  function openStopConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406367");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406367");
...
#${ROOT_ID} .kill-mini-ring{
  ...
  background:conic-gradient(from -90deg,
    rgba(104,30,40,.72) 0 calc(var(--redivider-ready) * 1%),
    rgba(62,45,49,.20) 0 100%);
...
#${ROOT_ID}[data-kill-lamp="stopped"] .kill-mini-ring{
  filter:brightness(.60) saturate(.46);
  box-shadow:0 0 0 1px rgba(101,67,72,.12);
}
...
#${ROOT_ID}[data-kill-lamp="confirm"] .kill-mini-ring{
  ...
  box-shadow:0 0 0 1px rgba(205,50,64,.30),0 0 20px rgba(166,16,36,.34),0 0 38px rgba(103,9,24,.13);
}
...
#${ROOT_ID}:not([data-kill-hover-suppressed="true"]) .kill-mini-ring:hover{
  ...
  background:conic-gradient(from -90deg,
    rgba(200,39,54,.98) 0 calc(var(--redivider-ready) * 1%),
    rgba(73,46,50,.22) 0 100%);
  box-shadow:0 0 0 1px rgba(214,54,68,.34),0 0 20px rgba(177,18,38,.42),0 0 38px rgba(111,9,25,.16);
}
...
#${ROOT_ID}:not([data-kill-hover-suppressed="true"]) .kill-mini-ring:hover .redivider-core{
  background:radial-gradient(circle at 50% 42%,rgba(146,19,33,.26),rgba(16,9,13,.96) 60%,rgba(5,6,8,.99));
  border-color:rgba(172,48,59,.42);
  box-shadow:inset 0 0 22px rgba(95,8,20,.34);
}
...
#${ROOT_ID}:not([data-kill-hover-suppressed="true"]) .kill-mini-ring:hover .redivider-brand{
  color:#f0ded9;text-shadow:0 0 6px rgba(191,39,54,.26);
}
...
#${ROOT_ID}:not([data-kill-hover-suppressed="true"]) .kill-mini-ring:hover .redivider-glyphs{
  color:#b73340;text-shadow:0 0 7px rgba(155,15,32,.38);
}
...
#${ROOT_ID}:not([data-kill-hover-suppressed="true"]) .kill-mini-ring:hover .redivider-glyphs b{
  color:#e03a49;text-shadow:0 0 10px rgba(185,20,39,.52);
}
...
#${ROOT_ID}[data-kill-hover-suppressed="true"] .kill-mini-ring{
  ...
  background:conic-gradient(from -90deg,
    rgba(104,30,40,.72) 0 calc(var(--redivider-ready) * 1%),
    rgba(62,45,49,.20) 0 100%) !important;
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406367">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406367">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406367">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406367">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406367">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406367">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406367" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406367">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406367">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406367" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406367" aria-describedby="acOperatorRedividerConfirmDesc406367">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406367">REDIVIDER</h2>
    ...
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406367">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406367">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406367" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406367" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406367")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406367")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406367");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406367");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406367");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406367");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406367");
    if(context)context.textContent=math.context;

    const ks=stopState();
    const readyPct=ks.available?100:0;
    ...
    root.dataset.killLamp=!ks.available
      ?"unavailable"
      :(ks.persisted||ks.enabled===false)
        ?"stopped"
        :state.confirm_open
  ...
  globalThis.AgentCryptoOperatorDashboard406367=Object.freeze({
    build:BUILD,
    mount,
    ...
    redivider_ready_ring:"100_PERCENT_WHEN_OWNER_AVAILABLE",
    redivider_lighting_logic:"READY_DIM__HOVER_BRIGHT__CONFIRM_BRIGHT__STOPPED_MUTED__UNAVAILABLE_OFF",
    readiness_encoded_by:"RING_FILL",
    interaction_encoded_by:"LUMINANCE",
    ...
    css_authority:"SINGLE_CLEAN_REWRITE_REDIVIDER_STATE_LIGHTING"
  });
})();
