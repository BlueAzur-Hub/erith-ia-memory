/* Agent-Crypto Administrator — 40.6.375
   OPERATOR DASHBOARD · KILL SWITCH STATE TRUTH
   Kill Switch state truth: REDIVIDER is treated as stopped only after an explicit operator stop/manual-stop latch; transient Auto A OFF/boot states no longer invert the control into resume mode. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406375)return;

  const BUILD="40.6.375";
  const ROOT_ID="agentCryptoOperatorDashboard406375";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406375";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.375.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406375");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406375");
...
#${ROOT_ID} .operator-hover-readout{
  ...
  display:grid;place-items:center;padding:7px 12px;border-radius:11px;
...
#${ROOT_ID} .math-hover-readout{
  left:16px;justify-items:center;text-align:center;color:#8eeff0;border:1px solid rgba(55,221,226,.24);
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406375">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406375">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406375">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406375">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406375">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406375">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406375" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406375">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406375">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406375" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406375" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406375" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406375" aria-describedby="acOperatorRedividerConfirmDesc406375">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406375">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406375">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406375">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406375">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406375">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406375" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406375" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406375")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406375")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406375");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406375");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406375");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406375");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406375");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406375");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406375");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406375");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406375");
  ...
  globalThis.AgentCryptoOperatorDashboard406375=Object.freeze({
    build:BUILD,
    mount,
