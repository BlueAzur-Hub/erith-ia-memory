...
(() => {
  ...
  function replay(){return globalThis.AgentCryptoStrategyAReplay||null;}
  function probe(){
    const r=replay();
    const p=r?.policy||{};
  ...
  function ensureStyle(){
    if(typeof document==='undefined'||document.getElementById('strategyACanonicalSpecStyle'))return;
    const s=document.createElement('style'); s.id='strategyACanonicalSpecStyle'; s.textContent=`
      #strategyACanonicalSpec{margin:8px 0;padding:8px 10px;border:1px solid rgba(98,236,255,.20);border-radius:10px;background:rgba(5,15,27,.46)}
      #strategyACanonicalSpec .sacs-row{display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap}
      #strategyACanonicalSpec .sacs-title{font:950 9px/1.2 system-ui,sans-serif;letter-spacing:.08em;color:#9eefff}
      #strategyACanonicalSpec .sacs-state{font:900 9px/1 system-ui,sans-serif;padding:4px 7px;border-radius:999px;border:1px solid rgba(255,255,255,.12)}
      #strategyACanonicalSpec[data-status="OK"] .sacs-state{color:#82f5b9;border-color:rgba(130,245,185,.35)}
      #strategyACanonicalSpec[data-status="DRIFT"] .sacs-state{color:#ff9b9b;border-color:rgba(255,120,120,.4)}
      #strategyACanonicalSpec .sacs-sub{margin-top:5px;font:500 8px/1.35 system-ui,sans-serif;color:#8da7b8}`; document.head.appendChild(s);
  }
  function render(){
    if(typeof document==='undefined')return false; ensureStyle();
    const anchor=document.getElementById('strategyAReplaySandbox')||document.getElementById('strategyAExperimentLedger')||document.getElementById('strategyADecisionTrace');
    if(!anchor)return false;
    let panel=document.getElementById('strategyACanonicalSpec');
    if(!panel){panel=document.createElement('section');panel.id='strategyACanonicalSpec';anchor.insertAdjacentElement('beforebegin',panel);}
