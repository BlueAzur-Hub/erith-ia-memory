(() => {
  ...
  /* ============================================================
     40.0.0R6 — GRAPH WINDOW MENU OPAQUE RESTORE / FULL FUNCTION RECOVERY PREFLIGHT
     ...
     - Verify publication identity, runtime truth, module wiring,
       read-only contracts, script order, protected UI anchors, the canonical ghost base for non-Graphique windows, and the explicit opaque Graphique restore contract.
  ...
  function scriptOrder() {
    const expected = [
      ...
      "./js/architecture-freeze-40.0.0R6.js"
    ];
  ...
  function graphOpaqueRecoveryCssContract() {
    ...
    try {
      const walk = rules => {
        for (const rule of [...(rules || [])]) {
          ...
          if (!selector.includes("#analyste.admin-native-anchor > .admin-native-controls")) continue;
          const display = String(rule.style?.display || "").trim().toLowerCase();
          const opacity = String(rule.style?.opacity || "").trim();
          const visibility = String(rule.style?.visibility || "").trim().toLowerCase();
          const pointer = String(rule.style?.pointerEvents || "").trim().toLowerCase();
          if (display === "flex" && (opacity === "1" || opacity === "1.0") && visibility === "visible" && pointer === "auto") {
            hit = { selector, display, opacity, visibility, pointer };
          }
        }
      };
      walk(sheet.cssRules);
    ...
    return hit ? { ok:true, detail:`${hit.selector} · display=${hit.display} · opacity=${hit.opacity}` } : { ok:false, detail:"override opaque Graphique absent" };
  }
  ...
  function derive() {
    ...
    const graphOpaqueCss = graphOpaqueRecoveryCssContract();
    ...
    const checks = [
      ...
      check("Base CSS 39.2.11 intacte pour les autres menus", chromeCss.ok === true, chromeCss.detail || "contrat CSS absent"),
      check("Override Graphique opaque R6", graphOpaqueCss.ok === true, graphOpaqueCss.detail || "override opaque absent"),
      ...
      check("Graphique menu opaque restauré", !!graphChrome && graphChrome.complete && graphChrome.interactive && Number.isFinite(graphChrome.computedOpacity) && graphChrome.computedOpacity >= 0.95, graphChrome ? `5/5=${String(graphChrome.complete)} · display actif · opacity runtime=${Number.isFinite(graphChrome.computedOpacity) ? graphChrome.computedOpacity.toFixed(2) : "—"}` : "chrome Graphique absent"),
      ...
      check("Layout repair 40.0.0R6", !!globalThis.atlasAdministratorLayoutRepair3990R2 && hasScriptSuffix("/js/layout-repair-39.9.0R2.js"), "module DOM final + script unique"),
      ...
      check("Freeze singleton", hasScriptSuffix("/js/architecture-freeze-40.0.0R6.js"), "script 40.0.0R6 unique")
    ];
  ...
  function ensureRoot() {
    ...
    root.innerHTML = `
      <div class="atlas-memory-intelligence-head">
        <div>
          <p class="eyebrow">GRAPH MENU OPAQUE RESTORE · 40.0.0R6 · READ ONLY</p>
  ...
  function render() {
    ...
    setText("architectureFreezeContract4000", data.pass
      ? "40.0.0R6 : Graphique + Lecture technique garde ses commandes métalliques 5/5 et son menu est opaque au repos ; Target Top, Market Flow, Market Snapshot, Math Core et le dock conservent leur comportement ghost validé ; aucune règle display:none destructive. Les limites de couverture restent visibles ; aucune n’est transformée en faux défaut structurel."
  ...
  function markdown(data = derive()) {
    const lines = [
      "# Agent-Crypto — Graph Window Menu Opaque Restore 40.0.0R6", "",
  ...
  function exportMarkdown() {
    ...
    const name = `agent_crypto_window_menu_recovery_40_0_0R5_${stamp}.md`;
  ...
  const freezeContract4000 = Object.freeze({
    build: BUILD_3990R2,
    role: "read-only validated metallic window-menu recovery over protected 40.0 architecture",
    feature_delta: false,
  ...
  globalThis.atlasArchitectureFreeze4000R6 = Object.freeze({ derive, render, markdown });
