/* Agent-Crypto Administrator — 40.6.377
   OPERATOR DASHBOARD · ATLAS MATH COLOR PARITY
   Atlas Math color parity: the operator Math Core now follows the canonical Atlas Math Core score color band; all 40.6.376 centering and REDIVIDER behavior remain unchanged. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406377)return;

  const BUILD="40.6.377";
  const ROOT_ID="agentCryptoOperatorDashboard406377";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406377";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.377.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406377");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406377");
...
#${ROOT_ID} .math-ring .value{
  color:var(--math-tone);font-size:27px;line-height:1;font-weight:1000;
  text-shadow:0 0 5px color-mix(in srgb,var(--math-tone) 30%,transparent);
}
#${ROOT_ID} .ring-label,
#${ROOT_ID} .mini-math-label{
  max-width:72px;color:var(--math-tone);font-size:6.5px;font-weight:900;line-height:1.05;text-align:center;
  text-shadow:0 0 3px color-mix(in srgb,var(--math-tone) 22%,transparent);
}
#${ROOT_ID} .math-status{
...
#${ROOT_ID} .mini-score{
  color:var(--math-tone);font-size:23px;line-height:1;font-weight:1000;
  text-shadow:0 0 5px color-mix(in srgb,var(--math-tone) 30%,transparent);
}
#${ROOT_ID} .mini-open{
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406377">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406377">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406377">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406377">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406377">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406377">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406377" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406377">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406377">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406377" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406377" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406377" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406377" aria-describedby="acOperatorRedividerConfirmDesc406377">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406377">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406377">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406377">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406377">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406377">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406377" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406377" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406377")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406377")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406377");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406377");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406377");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406377");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406377");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406377");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406377");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406377");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406377");
  ...
  globalThis.AgentCryptoOperatorDashboard406377=Object.freeze({
    build:BUILD,
    mount,
