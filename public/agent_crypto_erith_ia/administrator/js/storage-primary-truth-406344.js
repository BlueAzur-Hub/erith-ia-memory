/* Agent-Crypto Administrator — 40.6.344
...
(()=>{
  "use strict";
  if(globalThis.AgentCryptoStoragePrimaryTruth406344)return;

  const BUILD="40.6.344";
  ...
  async function retire(){
    ...
    const reclaim=candidates.reduce((s,r)=>s+r.local_bytes,0);
    const ok=window.confirm("40.6.344 · PRIMARY IndexedDB vérifié. Retirer uniquement "+candidates.length+" ancienne(s) copie(s) localStorage (~"+fmt(reclaim)+") ? Backup complet exporté. IndexedDB/Market/Evidence conservés.");
    ...
    const box=document.getElementById("atlasStoragePrimaryTruth289");
    const badge=document.getElementById("atlasStoragePrimaryTruthState289");
    if(box)box.textContent=["TERMINÉ · 40.6.344","localStorage : "+fmt(before.bytes)+" → "+fmt(after.bytes)+" · libéré "+fmt(result.reclaimed_bytes),"",...rows.map(r=>"- "+r.key+" · "+r.state+" · lecture PRIMARY après retrait : "+(r.read_after_ok?"OK":"À VÉRIFIER")),"","IndexedDB conservé. Market REVIEW_REQUIRED conservé."].join("\n");
  ...
  globalThis.AgentCryptoStoragePrimaryTruth406344=Object.freeze({
