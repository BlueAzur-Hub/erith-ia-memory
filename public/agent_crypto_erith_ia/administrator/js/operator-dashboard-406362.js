/* Agent-Crypto Administrator — 40.6.362
   OPERATOR DASHBOARD · MATH CORE BALANCE
   Single DOM/CSS authority. REDIVIDER is frozen from the prior runtime; Math V3 NORMAL is rebalanced into the same centered instrument grammar. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406362)return;

  const BUILD="40.6.362";
  const ROOT_ID="agentCryptoOperatorDashboard406362";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406362";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.362.")};
    }
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406362">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406362">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406362">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406362">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406362">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406362">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406362" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406362">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406362">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
`;
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406362");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406362");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406362");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406362");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406362");
  ...
  globalThis.AgentCryptoOperatorDashboard406362=Object.freeze({
    build:BUILD,
    mount,
    ...
    css_authority:"SINGLE_CLEAN_REWRITE_MATH_BALANCE_REDIVIDER_FROZEN"
  });
})();
