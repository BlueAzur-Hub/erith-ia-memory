...
(() => {
  ...
  function snapshot(){
    const acceptance=api("AtlasDecisionIntelligenceAcceptance")?.matrix?.()||null;
    const explain=api("AtlasDecisionExplainability")?.current?.()||null;
    const cross=api("AtlasCrossMarketOwnerMap")?.integrity?.()||null;
    ...
    const checks={
      ...
      no_advice_output:api("AtlasDecisionExplainability")?.investment_recommendation===false&&api("AtlasDecisionExplainability")?.execution_authorized===false,
      capital_survival_no_execution:api("AtlasCapitalSurvival")?.execution_authorized===false&&api("AtlasCapitalSurvival")?.automatic_order===false,
  ...
  globalThis.AgentCryptoCanonicalFreeze=Object.freeze({build:BUILD,schema:SCHEMA,snapshot,read_only:true,freeze:true,new_source:false,new_storage_owner:false,storage_write:false,new_fetch:false,new_timer:false,new_observer:false,investment_recommendation:false,execution_authorized:false,automatic_order:false});
})();