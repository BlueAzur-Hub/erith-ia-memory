/* Agent-Crypto Administrator — 40.6.359
   OPERATOR DASHBOARD · ATLAS QUIET GLASS
   Single DOM/CSS authority. Geometry frozen; chroma reduced to neutral graphite with thin Atlas-style state light. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406359)return;

  const BUILD="40.6.359";
  const ROOT_ID="agentCryptoOperatorDashboard406359";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406359";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.359.")};
    }
...
#${ROOT_ID}{
  ...
  --cyan:#73d9e8;--cyan-hot:#a9eef4;--red:#c56d7d;--red-hot:#dda0aa;--gold:#d9bf7b;
  --math-score:0;
}
...
#${ROOT_ID} .kill-wing .panel{
  border-color:rgba(190,154,162,.24);
  background:linear-gradient(145deg,rgba(15,12,15,.31),rgba(7,8,11,.24));
  box-shadow:0 14px 30px rgba(0,0,0,.29),inset 0 0 0 1px rgba(255,255,255,.016);
}
...
#${ROOT_ID} .kill-wing .panel:before{
  ...
  background:linear-gradient(270deg,rgba(197,109,125,.66),rgba(217,191,123,.30),transparent);
  filter:drop-shadow(0 0 3px rgba(197,109,125,.22));
}
#${ROOT_ID} .head{
...
#${ROOT_ID} .kill-wing .kicker{
  color:#d8b0b7;text-shadow:0 0 3px rgba(197,109,125,.20);
}
...
#${ROOT_ID} .math-mini-ring{
  ...
  background:conic-gradient(from -90deg,
    rgba(115,217,232,.86) calc(var(--math-score) * 1%),
    rgba(145,170,176,.10) 0 100%);
  box-shadow:0 0 7px rgba(115,217,232,.18);
}
#${ROOT_ID} .math-ring{width:82px;height:82px;flex:0 0 82px}
#${ROOT_ID} .ring-core{
  width:100%;height:100%;border-radius:50%;display:grid;place-items:center;
  background:radial-gradient(circle at 42% 26%,rgba(153,184,190,.10),rgba(16,25,30,.14) 40%,rgba(5,9,13,.93) 78%);
  border:1px solid rgba(133,180,188,.22);
}
#${ROOT_ID} .math-ring .value{
  color:#9ee5ec;font-size:21px;font-weight:1000;
  text-shadow:0 0 3px rgba(115,217,232,.24);
}
#${ROOT_ID} .math-copy{min-width:0}
#${ROOT_ID} .math-copy b{
  display:block;color:#aee2e7;font-size:12px;
  text-shadow:0 0 2px rgba(115,217,232,.18);
}
...
#${ROOT_ID} .kill-mini-ring{
  ...
  background:conic-gradient(from -90deg,
    rgba(197,109,125,.78) 0 79%,
    rgba(217,191,123,.68) 79% 82%,
    rgba(197,109,125,.78) 82% 100%);
  box-shadow:0 0 7px rgba(197,109,125,.16);
...
#${ROOT_ID} .kill-core{
  ...
  background:radial-gradient(circle at 45% 28%,rgba(174,154,159,.10),rgba(34,24,28,.34) 48%,rgba(10,8,11,.95) 80%);
  border:1px solid rgba(190,154,162,.22);
}
#${ROOT_ID} .kill-core .kill-word{color:#d9bf7b;font-size:9px;font-weight:1000;text-shadow:0 0 2px rgba(217,191,123,.22)}
#${ROOT_ID} .kill-core .switch-word{color:#d8a1ab;font-size:17px;font-weight:1000;text-shadow:0 0 2px rgba(197,109,125,.20)}
...
#${ROOT_ID} .mini-math-core{
  ...
  background:radial-gradient(circle at 44% 26%,rgba(154,183,188,.10),rgba(16,25,30,.13) 42%,rgba(5,9,13,.95) 78%);
  border:1px solid rgba(133,180,188,.22);
}
#${ROOT_ID} .mini-title{
  color:#d9bf7b;font-size:7.5px;font-weight:1000;letter-spacing:.09em;text-shadow:0 0 2px rgba(217,191,123,.20);
}
#${ROOT_ID} .mini-score{
  color:#9ee5ec;font-size:19px;font-weight:1000;
  text-shadow:0 0 3px rgba(115,217,232,.22);
}
...
#${ROOT_ID} .math-recall{
  left:-8px;color:#a9e3e9;border:1px solid rgba(115,217,232,.36);
}
#${ROOT_ID} .kill-recall{
  right:-8px;color:#d9aab2;border:1px solid rgba(197,109,125,.34);
}
...
<section class="wing math-wing" aria-label="Math Core synthétique">
  <div class="normal panel">
    ...
    <div class="body">
      <div class="math-row">
        <div class="math-ring"><div class="ring-core"><span class="value" id="acOperatorMathScore406359">—</span></div></div>
        <div class="math-copy">
          <b id="acOperatorMathLabel406359">En attente</b>
          <small id="acOperatorMathContext406359">Contexte Market : en attente</small>
        </div>
      </div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        ...
        <span class="mini-score" id="acOperatorMathMiniScore406359">—</span>
      </span>
    </button>
  </div>
</section>
      ...
      <div class="kill-layout">
        <div>
          <div class="kill-state">
            <strong id="acOperatorKillState406359">PRÊT</strong>
            <span class="paper" id="acOperatorPaperOnly406359">PAPER ONLY</span>
          </div>
          <div class="kill-detail" id="acOperatorKillDetail406359">Auto A Paper · lecture et monitoring préservés.</div>
        </div>
...
`;
  ...
  function render(){
    ...
    for(const id of ["acOperatorMathScore406359","acOperatorMathMiniScore406359"]){
      const node=byId(id);
      if(node)node.textContent=scoreText;
    }
    const label=byId("acOperatorMathLabel406359");
    if(label)label.textContent=math.label;
    const context=byId("acOperatorMathContext406359");
  ...
  globalThis.AgentCryptoOperatorDashboard406359=Object.freeze({
    build:BUILD,
    mount,
    ...
    chroma_reduced:true,
    css_authority:"SINGLE_CLEAN_REWRITE_ATLAS_QUIET_GLASS"
  });
})();
