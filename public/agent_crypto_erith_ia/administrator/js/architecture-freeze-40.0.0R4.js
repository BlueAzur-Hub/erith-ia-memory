(() => {
  ...
  /* ============================================================
     40.0.0R4 — EXACT GRAPH WINDOW CHROME RECOVERY / PREFLIGHT
     ...
     - Verify publication identity, runtime truth, module wiring,
       read-only contracts, script order, protected UI anchors and the exact validated 39.2.11 hover-ghost chrome contract.
  ...
  function scriptOrder() {
    const expected = [
      ...
      "./js/architecture-freeze-40.0.0R4.js"
    ];
  ...
  function derive() {
    ...
    const checks = [
      ...
      check("Contrat CSS hover ghost canonique 39.2.11", chromeCss.ok === true, chromeCss.detail || "contrat CSS absent"),
      check("Correctif destructeur Graphique neutralisé", !!byId("administrator-graph-window-chrome-restore-40-0-0-r4"), "display:none 39.4.5R4 neutralisé sans toucher au contrat d’opacité/hover"),
      check("Graphique ghost au repos", !!graphChrome && graphChrome.interactive && Number.isFinite(graphChrome.computedOpacity) && graphChrome.computedOpacity > 0 && graphChrome.computedOpacity <= 0.20, graphChrome ? `display actif · opacity runtime=${Number.isFinite(graphChrome.computedOpacity) ? graphChrome.computedOpacity.toFixed(2) : "—"}` : "chrome Graphique absent"),
      ...
      check("Layout repair 40.0.0R4", !!globalThis.atlasAdministratorLayoutRepair3990R2 && hasScriptSuffix("/js/layout-repair-39.9.0R2.js"), "module DOM final + script unique"),
      ...
      check("Freeze singleton", hasScriptSuffix("/js/architecture-freeze-40.0.0R4.js"), "script 40.0.0R4 unique")
    ];
  ...
  function ensureRoot() {
    ...
    root.innerHTML = `
      <div class="atlas-memory-intelligence-head">
        <div>
          <p class="eyebrow">GRAPH WINDOW RESTORE · 40.0.0R4 · READ ONLY</p>
  ...
  function render() {
    ...
    setText("architectureFreezeContract4000", data.pass
      ? "40.0.0R4 : Graphique rétabli sans display:none ; commandes 5/5 fonctionnelles, ghost au repos et révélation au survol. Les limites de couverture restent visibles ; aucune n’est transformée en faux défaut structurel."
  ...
  function markdown(data = derive()) {
    const lines = [
      "# Agent-Crypto — Exact Graph Window Chrome Recovery 40.0.0R4", "",
  ...
  const freezeContract4000 = Object.freeze({
    build: BUILD_3990R2,
    role: "read-only exact Graphique window chrome recovery over validated 40.0 architecture",
    feature_delta: false,
  ...
  globalThis.atlasArchitectureFreeze4000R4 = Object.freeze({ derive, render, markdown });
