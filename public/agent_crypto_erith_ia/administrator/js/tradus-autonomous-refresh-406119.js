...
(() => {
  ...
  function status(){return clone({build:BUILD,running,page_visible:typeof document!=="undefined"?!document.hidden:false,interval_ms:INTERVAL_MS,cycles,last_at:lastAt,last_trigger:lastTrigger,last_error:lastError,owner_available:!!shadow(),visible_contract_synced:syncVisibleContract(),paper_only:true,shadow_only:true,real_orders:false,credentials:false,wallet:false,strategy_a_mutated:false,storage_write:false,recurring_timer:true});}
  globalThis[API_KEY]=Object.freeze({build:BUILD,interval_ms:INTERVAL_MS,start,stop,run_once:()=>cycle("explicit_run_once"),status,sync_visible_contract:syncVisibleContract,paper_only:true,shadow_only:true,real_orders:false,credentials:false,wallet:false,strategy_a_mutated:false,storage_write:false,recurring_timer:true});
  if(typeof document!=="undefined"){ensure120();document.addEventListener("visibilitychange",()=>{if(document.hidden)clearTimer();else if(running){syncVisibleContract();schedule(250);}},{passive:true});document.addEventListener("agentcrypto:tradus-shadow-observation",syncVisibleContract,{passive:true});window.addEventListener("pagehide",()=>stop("pagehide"),{passive:true});window.addEventListener("pageshow",()=>{ensure120();start("pageshow");},{passive:true});const boot=()=>{ensure120();start("boot");};if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",boot,{once:true});else boot();}
})();
