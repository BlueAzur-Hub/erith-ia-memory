(() => {
  ...
  /* ============================================================
     40.0.0R1 — WINDOW CONTROLS FUNCTIONAL RECOVERY / PREFLIGHT
  ...
  function versionedAdminStyles() {
    const required = ["admin-windows.css", "admin-visual-assets.css", "admin-ribbons.css", "admin-window-controls-recovery-40.0.0R1.css"];
  ...
  function scriptOrder() {
    const expected = [
      ...
      "./js/architecture-freeze-40.0.0R1.js"
    ];
  ...
  function derive() {
    ...
    const recoveryStyle = styleRows().filter(row => pathOnly(row.raw).endsWith("/admin-window-controls-recovery-40.0.0R1.css") || pathOnly(row.raw).endsWith("admin-window-controls-recovery-40.0.0R1.css"));
    ...
    const checks = [
      ...
      check("Window controls recovery stylesheet", recoveryStyle.length === 1 && recoveryStyle[0].raw.includes(BUILD_3990R2), recoveryStyle.length === 1 ? recoveryStyle[0].raw : `${recoveryStyle.length} occurrence(s)`),
      check("Graphique direct window controls", !!graphChrome && graphChrome.complete && graphChrome.visible, graphChrome ? `5/5=${String(graphChrome.complete)} · visible=${String(graphChrome.visible)} · opacity=${graphChrome.opacity}` : "chrome Graphique absent"),
      check("Market Snapshot direct window controls", !!marketChrome && marketChrome.complete && marketChrome.visible, marketChrome ? `5/5=${String(marketChrome.complete)} · visible=${String(marketChrome.visible)} · opacity=${marketChrome.opacity}` : "chrome Market Snapshot absent"),
      ...
      check("Layout repair 40.0.0R1", !!globalThis.atlasAdministratorLayoutRepair3990R2 && hasScriptSuffix("/js/layout-repair-39.9.0R2.js"), "module DOM final + script unique"),
      ...
      check("Freeze singleton", hasScriptSuffix("/js/architecture-freeze-40.0.0R1.js"), "script 40.0.0R1 unique")
    ];
  ...
  function ensureRoot() {
    ...
    root.innerHTML = `
      <div class="atlas-memory-intelligence-head">
        <div>
          <p class="eyebrow">FUNCTIONAL RECOVERY · 40.0.0R1 · READ ONLY</p>
  ...
  function render() {
    ...
    setText("architectureFreezeContract4000", data.pass
      ? "40.0.0R1 : récupération fonctionnelle des commandes de fenêtres validée. Les limites de couverture restent visibles ; aucune n’est transformée en faux défaut structurel."
  ...
  function markdown(data = derive()) {
    const lines = [
      "# Agent-Crypto — Window Controls Functional Recovery 40.0.0R1", "",
  ...
  const freezeContract4000 = Object.freeze({
    build: BUILD_3990R2,
    role: "read-only 40.0 stable candidate preflight over validated 39.9.0R2 architecture",
    feature_delta: false,
  ...
  globalThis.atlasArchitectureFreeze4000R1 = Object.freeze({ derive, render, markdown });
