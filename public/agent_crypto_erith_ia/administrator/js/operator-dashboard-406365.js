/* Agent-Crypto Administrator — 40.6.365
   OPERATOR DASHBOARD · REDIVIDER HOVER SIGNAL
   40.6.364 cockpit is frozen; REDIVIDER gains a restrained deep-crimson hover signal on both NORMAL and MINI controls. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406365)return;

  const BUILD="40.6.365";
  const ROOT_ID="agentCryptoOperatorDashboard406365";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406365";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.365.")};
    }
  ...
  function openStopConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406365");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406365");
...
#${ROOT_ID} .kill-ring:hover,
#${ROOT_ID} .kill-mini-ring:hover{
  filter:brightness(1.12) saturate(1.10);
  box-shadow:0 0 0 1px rgba(194,44,58,.30),0 0 18px rgba(158,16,35,.34),0 0 34px rgba(103,9,24,.12);
}
#${ROOT_ID} .kill-ring:hover .redivider-core,
#${ROOT_ID} .kill-mini-ring:hover .redivider-core{
...
#${ROOT_ID} .kill-ring:hover .redivider-brand,
#${ROOT_ID} .kill-mini-ring:hover .redivider-brand{
  color:#f0ded9;text-shadow:0 0 6px rgba(191,39,54,.26);
}
#${ROOT_ID} .kill-ring:hover .redivider-glyphs,
#${ROOT_ID} .kill-mini-ring:hover .redivider-glyphs{
  color:#b73340;text-shadow:0 0 7px rgba(155,15,32,.38);
}
#${ROOT_ID} .kill-ring:hover .redivider-glyphs b,
#${ROOT_ID} .kill-mini-ring:hover .redivider-glyphs b{
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406365">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406365">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406365">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406365">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406365">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406365">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406365" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406365">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406365">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406365" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406365" aria-describedby="acOperatorRedividerConfirmDesc406365">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406365">REDIVIDER</h2>
    ...
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406365">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406365">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406365" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406365" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406365")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406365")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406365");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406365");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406365");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406365");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406365");
  ...
  globalThis.AgentCryptoOperatorDashboard406365=Object.freeze({
    build:BUILD,
    mount,
    ...
    css_authority:"SINGLE_CLEAN_REWRITE_REDIVIDER_HOVER_SIGNAL"
  });
})();
