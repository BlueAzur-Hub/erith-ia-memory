/* Agent-Crypto @erith.IA — 40.6.124 maintenance: PREBOOT VERSION AUTHORITY.
   Runs from the first parser-blocking Administrator script, before app.js.
   Normal canonical index.html therefore starts as the current published build,
   while immutable/ac-build entries keep their explicit loaded-build authority.
   No timer, observer, storage write, trading or market mutation. */
(() => {
  "use strict";
  const CURRENT_BUILD = "40.6.124";
  ...
  const requested = String(params.get("ac-build") || "").trim();
  const loaded = String(entryMatch?.[1] || (BUILD_RE.test(requested) ? requested : CURRENT_BUILD));

  const setMeta = (name, value) => {
    const node = document.querySelector(`meta[name="${name}"]`);
    if (node) node.content = value;
  };
  setMeta("atlas-build", loaded);
  setMeta("administrator-build", loaded);
  setMeta("atlas-engine-build", ENGINE);
  setMeta("atlas-asset-token", `market-core-v2.0-alpha-build-${loaded}`);
  if (loaded === CURRENT_BUILD) setMeta("administrator-release", "STRATEGY A OWNER API AUTO-START");
  if (/Build\s+\d+\.\d+\.\d+/i.test(document.title)) {
    document.title = document.title.replace(/Build\s+\d+\.\d+\.\d+/i, `Build ${loaded}`);
  }

  document.documentElement.dataset.versionBootAuthority = "preboot-version-authority";
  document.documentElement.dataset.versionBootBuild = loaded;
  globalThis.ErithBuildAuthority = Object.freeze({
    owner: "preboot-version-authority",
    current: loaded,
    canonical_current: CURRENT_BUILD,
    engine: ENGINE,
    immutable_entry: Boolean(entryMatch),
    explicit_build: BUILD_RE.test(requested),
    normal_index_defaults_to_current: !entryMatch && !BUILD_RE.test(requested)
  });

  const syncVisible = () => {
    const text = document.getElementById("atlasVersionTruthText");
    if (text) text.textContent = `Build ${loaded} · Administrator`;
    const footer = document.getElementById("footerRelease");
    if (footer) footer.textContent = String(footer.textContent || "")
      .replace(/Administrator\s+\d+\.\d+\.\d+/gi, `Administrator ${loaded}`)
      .replace(/Build\s+\d+\.\d+\.\d+/gi, `Build ${loaded}`);
    const mirror = document.querySelector(".admin-mirror-brand");
    if (mirror) mirror.innerHTML = `AGENT-CRYPTO <b>${loaded}</b> · ADMINISTRATOR`;
  };
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", syncVisible, { once: true });
  else syncVisible();
})();
