...
(() => {
  ...
  function ensureStyle(){if(typeof document==="undefined"||document.getElementById("strategyADossierStyle404294"))return;const st=document.createElement("style");st.id="strategyADossierStyle404294";st.textContent=`#strategyADossier{margin-top:10px;padding:10px;border:1px solid rgba(132,174,255,.22);border-radius:10px;background:rgba(7,15,31,.48)}#strategyADossier .sad-head{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap}#strategyADossier .sad-title{font-size:9px;font-weight:950;letter-spacing:.09em;color:#bdd2ff;text-transform:uppercase}#strategyADossier .sad-sub{font-size:8px;color:#8598b7;margin-top:3px}#strategyADossier .sad-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:5px;margin-top:8px}#strategyADossier .sad-k{padding:6px;border:1px solid rgba(255,255,255,.06);border-radius:7px}#strategyADossier .sad-k span{font-size:7px;color:#7588a7;display:block;text-transform:uppercase}#strategyADossier .sad-k b{font-size:9px;color:#edf3ff;display:block;margin-top:3px}#strategyADossier .sad-foot{font-size:8px;color:#8190a9;margin-top:7px}@media(max-width:950px){#strategyADossier .sad-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}`;document.head.appendChild(st);}
  function render(){if(typeof document==="undefined")return false;ensureStyle();const anchor=document.getElementById("strategyASafety")||document.getElementById("strategyAAfterCost");if(!anchor)return false;let p=document.getElementById("strategyADossier");if(!p){p=document.createElement("section");p.id="strategyADossier";p.innerHTML=`<div class="sad-head"><div><div class="sad-title">STRATEGY A · PAPER V2 · EVIDENCE DOSSIER</div><div class="sad-sub">Ce dossier distingue ce qui est prouvé, ce qui est seulement testé, et ce qui reste PENDING.</div></div><button class="btn small" id="strategyADossierExport" type="button">EXPORTER DOSSIER</button></div><div class="sad-grid"><div class="sad-k"><span>Fondation déterministe</span><b data-sad="det">—</b></div><div class="sad-k"><span>Cycles réels</span><b data-sad="cycles">0</b></div><div class="sad-k"><span>Trades après coûts</span><b data-sad="trades">0</b></div><div class="sad-k"><span>Gates non soldés</span><b data-sad="pending">—</b></div><div class="sad-k"><span>Micro-live</span><b data-sad="live">LOCKED</b></div></div><div class="sad-foot">Aucun backtest inventé · aucun résultat de rentabilité fabriqué · PAPER ONLY. Le prochain opérateur peut reprendre directement depuis ce dossier.</div>`;anchor.insertAdjacentElement("afterend",p);p.querySelector("#strategyADossierExport")?.addEventListener("click",exportJson);}const d=snapshot();const set=(k,v)=>{const n=p.querySelector(`[data-sad="${k}"]`);if(n)n.textContent=v;};set("det",d.deterministic_foundation.pass?"PASS":"À VÉRIFIER");set("cycles",num(d.experiment_ledger?.summary?.cycles));set("trades",num(d.after_cost?.summary?.trades));set("pending",d.certification.unresolved.length);set("live","LOCKED");return true;}
