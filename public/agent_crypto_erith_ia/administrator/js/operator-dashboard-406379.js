/* Agent-Crypto Administrator — 40.6.379
   OPERATOR DASHBOARD · CANONICAL ATLAS MATH SKIN
   Canonical Atlas Math skin: the floating/operator Math Core now reproduces the visual contract of the real Atlas Math Core beside Market — same conic score ring, same dark #0b1c30 inner disc, same restrained color glow, same score/label tone. Geometry, REDIVIDER and Math computation remain unchanged. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406379)return;

  const BUILD="40.6.379";
  const ROOT_ID="agentCryptoOperatorDashboard406379";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406379";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.379.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406379");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406379");
...
#${ROOT_ID} .math-ring{width:108px;height:108px;z-index:2}
#${ROOT_ID} .ring-core,
...
#${ROOT_ID} .math-ring .value{font-size:27px}
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406379">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406379">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406379">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406379">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406379">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406379">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406379" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406379">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406379">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406379" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406379" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406379" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406379" aria-describedby="acOperatorRedividerConfirmDesc406379">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406379">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406379">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406379">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406379">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406379">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406379" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406379" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406379")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406379")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406379");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406379");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406379");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406379");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406379");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406379");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406379");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406379");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406379");
  ...
  globalThis.AgentCryptoOperatorDashboard406379=Object.freeze({
    build:BUILD,
    mount,
