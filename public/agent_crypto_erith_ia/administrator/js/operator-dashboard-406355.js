/* Agent-Crypto Administrator — 40.6.355
   OPERATOR DASHBOARD · CLEAN REWRITE
   Single DOM/CSS authority. No inherited dashboard override stack. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406355)return;

  const BUILD="40.6.355";
  const ROOT_ID="agentCryptoOperatorDashboard406355";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406355";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.355.")};
    }
...
#${ROOT_ID}{
  ...
  --cyan:#72efff;--cyan-hot:#a5f8ff;--red:#ff526c;--red-hot:#ff93a3;--gold:#ffd978;
  --math-score:0;
}
...
#${ROOT_ID} .kill-wing .panel{
  border-color:rgba(255,82,108,.62);
  box-shadow:0 14px 34px rgba(0,0,0,.28),0 0 28px rgba(255,65,92,.13),inset 0 0 28px rgba(255,65,92,.045);
}
...
#${ROOT_ID} .kill-wing .panel:before{
  ...
  background:linear-gradient(270deg,var(--red),rgba(255,217,120,.50),transparent);
  filter:drop-shadow(0 0 5px rgba(255,82,108,.48));
}
#${ROOT_ID} .head{
...
#${ROOT_ID} .kill-wing .kicker{
  color:var(--red-hot);text-shadow:0 0 5px rgba(255,82,108,.58);
}
...
#${ROOT_ID}[data-kill-state="stopped"] .kill-state strong{
  color:var(--red-hot);text-shadow:0 0 6px rgba(255,82,108,.55);
}
#${ROOT_ID} .paper{
...
#${ROOT_ID} .kill-mini-ring{
  ...
  background:conic-gradient(from -90deg,
    rgba(255,82,108,.99) 0 78%,
    rgba(255,217,120,.98) 78% 82%,
    rgba(255,82,108,.99) 82% 100%);
  box-shadow:0 0 15px rgba(255,82,108,.38),0 0 32px rgba(255,43,76,.22);
...
#${ROOT_ID} .kill-core{
  ...
  background:radial-gradient(circle at 45% 28%,rgba(255,91,115,.33),rgba(92,15,30,.76) 48%,rgba(29,5,11,.94) 78%);
  border:1px solid rgba(255,135,153,.45);
}
#${ROOT_ID} .kill-core .kill-word{color:var(--gold);font-size:9px;font-weight:1000;text-shadow:0 0 4px rgba(255,217,120,.45)}
#${ROOT_ID} .kill-core .switch-word{color:var(--red-hot);font-size:17px;font-weight:1000;text-shadow:0 0 4px rgba(255,135,153,.72)}
...
#${ROOT_ID} .kill-recall{
  right:-8px;color:var(--red-hot);border:1px solid rgba(255,82,108,.66);
}
...
<section class="wing math-wing" aria-label="Math Core synthétique">
  <div class="normal panel">
    ...
    <div class="body">
      <div class="math-row">
        <div class="math-ring"><div class="ring-core"><span class="value" id="acOperatorMathScore406355">—</span></div></div>
        <div class="math-copy">
          <b id="acOperatorMathLabel406355">En attente</b>
          <small id="acOperatorMathContext406355">Contexte Market : en attente</small>
        </div>
      </div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        ...
        <span class="mini-score" id="acOperatorMathMiniScore406355">—</span>
      </span>
    </button>
  </div>
</section>
      ...
      <div class="kill-layout">
        <div>
          <div class="kill-state">
            <strong id="acOperatorKillState406355">PRÊT</strong>
            <span class="paper" id="acOperatorPaperOnly406355">PAPER ONLY</span>
          </div>
          <div class="kill-detail" id="acOperatorKillDetail406355">Auto A Paper · lecture et monitoring préservés.</div>
        </div>
...
`;
  ...
  function render(){
    ...
    for(const id of ["acOperatorMathScore406355","acOperatorMathMiniScore406355"]){
      const node=byId(id);
      if(node)node.textContent=scoreText;
    }
    const label=byId("acOperatorMathLabel406355");
    if(label)label.textContent=math.label;
    const context=byId("acOperatorMathContext406355");
  ...
  globalThis.AgentCryptoOperatorDashboard406355=Object.freeze({
    build:BUILD,
    mount,
