/* Agent-Crypto Administrator — 40.6.361
   OPERATOR DASHBOARD · REDIVIDER WINGS
   Single DOM/CSS authority. Math V3 remains stable; REDIVIDER gains normal-mode cinematic rails around the same deep-crimson safety core. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406361)return;

  const BUILD="40.6.361";
  const ROOT_ID="agentCryptoOperatorDashboard406361";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406361";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.361.")};
    }
...
/* MATH OPEN — canonical Atlas Math Core V3 ring language */
...
#${ROOT_ID} .math-ring{width:88px;height:88px;flex:0 0 88px}
#${ROOT_ID} .ring-core,
...
#${ROOT_ID} .math-ring .value{
  color:#4ee5e8;font-size:22px;line-height:1;font-weight:1000;
  text-shadow:0 0 5px rgba(55,221,226,.28);
}
#${ROOT_ID} .ring-label,
#${ROOT_ID} .mini-math-label{
  max-width:68px;color:#66e8e8;font-size:6.5px;font-weight:900;line-height:1.05;text-align:center;
  text-shadow:0 0 3px rgba(55,221,226,.18);
}
#${ROOT_ID} .math-copy{min-width:0}
#${ROOT_ID} .math-copy b{
  display:block;color:#d9eef0;font-size:9px;letter-spacing:.08em;
}
  ...
  <div class="normal panel">
    ...
    <div class="body">
      <div class="math-row">
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406361">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406361">En attente</small>
          </div>
        </div>
        <div class="math-copy">
          <b>ATLAS · MATH CORE V3</b>
          <small id="acOperatorMathContext406361">Contexte Market : en attente</small>
        </div>
      </div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406361">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406361">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406361">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406361" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406361">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406361">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
`;
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406361");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406361");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406361");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406361");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406361");
  ...
  globalThis.AgentCryptoOperatorDashboard406361=Object.freeze({
    build:BUILD,
    mount,
    ...
    css_authority:"SINGLE_CLEAN_REWRITE_MATH_V3_REDIVIDER_WINGS"
  });
})();
