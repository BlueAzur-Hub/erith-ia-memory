...
(() => {
  ...
  function fromReconciliation(row={}){
    ...
    ROWS.push(out);if(typeof document!=="undefined"&&document.getElementById("strategyAAfterCost"))render();return {ok:true,row:clone(out)};
  }
  ...
  function selfTest(){
    ...
    }finally{ROWS.splice(0);ROWS.push(...saved);if(typeof document!=="undefined"&&document.getElementById("strategyAAfterCost"))render();}
  }
  ...
  function ensureStyle(){if(typeof document==="undefined")return;if(document.getElementById("strategyAAfterCostStyle"))return;const st=document.createElement("style");st.id="strategyAAfterCostStyle";st.textContent=`#strategyAAfterCost{margin-top:10px;padding:10px;border:1px solid rgba(95,220,182,.22);border-radius:10px;background:rgba(3,23,20,.48)}#strategyAAfterCost .sam-head{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:flex-start}#strategyAAfterCost .sam-title{font-size:9px;font-weight:950;letter-spacing:.09em;color:#a8f1dc;text-transform:uppercase}#strategyAAfterCost .sam-sub{font-size:8px;color:#7faaa0;margin-top:3px}#strategyAAfterCost .sam-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:5px;margin-top:8px}#strategyAAfterCost .sam-k{padding:6px;border:1px solid rgba(255,255,255,.06);border-radius:7px}#strategyAAfterCost .sam-k span{display:block;font-size:7px;color:#78958e;text-transform:uppercase;font-weight:900}#strategyAAfterCost .sam-k b{display:block;margin-top:3px;font-size:9px;color:#eefaf6}#strategyAAfterCost .sam-foot{font-size:8px;color:#7e9c95;margin-top:7px}@media(max-width:950px){#strategyAAfterCost .sam-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}`;document.head.appendChild(st);}
  function render(){if(typeof document==="undefined")return false;ensureStyle();const anchor=document.getElementById("strategyAPaperLifecycle")||document.getElementById("strategyAExperimentLedger");if(!anchor)return false;let p=document.getElementById("strategyAAfterCost");if(!p){p=document.createElement("section");p.id="strategyAAfterCost";anchor.insertAdjacentElement("afterend",p);}p.innerHTML=`<div class="sam-head"><div><div class="sam-title">PAPER V2 · ACTUAL AFTER-COST EVIDENCE</div><div class="sam-sub">Réconciliation Paper · identité comptable vérifiée seulement avec coûts complets · modèle partiel = indéterminé.</div></div><button class="btn small" type="button" id="strategyAAfterCostExport">EXPORTER</button></div><div class="sam-grid"><div class="sam-k"><span>Trades</span><b data-sam="trades">0</b></div><div class="sam-k"><span>Coûts complets</span><b data-sam="complete">0</b></div><div class="sam-k"><span>Brut référence</span><b data-sam="gross">0,00 €</b></div><div class="sam-k"><span>Coûts connus</span><b data-sam="costs">0,00 €</b></div><div class="sam-k"><span>Net moteur</span><b data-sam="net">0,00 €</b></div><div class="sam-k"><span>Échantillon</span><b data-sam="sample">INSUFFICIENT_SAMPLE</b></div></div><div class="sam-foot">Ledger : doublon refusé · identité complète contradictoire refusée · UNKNOWN reste non vérifiable tant que le coût manque.</div>`;p.querySelector("#strategyAAfterCostExport")?.addEventListener("click",exportJson);const s=summary(),set=(k,v)=>{const n=p.querySelector(`[data-sam="${k}"]`);if(n)n.textContent=v;};set("trades",s.trades);set("complete",`${s.complete_cost_trades}/${s.trades}`);const eur=v=>typeof v==="number"&&Number.isFinite(v)?`${v.toFixed(2)} €`:"UNKNOWN";set("gross",eur(s.reference_gross_pnl_eur));set("costs",eur(s.known_costs_eur));set("net",eur(s.net_pnl_eur));set("sample",s.sample_state);return true;}
  const api=Object.freeze({build:BUILD,schema:SCHEMA,from_reconciliation:fromReconciliation,record_from_reconciliation:fromReconciliation,read:()=>clone(ROWS)||[],summary,self_test:selfTest,export_json:exportJson,render,sample_min:SAMPLE_MIN,paper_only:true,real_orders:false,network:false,storage_write:false,profitability_claim:false,unknown_cost_is_zero:false,actual_paper_connected:true,ledger_consistency_406016:true,strict_unknown_semantics_406017:true,null_blank_boolean_unknown:true,negative_cost_not_zero:true,known_costs_unknown_when_none_known:true,complete_identity_mismatch_rejected:true,partial_cost_identity_indeterminate:true,accounting_tolerance_eur:ACCOUNTING_TOLERANCE_EUR});
  globalThis.AgentCryptoStrategyAAfterCostMetrics=api;
  // 40.6.405 — calcul automatique, panneau à la demande.
