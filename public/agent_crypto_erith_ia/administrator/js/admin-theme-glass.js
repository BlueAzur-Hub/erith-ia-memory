...
(() => {
  ...
  function apply(theme,{persist=true}={}){const value=ALLOWED.has(theme)?theme:"metal";document.documentElement.dataset.adminTheme=value;if(persist){try{localStorage.setItem(KEY,value);}catch(_){}}const b=document.getElementById("atlasThemeGlassToggle");if(b){const glass=value==="glass";b.setAttribute("aria-pressed",glass?"true":"false");b.textContent=glass?"THÈME · GLASS":"THÈME · METAL";b.title=glass?"Revenir au thème Metal":"Activer le thème Glass transparent";}return value;}
  function toggle(){return apply(read()==="glass"?"metal":"glass");}
  function install(){try{document.documentElement.dataset.glassCompositor=/firefox\//i.test(String(navigator.userAgent||""))?"firefox-safe":"standard";}catch(_){}const b=document.getElementById("atlasThemeGlassToggle");if(b&&!b.dataset.bound){b.dataset.bound="1";b.addEventListener("click",toggle);}apply(read(),{persist:false});}
  const api=Object.freeze({build:BUILD,read,apply,toggle,storage_key:KEY,presentation_only:true,business_logic_changed:false,window_manager_changed:false,market_core_changed:false,network:false,timer:false,observer:false});globalThis.AgentCryptoAdminGlassTheme=api;
