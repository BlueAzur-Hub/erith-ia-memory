/* Agent-Crypto Administrator — 40.6.373
   OPERATOR DASHBOARD · HOVER STATUS BIND FIX
   Hover status bind fix: the reversible REDIVIDER contract is preserved; hover readouts are bound at mount time and work on NORMAL, MINI and recall controls. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406373)return;

  const BUILD="40.6.373";
  const ROOT_ID="agentCryptoOperatorDashboard406373";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406373";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.373.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406373");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406373");
...
#${ROOT_ID}[data-math-mode="normal"] .math-hover-readout{bottom:214px}
#${ROOT_ID}[data-kill-mode="normal"] .kill-hover-readout{bottom:214px}
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406373">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406373">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406373">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406373">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406373">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406373">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406373" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406373">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406373">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406373" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406373" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406373" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406373" aria-describedby="acOperatorRedividerConfirmDesc406373">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406373">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406373">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406373">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406373">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406373">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406373" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406373" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406373")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406373")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406373");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406373");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406373");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406373");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406373");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406373");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406373");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406373");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406373");
  ...
  globalThis.AgentCryptoOperatorDashboard406373=Object.freeze({
    build:BUILD,
    mount,
