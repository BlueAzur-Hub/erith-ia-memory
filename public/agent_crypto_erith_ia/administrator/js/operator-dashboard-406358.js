/* Agent-Crypto Administrator — 40.6.358
   OPERATOR DASHBOARD · COCKPIT FREEZE
   Single DOM/CSS authority. Frozen cockpit baseline from 40.6.357; no visual delta. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406358)return;

  const BUILD="40.6.358";
  const ROOT_ID="agentCryptoOperatorDashboard406358";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406358";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.358.")};
    }
...
`;
    style.textContent += "\n/* 40.6.358 PREMIUM FINISH — micro accents only; geometry frozen */\n#${ROOT_ID} .kill-wing .panel{\n  border-color:rgba(216,91,114,.38);\n  box-shadow:0 14px 34px rgba(0,0,0,.31),0 0 20px rgba(216,91,114,.065),inset 0 0 0 1px rgba(240,205,125,.018),inset 0 0 22px rgba(216,91,114,.020);\n}\n#${ROOT_ID} .kill-state{\n  border-color:rgba(240,205,125,.18);\n  background:linear-gradient(145deg,rgba(20,14,19,.42),rgba(8,8,13,.28));\n}\n#${ROOT_ID} .kill-state strong{color:#f0cf84;text-shadow:0 0 4px rgba(240,205,132,.24)}\n#${ROOT_ID}[data-kill-state=\"stopped\"] .kill-state strong{color:#e996a6;text-shadow:0 0 3px rgba(216,91,114,.30)}\n#${ROOT_ID} .paper{color:#b9ffc3;background:rgba(95,255,128,.055);border-color:rgba(95,255,128,.16)}\n#${ROOT_ID} .kill-detail{color:#aeb8c0}\n#${ROOT_ID} .kill-core{box-shadow:inset 0 0 0 1px rgba(240,205,125,.025),inset 0 0 18px rgba(216,91,114,.045)}\n#${ROOT_ID} .kill-core .kill-word{color:#f0cf84;text-shadow:0 0 3px rgba(240,205,132,.28)}\n#${ROOT_ID} .kill-core .switch-word{color:#eca1ae;text-shadow:0 0 2px rgba(216,91,114,.34)}\n#${ROOT_ID} .kill-ring,#${ROOT_ID} .kill-mini-ring{box-shadow:0 0 10px rgba(216,91,114,.20),0 0 20px rgba(190,48,78,.085)}\n";
...
<section class="wing math-wing" aria-label="Math Core synthétique">
  <div class="normal panel">
    ...
    <div class="body">
      <div class="math-row">
        <div class="math-ring"><div class="ring-core"><span class="value" id="acOperatorMathScore406358">—</span></div></div>
        <div class="math-copy">
          <b id="acOperatorMathLabel406358">En attente</b>
          <small id="acOperatorMathContext406358">Contexte Market : en attente</small>
        </div>
      </div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        ...
        <span class="mini-score" id="acOperatorMathMiniScore406358">—</span>
      </span>
    </button>
  </div>
</section>
      ...
      <div class="kill-layout">
        <div>
          <div class="kill-state">
            <strong id="acOperatorKillState406358">PRÊT</strong>
            <span class="paper" id="acOperatorPaperOnly406358">PAPER ONLY</span>
          </div>
          <div class="kill-detail" id="acOperatorKillDetail406358">Auto A Paper · lecture et monitoring préservés.</div>
        </div>
...
`;
  ...
  function render(){
    ...
    for(const id of ["acOperatorMathScore406358","acOperatorMathMiniScore406358"]){
      const node=byId(id);
      if(node)node.textContent=scoreText;
    }
    const label=byId("acOperatorMathLabel406358");
    if(label)label.textContent=math.label;
    const context=byId("acOperatorMathContext406358");
  ...
  globalThis.AgentCryptoOperatorDashboard406358=Object.freeze({
    build:BUILD,
    mount,
    ...
    cockpit_frozen:true,
    frozen_visual_baseline:"40.6.357",
    css_authority:"SINGLE_CLEAN_REWRITE"
  });
})();
