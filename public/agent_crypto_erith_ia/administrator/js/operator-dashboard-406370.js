/* Agent-Crypto Administrator — 40.6.370
   OPERATOR DASHBOARD · REDIVIDER MODAL EMPHASIS
   REDIVIDER modal emphasis: the validated continuous 360° readiness ring is preserved; the safety copy is centered, crimson and bold without changing business behavior. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406370)return;

  const BUILD="40.6.370";
  const ROOT_ID="agentCryptoOperatorDashboard406370";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406370";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",run:()=>globalThis.strategyAAutoStop("KILL SWITCH · arrêt opérateur depuis le tableau de bord 40.6.370.")};
    }
  ...
  function openStopConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406370");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406370");
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406370">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406370">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406370">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406370">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406370">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406370">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406370" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406370">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406370">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406370" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406370" aria-describedby="acOperatorRedividerConfirmDesc406370">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406370">REDIVIDER</h2>
    ...
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406370">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406370">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406370" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406370" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406370")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406370")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406370");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406370");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406370");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406370");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406370");
  ...
  globalThis.AgentCryptoOperatorDashboard406370=Object.freeze({
    build:BUILD,
    mount,
