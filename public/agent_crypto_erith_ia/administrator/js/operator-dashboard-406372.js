/* Agent-Crypto Administrator — 40.6.372
   OPERATOR DASHBOARD · HOVER STATUS LINE
   Operator hover status: the reversible REDIVIDER contract is preserved and native tooltip help is replaced by compact state readouts above the REDIVIDER and Math Core mini controls. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406372)return;

  const BUILD="40.6.372";
  const ROOT_ID="agentCryptoOperatorDashboard406372";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406372";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.372.")};
    }
  ...
  function stopState(){
    ...
    const stopped=persisted||snap?.enabled===false||String(snap?.phase||"").toUpperCase()==="OFF";
  ...
  function openSwitchConfirm(trigger=null){
    ...
    bindHover(".math-mini-ring,.math-recall","math");
    bindHover(".kill-mini-ring,.kill-recall","kill");

    const overlay=byId("acOperatorRedividerConfirm406372");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406372");
...
#${ROOT_ID}{
  ...
  --math-score:0;--redivider-ready:100;
}
...
#${ROOT_ID} .operator-hover-readout{
  position:absolute;z-index:18;bottom:108px;width:min(390px,calc(100vw - 36px));min-height:28px;
  display:flex;align-items:center;padding:6px 11px;border-radius:10px;
  background:rgba(4,10,15,.91);backdrop-filter:blur(9px) saturate(90%);
  font-size:9px;line-height:1.2;font-weight:900;letter-spacing:.055em;
...
#${ROOT_ID} .math-hover-readout{
  left:16px;color:#8eeff0;border:1px solid rgba(55,221,226,.24);
  box-shadow:0 8px 22px rgba(0,0,0,.32),0 0 12px rgba(55,221,226,.035);
}
#${ROOT_ID} .kill-hover-readout{
  right:16px;justify-content:flex-end;text-align:right;color:#c8868d;border:1px solid rgba(159,29,43,.30);
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406372">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406372">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406372">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406372">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406372">En attente</span>
      </span>
    </button>
  </div>
</section>
...
<section class="wing kill-wing" aria-label="REDIVIDER · Kill Switch opérateur">
  <div class="normal panel">
    ...
    <div class="body redivider-stage">
      ...
      <div class="redivider-status" id="acOperatorRedividerStatus406372">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406372" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406372">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406372">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406372" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406372" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406372" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406372" aria-describedby="acOperatorRedividerConfirmDesc406372">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406372">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406372">Confirmation de coupure</p>
    ...
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406372">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406372">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406372">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406372" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406372" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406372")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406372")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406372");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406372");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406372");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406372");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406372");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406372");
    if(mathHover)mathHover.textContent="MATH CORE · "+scoreText+" · "+math.label+" · Appuyez pour ouvrir";
    ...
    const stateNode=byId("acOperatorKillState406372");
    if(stateNode)stateNode.textContent=status;
    const detail=byId("acOperatorKillDetail406372");
    if(detail)detail.textContent=ks.persisted?"Arrêt opérateur persistant · session actuelle":ks.last_action;
    const paper=byId("acOperatorPaperOnly406372");
    const paperText=ks.paper_only&&!ks.real_orders?"PAPER ONLY":"ÉTAT À VÉRIFIER";
    if(paper)paper.textContent=paperText;
    const redividerStatus=byId("acOperatorRedividerStatus406372");
    if(redividerStatus)redividerStatus.textContent=paperText+" · "+status;
    const redividerHover=byId("acOperatorRedividerHoverStatus406372");
    if(redividerHover)redividerHover.textContent=!ks.available
      ?"REDIVIDER · INDISPONIBLE · Auto A Paper non résident"
      :ks.stopped
        ?"REDIVIDER · COUPÉ · Auto A Paper arrêté · Appuyez pour relancer"
        :"REDIVIDER · 100% PRÊT · CHARGÉ · Appuyez pour stopper";

    const confirmSubtitle=byId("acOperatorRedividerConfirmSubtitle406372");
    if(confirmSubtitle)confirmSubtitle.textContent=action==="start"?"Confirmation de relance":"Confirmation de coupure";
    const confirmQuestion=byId("acOperatorRedividerConfirmQuestion406372");
    if(confirmQuestion)confirmQuestion.textContent=action==="start"?"Relancer Auto A Paper ?":"Couper Auto A Paper ?";
    const confirmSafety=byId("acOperatorRedividerConfirmDesc406372");
    ...
    const confirmState=byId("acOperatorRedividerConfirmState406372");
    if(confirmState)confirmState.textContent=paperText+" · "+status;
    const confirmAccept=byId("acOperatorRedividerAccept406372");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406372");
  ...
  globalThis.AgentCryptoOperatorDashboard406372=Object.freeze({
    build:BUILD,
    mount,
    ...
    hover_status_targets:"MATH_MINI_RECALL + REDIVIDER_MINI_RECALL",
