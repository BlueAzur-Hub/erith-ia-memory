/* Agent-Crypto @erith.IA — 40.6.109
   Local AI Reserve Truth.
   Refines 40.6.106 so a correct limitation is never rejected merely because
   another field in the same contract is present. Price evidence and volume
   evidence are independent; Math completeness requires the four core risk
   measures. Read-side only: no timer, observer, network, storage or trading. */
(() => {
  "use strict";
  const PATCH = "40.6.109";
  const previous = globalThis.atlasLocalBoundedCommentTruthGuard;
  if (typeof previous !== "function") {
    globalThis.AgentCryptoLocalAIReserveTruth406109 = Object.freeze({patch:PATCH,active:false,reason:"prior_guard_missing"});
    return;
  }
  ...
  function evidence(snapshot){
    ...
    const priceCount=rows.filter(row=>row?.available===true&&finite(row?.price_eur??row?.price??row?.eur)).length;
    const hasPrices=priceCount>0;
    const hasVolume=finite(market.total_volume_24h_eur??market.total_volume_24h??market.volume_24h_eur??market.volume_24h)
      || finite(contract.math?.volume_market_cap_ratio_pct);
    const math=contract.math||{}, risk=math.historical_risk||{};
    const values=[
    ...
    return Object.freeze({hasPrices,hasVolume,mathCount,mathComplete:mathCount===4&&!degraded,degraded});
  }
  function refinedGuard(mode,body,snapshot){
    ...
    const e=evidence(snapshot), text=normalize(source);
    if(reason==="market_data_truth_conflict_406106"){
      ...
      return conflict
        ? {conflict:true,reason:"market_field_truth_conflict_406109",body:source}
        : {conflict:false,reason:"reserve_supported_406109",body:source};
    }
    if(reason==="math_completeness_truth_conflict_406106"){
      return e.mathComplete
        ? {conflict:true,reason:"math_4of4_truth_conflict_406109",body:source}
        : {conflict:false,reason:"math_reserve_supported_406109",body:source};
    }
    return prior;
  }
  function selfTest(){
    const mk=(prices,volume,mathCount)=>({strict_contract:{canonical_top5:{assets:Array.from({length:prices?5:0},(_,i)=>({available:true,price_eur:100+i}))},market:volume?{total_volume_24h_eur:123}: {},math:{historical_risk:{realized_volatility_pct:mathCount>0?1:null,max_drawdown_pct:mathCount>1?2:null,var_95_pct:mathCount>2?3:null,expected_shortfall_95_pct:mathCount>3?4:null}}}});
    const checks=[];
    const e1=evidence(mk(true,false,4)); checks.push(e1.hasPrices&&!e1.hasVolume);
    const e2=evidence(mk(true,true,4)); checks.push(e2.hasPrices&&e2.hasVolume);
  ...
  globalThis.AgentCryptoLocalAIReserveTruth406109=Object.freeze({patch:PATCH,active:true,owner:"atlasLocalBoundedCommentTruthGuard",price_volume_evidence_separated:true,math_completeness_requires_4_of_4:true,selfTest,recurring_timer:false,observer:false,network_owner:false,storage_write:false,trading:false,wallet:false});
})();
