/* Agent-Crypto Administrator — 40.6.382
   OPERATOR DASHBOARD · HUD TEXT FRAME
   Presentation-only HUD text framing for Math and REDIVIDER. Existing Math computation, REDIVIDER 360° ring, STOP/RESUME logic and protected owners remain unchanged. */
(()=>{
  "use strict";
  if(globalThis.AgentCryptoOperatorDashboard406382)return;

  const BUILD="40.6.382";
  const ROOT_ID="agentCryptoOperatorDashboard406382";
  const STYLE_ID="agentCryptoOperatorDashboardStyle406382";
  ...
  function stopOwner(){
    if(typeof globalThis.strategyAAutoStop==="function"){
      return {kind:"function",id:"strategyAAutoStop",run:()=>globalThis.strategyAAutoStop("REDIVIDER · arrêt opérateur depuis le tableau de bord 40.6.382.")};
    }
  ...
  function openSwitchConfirm(trigger=null){
    ...
    const overlay=byId("acOperatorRedividerConfirm406382");
    ...
    requestAnimationFrame(()=>{
      const cancel=byId("acOperatorRedividerCancel406382");
...
/* 40.6.382 — QUIET CHROME + TACTICAL REDIVIDER
...
#${ROOT_ID} .redivider-status{font-weight:1000}
#${ROOT_ID} .operator-hover-readout{font-weight:1000}

/* 40.6.382 — HUD TEXT FRAME
  ...
  <div class="normal panel">
    ...
    <div class="body math-stage">
      <div class="math-hud">
        ...
        <div class="math-ring">
          <div class="ring-core">
            <span class="value" id="acOperatorMathScore406382">—</span>
            <small class="ring-label" id="acOperatorMathRingLabel406382">En attente</small>
          </div>
        </div>
        <span class="math-balance-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="math-status" id="acOperatorMathContext406382">Contexte Market : en attente</div>
    </div>
  </div>
  <div class="mini">
    <button class="math-mini-ring" type="button" data-wing="math" data-mode="normal" aria-label="Ouvrir Math Core">
      <span class="mini-math-core">
        <span class="mini-score" id="acOperatorMathMiniScore406382">—</span>
        <span class="mini-math-label" id="acOperatorMathMiniLabel406382">En attente</span>
      </span>
    </button>
  </div>
</section>
    ...
    <div class="body redivider-stage">
      <div class="redivider-hud">
        ...
        <span class="redivider-center-stack">
          <span class="redivider-orbit redivider-orbit-top" id="acOperatorRedividerOrbitTop406382" aria-hidden="true">- : PRÊT : -</span>
          ...
          <span class="redivider-orbit redivider-orbit-bottom" id="acOperatorRedividerOrbitBottom406382" aria-hidden="true">— PAPER ONLY · ACTIF —</span>
        </span>
        <span class="redivider-rail right" aria-hidden="true"><i class="rail-mark"></i></span>
      </div>
      <div class="redivider-status" id="acOperatorRedividerStatus406382">PAPER ONLY · PRÊT</div>
      <span class="sr-only" id="acOperatorKillState406382" aria-live="polite">PRÊT</span>
      <span class="sr-only" id="acOperatorPaperOnly406382">PAPER ONLY</span>
      <span class="sr-only" id="acOperatorKillDetail406382">Auto A Paper · lecture et monitoring préservés.</span>
    </div>
  </div>
  <div class="mini">
...
<div class="operator-hover-readout math-hover-readout" id="acOperatorMathHoverStatus406382" role="status" aria-live="polite"></div>
<div class="operator-hover-readout kill-hover-readout" id="acOperatorRedividerHoverStatus406382" role="status" aria-live="polite"></div>

<div class="redivider-confirm-overlay" id="acOperatorRedividerConfirm406382" data-confirm-overlay>
  <section class="redivider-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="acOperatorRedividerConfirmTitle406382" aria-describedby="acOperatorRedividerConfirmDesc406382">
    <div class="confirm-eyebrow">KILL SWITCH OPÉRATEUR</div>
    <h2 class="confirm-title" id="acOperatorRedividerConfirmTitle406382">REDIVIDER</h2>
    <p class="confirm-subtitle" id="acOperatorRedividerConfirmSubtitle406382">Confirmation de coupure</p>
    <div class="confirm-hud" aria-hidden="true">
      <span class="confirm-rail left"></span>
      <span class="confirm-ring"><span class="confirm-core"><strong>REDIVIDER</strong><span class="confirm-percent" id="acOperatorRedividerConfirmPercent406382">[ 100% ]</span><span class="confirm-glyphs"><span>◈</span><b>◉</b><span>◈</span></span></span></span>
      <span class="confirm-rail right"></span>
    </div>
    <p class="confirm-question" id="acOperatorRedividerConfirmQuestion406382">Couper Auto A Paper ?</p>
    <div class="confirm-safety" id="acOperatorRedividerConfirmDesc406382">
    ...
    <div class="confirm-state" id="acOperatorRedividerConfirmState406382">PAPER ONLY · PRÊT</div>
    <div class="confirm-actions">
      <button class="confirm-action cancel" id="acOperatorRedividerCancel406382" type="button" data-confirm-cancel>ANNULER</button>
      <button class="confirm-action confirm" id="acOperatorRedividerAccept406382" type="button" data-confirm-accept>CONFIRMER</button>
    </div>
  </section>
</div>
`;
    ...
    byId("acOperatorRedividerCancel406382")?.addEventListener("click",event=>{
      event.preventDefault();event.stopPropagation();closeStopConfirm("CANCEL");
    });
    byId("acOperatorRedividerAccept406382")?.addEventListener("click",event=>{
  ...
  function render(){
    ...
    const openScore=byId("acOperatorMathScore406382");
    if(openScore)openScore.textContent=scoreNumber;
    const miniScore=byId("acOperatorMathMiniScore406382");
    if(miniScore)miniScore.textContent=scoreNumber;
    const ringLabel=byId("acOperatorMathRingLabel406382");
    if(ringLabel)ringLabel.textContent=math.label;
    const miniLabel=byId("acOperatorMathMiniLabel406382");
    if(miniLabel)miniLabel.textContent=math.label;
    const context=byId("acOperatorMathContext406382");
    if(context)context.textContent=math.context;
    const mathHover=byId("acOperatorMathHoverStatus406382");
    ...
    const orbitTop=byId("acOperatorRedividerOrbitTop406382");
    const orbitBottom=byId("acOperatorRedividerOrbitBottom406382");
    ...
    const confirmPercent=byId("acOperatorRedividerConfirmPercent406382");
    if(confirmPercent)confirmPercent.textContent=action==="start"?"[ COUPÉ ]":"[ 100% ]";
    const confirmState=byId("acOperatorRedividerConfirmState406382");
    if(confirmState)confirmState.textContent=action==="start"
      ?"[ COUPÉ ]\n- : ARRÊT : -\n— AUTO A PAPER —"
      :"[ 100% ]\n- : PRÊT : -\n— CHARGÉ —";
    const confirmAccept=byId("acOperatorRedividerAccept406382");
    if(confirmAccept)confirmAccept.disabled=state.action_pending||!actionAvailable;
    const confirmCancel=byId("acOperatorRedividerCancel406382");
  ...
  globalThis.AgentCryptoOperatorDashboard406382=Object.freeze({
    build:BUILD,
    mount,
