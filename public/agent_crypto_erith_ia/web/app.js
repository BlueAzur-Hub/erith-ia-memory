/* V2.0-alpha · Build 28.1.86R2 — CLEAN HOME · INLINE DATA STATUS · GRAPH THREE-STATE · TOP5 FLOW PERSISTENCE · ADMIN GRAPH TOGGLE · MARKET RECENTER · FORGE PRO BRIDGE
...
*/
const ATLAS_RELEASE = "V2.0-alpha · Build 28.1.86R2";
...
/* =========================================================
   Build 28.1.86R2 — Scanner history transport fallback
...
els.btnChartGainers?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("gainers", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.1.86R2" });
});
els.btnChartLosers?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("losers", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.1.86R2" });
});
els.btnChartVolume5?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("volume", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.1.86R2" });
});
    ...
    if (!configured) {
      ...
      const next = {
        ...
        profiles: {
          ...
          operator_yohan: { id: "yohan", role: "operator", enabled: false }
        }
      };
...
function atlasAccessLock() {
  ...
  atlasV2ApplyMode("essential", { persist: false, silentAuth: true });
  try { history.replaceState(null, "", "#analyste"); } catch {}
  document.getElementById("analyste")?.scrollIntoView({ behavior: "smooth", block: "start" });
}

/* =========================================================
   Build 28.1.86R2 — Bridge Auto Health Quiet Status
...
function atlasV2ApplyMode(mode, options = {}) {
  ...
  document.documentElement.dataset.atlasRole = administrator ? "administrator" : "public";
  document.body.dataset.atlasRole = administrator ? "administrator" : "public";

  if (title) title.textContent = administrator ? "Privé Christophe" : "Marché public";
  ...
  if (accountToggle) {
    ...
    accountToggle.setAttribute("aria-label", administrator ? "Verrouiller l’administration locale" : "Ouvrir l’accès privé local");
  }
  if (accountText) accountText.textContent = administrator ? "Christophe · Verrouiller" : "Accès privé";
  ...
  window.dispatchEvent(new CustomEvent("atlas:v2mode", { detail: { mode: next, role: administrator ? "owner" : "public" } }));
  return administrator || next === "essential";
}
...
function atlasV2OpenAdvancedForTarget(hash, options = {}) {
  ...
  if (entry && !["essential", "adaptive"].includes(entry.level) && !atlasAccessIsAuthorized()) {
    atlasAccessOpen(`#${id}`);
    return false;
  }
...
function atlasInitV2Shell() {
  ...
  document.getElementById("btnAdminAccountToggle")?.addEventListener("click", () => {
    if (atlasAccessIsAuthorized() && atlasV2Mode() === "advanced") atlasAccessLock();
    else atlasAccessOpen();
  });
  ...
  atlasV2ApplyMode(atlasV2Mode(), { persist: false, silentAuth: true });
...
} /* Atlas-10 Crypto — Math Core V3 · Build 28.1.86R2
...
/* =========================================================
   Build 28.1.86R2 — Hausses 5 Transaction Rebuild
...
/* =========================================================
   Build 28.1.86R2 — Hausses 5 Rebuild
...
function atlasScannerCommit(tx, finalEntries, rejected) {
  ...
  try {
    ...
    */
    ...
    console.warn("Transaction scanner 28.1.86R2 annulée :", error);
    atlasScannerTransaction = null;
    atlasScannerSetTransactionFlag(false);
...
async function atlasScannerRun(tx) {
  ...
  try {
    ...
    console.error("Hausses 5 / scanner 28.1.86R2 :", error);
...
function atlasScannerStart(preset = "gainers", limit = 5, options = {}) {
  ...
  try {
    ...
    void atlasScannerRun(tx).catch(error => {
      if (error?.name === "AbortError") return;
      console.error("Scanner 28.1.86R2 non capturé :", error);
      if (atlasScannerTransaction === tx) atlasScannerTransaction = null;
    ...
    console.error("Démarrage Hausses 5 28.1.86R2 :", error);
    return atlasScannerVisibleFailure(
      preset,
...
/* =========================================================
   Agent-Crypto @erith.IA — Build 28.1.86R2
   MATH CORE V3 · DOUBLE PRÉSENCE PUBLIC + ADMINISTRATION
   Le panneau public reste canonique. Le Command Center reçoit
   un miroir en lecture seule, alimenté par le même état calculé.
   ========================================================= */
function atlasSyncAdminMathMirror() {
  const mirror = document.getElementById("atlasAdminMathMirror");
  if (!mirror) return;

  const copyText = (sourceId, selector, fallback = "—") => {
    const source = document.getElementById(sourceId);
    const target = mirror.querySelector(selector);
    if (target) target.textContent = source?.textContent?.trim() || fallback;
  };
  const copyHtml = (sourceId, selector, fallback = "") => {
    const source = document.getElementById(sourceId);
    const target = mirror.querySelector(selector);
    if (target) target.innerHTML = source?.innerHTML || fallback;
  };

  copyText("scoreValue", "[data-admin-math-score]");
  copyText("scoreLabel", "[data-admin-math-label]", "En attente");
  copyHtml("atlasHumanVerdict", "[data-admin-math-human]", "En attente du Livecheck.");
  copyHtml("atlasSummaryGrid", "[data-admin-math-summary]");
  copyHtml("atlasMathCorePanel", "[data-admin-math-metrics]");
  copyHtml("atlasMathVerdict", "[data-admin-math-status]", "Math Core V3 en attente");

  const verdict = document.getElementById("atlasMathVerdict");
  const status = mirror.querySelector("[data-admin-math-status]");
  if (status && verdict) status.className = `atlas-admin-math-mirror-status ${verdict.className || ""}`;
}

function atlasInitAdminMathMirror() {
  const sources = [
    "scoreValue", "scoreLabel", "atlasHumanVerdict",
    "atlasSummaryGrid", "atlasMathCorePanel", "atlasMathVerdict"
  ].map(id => document.getElementById(id)).filter(Boolean);

  const observer = new MutationObserver(atlasSyncAdminMathMirror);
  ...
  window.addEventListener("atlas:admin-center", event => {
    if (event?.detail?.open && event?.detail?.target === "analysis") {
      window.requestAnimationFrame(atlasSyncAdminMathMirror);
    }
  });
  atlasSyncAdminMathMirror();
}

atlasInitAdminMathMirror();
