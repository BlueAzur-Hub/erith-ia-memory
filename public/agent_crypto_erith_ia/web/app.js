/* V2.0-alpha · Build 28.1.97 — ANALYTICAL TRUTH & EVIDENCE LOCK · CLEAN HOME · INLINE DATA STATUS · GRAPH THREE-STATE · TOP5 FLOW PERSISTENCE · ADMIN GRAPH TOGGLE · MARKET RECENTER · FORGE PRO BRIDGE
...
*/
const ATLAS_RELEASE = "V2.0-alpha · Build 28.1.97";
...
/* =========================================================
   Build 28.1.97 — Stable Stack Consolidation + Permanent Inline Atlas/Aerith Synthesis
...
/* =========================================================
   Build 28.1.97 — Analytical Truth & Evidence Lock
...
function atlasAnalyticalTruthInit() {
  ...
  atlasAnalyticalTruthRender();
  window.addEventListener("online", () => atlasAnalyticalTruthRender());
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") atlasAnalyticalTruthRender();
  });
  window.setTimeout(() => {
    if (Array.isArray(state.coins) && state.coins.length) {
      try { atlasBuildCryptoPageSnapshot(); } catch { atlasAnalyticalTruthRender(); }
    } else atlasAnalyticalTruthRender();
  }, 2200);
  atlasAnalyticalTruthState.timer = window.setInterval(() => {
    if (document.visibilityState !== "visible") return;
    if (atlasLocalReportsState.running || atlasLocalConclusionState.running) return;
    if (Array.isArray(state.coins) && state.coins.length) {
      try { atlasBuildCryptoPageSnapshot(); } catch { atlasAnalyticalTruthRender(); }
    } else atlasAnalyticalTruthRender();
  }, 15000);
