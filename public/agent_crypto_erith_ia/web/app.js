...
}; const MARKET_CACHE_KEY = "agent_crypto_erith_ia_market_cache_v1_1_alpha_26_36_atomic_250";
...
function atlasRenderMarketAccessNotice() {
  ...
  els.offlineNotice.style.display = "block";
  els.offlineNotice.innerHTML = `<strong>MODE ARCHIVE COINGECKO</strong><p>Dernière lecture directe conservée depuis ${origin} · ${escapeHtml(atlasBrokerAgeLabel(market.timestamp))}. Prix consultables ; scores et simulation suspendus jusqu’au retour des deux flux EUR + USD.</p>`;
}
...
const SourceAdapter = {
  async coingeckoAtomicMarket() {
    const common = "order=market_cap_desc&per_page=250&page=1&locale=fr&precision=full";
    const eurUrl = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&${common}&sparkline=true&price_change_percentage=1h,24h,7d,30d`;
    const usdUrl = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&${common}&sparkline=false`;
    const [eurRows, usdRows] = await Promise.all([
      fetchWithTimeout(eurUrl, {}, 12000),
      fetchWithTimeout(usdUrl, {}, 12000)
    ]);
    if (!Array.isArray(eurRows) || eurRows.length < 200) throw new Error(`Flux EUR incomplet : ${Array.isArray(eurRows) ? eurRows.length : 0}/250`);
    if (!Array.isArray(usdRows) || usdRows.length < 200) throw new Error(`Flux USD incomplet : ${Array.isArray(usdRows) ? usdRows.length : 0}/250`);

    const usdById = new Map(usdRows.filter(row => row?.id).map(row => [row.id, row]));
    const completedAt = new Date().toISOString();
    const markets = eurRows.map(coin => {
      const usd = usdById.get(coin.id);
      const priceEur = Number(coin.current_price);
      const priceUsd = Number(usd?.current_price);
      if (!usd || !Number.isFinite(priceEur) || priceEur <= 0 || !Number.isFinite(priceUsd) || priceUsd <= 0) return null;
      const updatedCandidates = [coin.last_updated, usd.last_updated].map(value => Date.parse(value || "")).filter(Number.isFinite);
      const updatedAt = updatedCandidates.length ? new Date(Math.max(...updatedCandidates)).toISOString() : completedAt;
      return {
        id: coin.id,
        ...
        volume24hUsd: Number.isFinite(Number(usd.total_volume)) ? Number(usd.total_volume) : null,
        sparkline7d: Array.isArray(coin.sparkline_in_7d?.price) ? coin.sparkline_in_7d.price.filter(value => Number.isFinite(Number(value)) && Number(value) > 0).map(Number) : [],
    ...
    if (markets.length < 200) throw new Error(`Fusion EUR/USD incomplète : ${markets.length}/250`);
    ...
    return {
      ...
      snapshotId: `coingecko-top250_${completedAt}`,
      legs: [
        { key: "coingecko-eur", name: "CoinGecko EUR", kind: "Top 250 groupé", status: "OK", detail: `${eurRows.length} actifs` },
        { key: "coingecko-usd", name: "CoinGecko USD", kind: "Top 250 groupé", status: "OK", detail: `${usdRows.length} actifs` }
      ]
    };
  }
};
const liveSources = [
  { key: "coingecko-eur", name: "CoinGecko EUR", kind: "Top 250 groupé" },
  { key: "coingecko-usd", name: "CoinGecko USD", kind: "Top 250 groupé" }
...
function atlasAnalysisLiveReady() {
  return !!state.liveOk && state.sourceLock?.valid === true && state.sourceLock?.mode === "direct" && atlasCanonicalSnapshot(state.coins) && state.coins.length >= 200 && atlasMarketAgeMs() <= ATLAS_ANALYSIS_MAX_AGE_MS;
}
function saveMarketCache() {
  ...
  try {
    localStorage.setItem(MARKET_CACHE_KEY, JSON.stringify({
      schema: "atlas_market_cache_atomic_250_v1",
      version: "V1.1-alpha.26.36",
...
async function runLivecheck() {
  ...
  const previousDirect = state.sourceLock?.mode === "direct" && atlasCanonicalSnapshot(state.coins)
    ...
    : null;
  try {
    setLiveStatus("warn", "Chargement Top 250");
    setTableDecision("Snapshot atomique EUR + USD en cours", "warn");
    setText(els.sourceName, "CoinGecko · deux flux groupés");
    state.sourceStatus = [];
    renderSourceGrid(); updateSourceMetric(0);

    const result = await SourceAdapter.coingeckoAtomicMarket();
    state.sourceStatus = result.legs.map(leg => ({ ...leg, ms: null }));
    const loaded = atlasApplyCanonicalSnapshot({
      coins: result.markets,
      global: result.global,
      timestamp: result.generatedAt,
      snapshotId: result.snapshotId,
      sourceLabel: "CoinGecko direct · Top 250 EUR + USD"
    }, "direct");
    if (!loaded) throw new Error("Snapshot atomique refusé");

    setLiveStatus("ok", "Livecheck Top 250 OK");
    setText(els.sourceName, state.mainSource);
    setText(els.sourceTime, new Date(state.timestamp).toLocaleString("fr-FR"));
    setTableDecision("250 actifs directs · Top 50 affiché", "ok");
    renderSourceGrid(); updateSourceMetric(2);
    atlasRenderMarketAccessNotice();
    renderAll();
    ...
    state.sourceStatus = [
      { key: "coingecko-eur", name: "CoinGecko EUR", kind: "Top 250 groupé", status: "ÉCHEC", detail: cleanError(error) },
      { key: "coingecko-usd", name: "CoinGecko USD", kind: "Top 250 groupé", status: "ÉCHEC", detail: cleanError(error) }
    ];
    if (previousDirect) {
      state.coins = previousDirect.coins; state.global = previousDirect.global; state.mainSource = "CoinGecko · dernière lecture directe"; state.timestamp = previousDirect.timestamp; state.liveOk = true;
      atlasSetSourceLock("local-cache", state.timestamp, "Actualisation directe échouée ; dernière lecture directe conservée en archive mémoire.", true, previousDirect.sourceLock?.snapshotId || null);
      ...
      clearMarketDisplay("Aucun snapshot CoinGecko direct disponible");
      setTableDecision("Marché indisponible", "fail");
    }
    renderSourceGrid(); updateSourceMetric(2); atlasRenderMarketAccessNotice(); renderTrustLock(false);
  } finally {
...
async function refreshMarketOnly() {
  ...
  try {
    ...
    if (!refreshed) throw new Error("Snapshot atomique refusé");
    setLiveStatus("ok", "Marché Top 250 actualisé");
    setText(els.sourceName, state.mainSource);
    setText(els.sourceTime, new Date(state.timestamp).toLocaleString("fr-FR"));
    setTableDecision("250 actifs directs · Top 50 affiché", "ok");
    atlasRenderMarketAccessNotice();
    ...
    if (previous && atlasCanonicalSnapshot(previous.coins)) {
      state.coins = previous.coins; state.global = previous.global; state.timestamp = previous.timestamp; state.mainSource = "CoinGecko · dernière lecture directe"; state.liveOk = true;
      atlasSetSourceLock("local-cache", state.timestamp, "Actualisation directe échouée ; analyses suspendues.", true, previous.snapshotId || null);
      atlasBrokerCommitMarket({ coins: state.coins, global: state.global, timestamp: state.timestamp, snapshotId: previous.snapshotId }, "local-cache");
      setLiveStatus("warn", "Mode archive");
      setTableDecision("Archive CoinGecko · analyses suspendues", "warn");
      renderAll();
    } else if (!applyMarketCache()) {
    ...
    state.auto.livecheckBusy = false;
    renderSourceGrid(); updateSourceMetric(state.sourceStatus.length || 0); atlasRenderMarketAccessNotice();
    if (refreshed) atlasAfterLivecheck(); else { renderAutoReader(); scheduleNextAutoRun(ATLAS_MARKET_REFRESH_MS); }
  }
}
...
const ATLAS_CHART_LOCAL_CACHE_KEY = "agent_crypto_erith_ia_real_charts_v1_1_alpha_26_36_direct_only";
...
} function situationPayload() { return { version: "V1.1-alpha.26.36", active_now: [ "public_market_observation", "charts", "source_diagnostic", "human_readable_tests", "local_paper_trading", "solo_beginner_profile_100_eur", "briefing_questions" ], prepared_only: [ "private_backend", "remote_access", "kraken_read_only", "physical_security_layer" ], locked: [ "real_wallet_connection", "private_key", "withdraw_key", "real_order", "automatic_trading" ], current_step: "collect_information_before_private_backend" };
...
} function backendBlueprintPayload() { return { version: "V1.1-alpha.26.36", principle: "separate_public_frontend_from_private_backend", public_layer: { host: "GitHub Pages", allowed: [ "market_observation", "charts", "watchlist", "command_layer_observation", "local_paper_trading" ], forbidden: [ "private_api_keys", "withdraw_keys", "real_orders", "admin_remote_access", "wallet_connection" ] }, private_layer_future: { host: "private_machine_or_secure_local_server", allowed: [ "encrypted_api_secrets", "Kraken_read_only_client", "server_side_paper_trading", "logs", "kill_switch", "restricted_remote_access" ], access: ["authorized_operator_1", "authorized_operator_2"] }, exchange_layer_future: { primary: "Kraken", first_mode: "read_only", later_modes_locked: [ "paper_trading_server", "human_validated_order", "real_micro_transaction" ], never_allowed_initially: [ "withdraw_permission", "fully_autonomous_trading" ] } };
...
/* =========================================================
   V1.1-alpha.26.36 — Atlas Decision Board
...
function decisionFromScore(score) { if (score === null || score === undefined) return "Analyse suspendue"; if (score <= 40) return "Données fragiles"; if (score <= 55) return "Lecture prudente"; if (score <= 65) return "Mouvement modéré"; if (score <= 75) return "Mouvement marqué"; return "Volatilité élevée";
...
} function renderRiskGrid() { if (!els.riskGrid) return; els.riskGrid.innerHTML = ` <div class="risk ${state.liveOk ? "ok" : "wait"}"><span>Marché</span><b>${state.sourceLock?.valid ? "Source Lock CoinGecko" : "Source canonique absente"}</b></div> <div class="risk warn"><span>Sécurité</span><b>Non vérifiée V1.1-alpha.26.36</b></div> <div class="risk warn"><span>Social</span><b>Non vérifié</b></div> <div class="risk warn"><span>On-chain</span><b>Non vérifié</b></div>`;
  ...
  return [
    ...
    `État : ${tone.label}.`,
    `Snapshot atomique : CoinGecko Top 250 EUR + USD · ${new Date(state.timestamp).toLocaleString("fr-FR")}.`,
...
function renderTrustLock(live = false) {
  ...
  if (locked) {
    ...
    els.trustLockText.textContent = "Snapshot live atomique absent : prix archivés consultables, scores Atlas et simulation suspendus. Deux flux CoinGecko EUR + USD sont requis.";
  }
}
...
} function simulationDataSnapshot() { if (!state.sim) loadSimulation(); const totals = getSimulationTotals(); return { generated_at: new Date().toISOString(), version: "V1.1-alpha.26.36", public_only: true, warning: "Données publiques et simulation locale uniquement. Aucun compte réel, aucune clé API, aucun wallet.", profile: getSimulationProfileStatus(), simulation: simulationPayload(), totals: { cash_eur: state.sim.cash, positions_value_eur: totals.positionsValue, total_value_eur: totals.total, pnl_eur: totals.pnl }, market_snapshot: publicMarketSnapshot() };
...
} function makeCollectorRecord() { const snapshot = simulationDataSnapshot(); const logs = snapshot?.simulation?.logs || []; const marketAssets = snapshot?.market_snapshot?.assets || []; return { id: `snapshot_${Date.now()}`, saved_at: new Date().toISOString(), version: "V1.1-alpha.26.36", public_only: true, source: snapshot?.market_snapshot?.source || "source live", live_ok: !!snapshot?.market_snapshot?.live_ok, symbols: marketAssets.map(a => a.symbol), learning_tags: collectorRecordReasonFromLogs(logs), snapshot };
...
} function downloadCollectorJSON() { const records = readCollectorMemory(); const payload = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.36", public_only: true, warning: "Export mémoire locale public-compatible. Aucun compte réel, aucune clé API, aucun wallet.", count: records.length, records }; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.json`, "application/json", JSON.stringify(payload, null, 2) ); showCollectorMemory();
} function downloadCollectorJSONL() { const records = readCollectorMemory(); const header = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.36", public_only: true, type: "agent_crypto_collector_memory_jsonl_header" }; const lines = [JSON.stringify(header), ...records.map(r => JSON.stringify(r))]; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.jsonl`, "application/x-ndjson", lines.join("\n") ); showCollectorMemory();
...
} function buildMemoryReportMarkdown() { const records = readCollectorMemory(); const lines = [ "# RAPPORT MÉMOIRE LOCALE — Agent-Crypto @erith.IA", "", "Version : V1.1-alpha.26.36", `Date : ${new Date().toISOString()}`, "", "## Statut sécurité", "", "- Mémoire locale navigateur uniquement.", "- Données public-compatible.", "- Aucun compte réel.", "- Aucune clé API.", "- Aucun wallet.", "- Aucun ordre réel.", "", "## Lecture mémoire", "", exploreMemoryText(records), "", "## Comparaison premier / dernier", "", compareMemoryText(records), "", "## Refus de sécurité", "", refusalSummaryText(records), "", "## Conclusion", "", "L’explorateur transforme les snapshots en lecture pédagogique. La prochaine vraie étape sera de déplacer ce principe vers une base locale plus solide sur PC Ryzen 7." ]; return lines.join("\n");
...
} /* ========================================================= V1.1-alpha.26.36 — Atlas Auto Reader Ouverture page -> Livecheck auto -> snapshots -> lecture marché. ========================================================= */ const AUTO_MEMORY_KEY = "agent_crypto_erith_ia_auto_reader_v1_1_alpha_13";
...
} function makeAutoSnapshot() { const collectorId = getCollectorId(); const wanted = new Set(state.watchIds || []); const watch = state.coins.filter(c => wanted.has(c.id)); const leaders = state.coins.slice(0, 20); const merged = [...leaders, ...watch].filter((c, i, arr) => arr.findIndex(x => x.id === c.id) === i); const created = new Date().toISOString(); return { id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, snapshot_id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, collector_id: collectorId, collector_type: "local_browser", saved_at: created, version: "V1.1-alpha.26.36", source: state.mainSource || null, source_time: state.timestamp || null, live_ok: !!state.liveOk, cadence_ms: state.auto?.intervalMs || 60000, global: { market_cap_eur: state.global?.total_market_cap?.eur ?? null, volume_24h_eur: state.global?.total_volume?.eur ?? null, btc_dominance_pct: state.global?.market_cap_percentage?.btc ?? null }, assets: merged.map(compactCoinForAuto) };
...
} /* Atlas-10 Crypto — Math Core intégré V1.1-alpha.26.36 Source: modules .md Atlas Math. Exécution: traduction JS condensée. Lecture seule : aucun ordre réel, aucune clé API, aucun capital engagé. */
