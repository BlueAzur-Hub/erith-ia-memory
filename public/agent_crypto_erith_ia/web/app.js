  ...
  for (const key of keys) {
    try {
      ...
      if (key !== MARKET_CACHE_KEY) {
        try { localStorage.setItem(MARKET_CACHE_KEY, JSON.stringify({ ...parsed, version: "V1.1-alpha.26.43", migrated_from: key })); } catch {}
      }
      return parsed;
    } catch {}
  }
...
function atlasPrepareChartSelection(coin, period = 1) {
  if (!coin?.id) return;
...
function atlasSelectMarketCoin(coin) {
  ...
  renderMarketTable();
  renderDecisionBoard();
  requestAnimationFrame(() => { void renderAnalystPanel({ selection: true }); });
}
...
} function situationPayload() { return { version: "V1.1-alpha.26.43", active_now: [ "public_market_observation", "charts", "source_diagnostic", "human_readable_tests", "local_paper_trading", "solo_beginner_profile_100_eur", "briefing_questions" ], prepared_only: [ "private_backend", "remote_access", "kraken_read_only", "physical_security_layer" ], locked: [ "real_wallet_connection", "private_key", "withdraw_key", "real_order", "automatic_trading" ], current_step: "collect_information_before_private_backend" };
...
} function backendBlueprintPayload() { return { version: "V1.1-alpha.26.43", principle: "separate_public_frontend_from_private_backend", public_layer: { host: "GitHub Pages", allowed: [ "market_observation", "charts", "watchlist", "command_layer_observation", "local_paper_trading" ], forbidden: [ "private_api_keys", "withdraw_keys", "real_orders", "admin_remote_access", "wallet_connection" ] }, private_layer_future: { host: "private_machine_or_secure_local_server", allowed: [ "encrypted_api_secrets", "Kraken_read_only_client", "server_side_paper_trading", "logs", "kill_switch", "restricted_remote_access" ], access: ["authorized_operator_1", "authorized_operator_2"] }, exchange_layer_future: { primary: "Kraken", first_mode: "read_only", later_modes_locked: [ "paper_trading_server", "human_validated_order", "real_micro_transaction" ], never_allowed_initially: [ "withdraw_permission", "fully_autonomous_trading" ] } };
...
/* =========================================================
   V1.1-alpha.26.43 — Atlas Decision Board
...
function renderAll(options = {}) {
  ...
  atlasRenderMarketAccessNotice(); atlasRenderBrokerStrip(); renderMetrics(); renderTicker(); renderMarketTable(); renderWatchlist(); renderScore(getSelectedCoin() || state.coins[0] || null); renderRiskGrid(); renderColdRead(true); renderBeginnerSummary(); renderDecisionBoard(); renderSimulation();
...
} function renderRiskGrid() { if (!els.riskGrid) return; els.riskGrid.innerHTML = ` <div class="risk ${state.liveOk ? "ok" : "wait"}"><span>Marché</span><b>${state.sourceLock?.valid ? "Source Lock CoinGecko" : "Source canonique absente"}</b></div> <div class="risk warn"><span>Sécurité</span><b>Non vérifiée V1.1-alpha.26.43</b></div> <div class="risk warn"><span>Social</span><b>Non vérifié</b></div> <div class="risk warn"><span>On-chain</span><b>Non vérifié</b></div>`;
...
} function simulationDataSnapshot() { if (!state.sim) loadSimulation(); const totals = getSimulationTotals(); return { generated_at: new Date().toISOString(), version: "V1.1-alpha.26.43", public_only: true, warning: "Données publiques et simulation locale uniquement. Aucun compte réel, aucune clé API, aucun wallet.", profile: getSimulationProfileStatus(), simulation: simulationPayload(), totals: { cash_eur: state.sim.cash, positions_value_eur: totals.positionsValue, total_value_eur: totals.total, pnl_eur: totals.pnl }, market_snapshot: publicMarketSnapshot() };
...
} function makeCollectorRecord() { const snapshot = simulationDataSnapshot(); const logs = snapshot?.simulation?.logs || []; const marketAssets = snapshot?.market_snapshot?.assets || []; return { id: `snapshot_${Date.now()}`, saved_at: new Date().toISOString(), version: "V1.1-alpha.26.43", public_only: true, source: snapshot?.market_snapshot?.source || "source live", live_ok: !!snapshot?.market_snapshot?.live_ok, symbols: marketAssets.map(a => a.symbol), learning_tags: collectorRecordReasonFromLogs(logs), snapshot };
...
} function downloadCollectorJSON() { const records = readCollectorMemory(); const payload = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.43", public_only: true, warning: "Export mémoire locale public-compatible. Aucun compte réel, aucune clé API, aucun wallet.", count: records.length, records }; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.json`, "application/json", JSON.stringify(payload, null, 2) ); showCollectorMemory();
} function downloadCollectorJSONL() { const records = readCollectorMemory(); const header = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.43", public_only: true, type: "agent_crypto_collector_memory_jsonl_header" }; const lines = [JSON.stringify(header), ...records.map(r => JSON.stringify(r))]; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.jsonl`, "application/x-ndjson", lines.join("\n") ); showCollectorMemory();
...
} function buildMemoryReportMarkdown() { const records = readCollectorMemory(); const lines = [ "# RAPPORT MÉMOIRE LOCALE — Agent-Crypto @erith.IA", "", "Version : V1.1-alpha.26.43", `Date : ${new Date().toISOString()}`, "", "## Statut sécurité", "", "- Mémoire locale navigateur uniquement.", "- Données public-compatible.", "- Aucun compte réel.", "- Aucune clé API.", "- Aucun wallet.", "- Aucun ordre réel.", "", "## Lecture mémoire", "", exploreMemoryText(records), "", "## Comparaison premier / dernier", "", compareMemoryText(records), "", "## Refus de sécurité", "", refusalSummaryText(records), "", "## Conclusion", "", "L’explorateur transforme les snapshots en lecture pédagogique. La prochaine vraie étape sera de déplacer ce principe vers une base locale plus solide sur PC Ryzen 7." ]; return lines.join("\n");
...
} /* ========================================================= V1.1-alpha.26.43 — Atlas Auto Reader Ouverture page -> Livecheck auto -> snapshots -> lecture marché. ========================================================= */ const AUTO_MEMORY_KEY = "agent_crypto_erith_ia_auto_reader_v1_1_alpha_13";
...
} function makeAutoSnapshot() { const collectorId = getCollectorId(); const wanted = new Set(state.watchIds || []); const watch = state.coins.filter(c => wanted.has(c.id)); const leaders = state.coins.slice(0, 20); const merged = [...leaders, ...watch].filter((c, i, arr) => arr.findIndex(x => x.id === c.id) === i); const created = new Date().toISOString(); return { id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, snapshot_id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, collector_id: collectorId, collector_type: "local_browser", saved_at: created, version: "V1.1-alpha.26.43", source: state.mainSource || null, source_time: state.timestamp || null, live_ok: !!state.liveOk, cadence_ms: state.auto?.intervalMs || ATLAS_MARKET_REFRESH_MS, global: { market_cap_eur: state.global?.total_market_cap?.eur ?? null, volume_24h_eur: state.global?.total_volume?.eur ?? null, btc_dominance_pct: state.global?.market_cap_percentage?.btc ?? null }, assets: merged.map(compactCoinForAuto) };
...
$("btnAddWatch")?.addEventListener("click", addWatch); document.querySelectorAll(".period-btn[data-period]").forEach(btn => { btn.addEventListener("click", () => { const period = Number(btn.dataset.period) || 1; const coin = getSelectedCoin(); if (!coin) return; atlasPrepareChartSelection(coin, period); requestAnimationFrame(() => { void renderAnalystPanel({ periodChange: true }); }); });
...
/* =========================================================
   V1.1-alpha.26.43 — Mesure d’audience active
   Collecte chiffrée côté navigateur -> canal ntfy dédié.
   La console privée de lecture est livrée séparément.
   ========================================================= */
...
const ATLAS_AUDIENCE_SENT_KEY = "agent_crypto_erith_ia_audience_sent_v2";
const ATLAS_AUDIENCE_BOOT_AT = Date.now();
const atlasAudienceState = { network: null, cryptoKey: null, sent: Number(localStorage.getItem(ATLAS_AUDIENCE_SENT_KEY) || 0), lastEvent: null, ready: false };
...
function atlasAudienceVisitorId() {
  try {
    ...
    if (!id) { id = crypto?.randomUUID?.() || `visitor-${Date.now()}-${Math.random().toString(36).slice(2)}`; localStorage.setItem(ATLAS_AUDIENCE_VISITOR_KEY, id); }
    return id;
  } catch { return null; }
}
function atlasAudienceSessionId() {
  try {
    let id = sessionStorage.getItem(ATLAS_AUDIENCE_SESSION_KEY);
    if (!id) { id = crypto?.randomUUID?.() || `session-${Date.now()}-${Math.random().toString(36).slice(2)}`; sessionStorage.setItem(ATLAS_AUDIENCE_SESSION_KEY, id); }
    return id;
...
async function atlasAudienceEncrypt(payload) {
  ...
  return { v: 1, alg: "RSA-OAEP-256+A256GCM", ek: atlasAudienceB64(encryptedKey), iv: atlasAudienceB64(iv), ct: atlasAudienceB64(ciphertext) };
}
...
function atlasAudiencePayload(eventName, detail = {}) {
  const network = atlasAudienceState.network || {};
  return {
    schema: "erith.audience.event.v2", app: "agent_crypto_erith_ia", version: "V1.1-alpha.26.43",
    event: String(eventName || "event"), occurred_at: new Date().toISOString(),
    visitor_id: atlasAudienceVisitorId(), session_id: atlasAudienceSessionId(), member_id: atlasAudienceMember() || null,
    route: location.pathname, section: location.hash || "#analyste", title: document.title,
    language: navigator.language || null, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || null,
    local_hour: new Date().getHours(), browser: atlasAudienceBrowser(), platform: navigator.platform || null,
    user_agent: navigator.userAgent || null, device: atlasAudienceDevice(),
    viewport: `${window.innerWidth || 0}x${window.innerHeight || 0}`, screen: `${screen?.width || 0}x${screen?.height || 0}`,
    online: navigator.onLine !== false, connection: navigator.connection?.effectiveType || null,
    referrer: document.referrer || null, ip: network.ip || null, city: network.city || null, region: network.region || null,
    country: network.country || null, country_code: network.country_code || null, postal: network.postal || null,
    latitude: network.latitude ?? null, longitude: network.longitude ?? null, asn: network.asn || null, network_org: network.org || null,
    detail
  };
}
async function atlasTrackAudience(eventName, detail = {}) {
  try {
    if (!atlasAudienceState.network) await atlasAudienceNetworkProfile();
    const payload = atlasAudiencePayload(eventName, detail);
    const envelope = await atlasAudienceEncrypt(payload);
    await fetch(ATLAS_AUDIENCE_PUBLISH_URL, { method: "POST", mode: "no-cors", cache: "no-store", keepalive: true, body: JSON.stringify(envelope) });
    atlasAudienceState.sent += 1;
    atlasAudienceState.lastEvent = payload.occurred_at;
    atlasAudienceState.ready = true;
    try { localStorage.setItem(ATLAS_AUDIENCE_SENT_KEY, String(atlasAudienceState.sent)); } catch {}
    atlasRenderAudienceStatus();
    return true;
  } catch (error) {
    atlasAudienceState.ready = false;
    atlasRenderAudienceStatus(String(error?.message || error));
    return false;
  }
}
function atlasRenderAudienceStatus(errorText = "") {
  ...
  setText(document.getElementById("audienceModuleStatus"), errorText ? "En attente réseau" : atlasAudienceState.ready ? "Collecte active" : "Initialisation");
  ...
  setText(document.getElementById("audienceSent"), String(atlasAudienceState.sent));
  setText(document.getElementById("audienceLast"), atlasAudienceState.lastEvent ? new Date(atlasAudienceState.lastEvent).toLocaleTimeString("fr-FR") : "—");
}
async function atlasInitAudienceModule() {
  atlasRenderAudienceStatus();
  await atlasDelay(900);
  await atlasAudienceNetworkProfile();
  await atlasTrackAudience("app_open", { hash: location.hash || "#analyste" });
  document.querySelectorAll(".atlas-collapse").forEach(section => section.addEventListener("toggle", () => { if (section.open) void atlasTrackAudience("section_open", { key: section.dataset.collapseKey || section.id || "section" }); }));
  ...
  window.setInterval(() => void atlasTrackAudience("session_ping", { duration_seconds: Math.max(0, Math.round((Date.now() - ATLAS_AUDIENCE_BOOT_AT) / 1000)), asset: state.selectedCoinId || null }), 5 * 60 * 1000);
}
...
} /* Atlas-10 Crypto — Math Core intégré V1.1-alpha.26.43 Source: modules .md Atlas Math. Exécution: traduction JS condensée. Lecture seule : aucun ordre réel, aucune clé API, aucun capital engagé. */
