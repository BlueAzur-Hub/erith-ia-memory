...
} function pseudoSeries(c, points = 36) { if (!c || typeof c.price !== "number") return []; const period = Number(state.chartPeriodDays || 1); const change = period === 1 ? (typeof c.change24h === "number" ? c.change24h : 0) : (typeof c.change7d === "number" ? c.change7d : (typeof c.change24h === "number" ? c.change24h : 0)); const start = c.price / (1 + change / 100 || 1); const now = Date.now(); const spanMs = period * 24 * 60 * 60 * 1000; return Array.from({ length: points }, (_, i) => { const t = points <= 1 ? 1 : i / (points - 1); const wave = Math.sin(t * Math.PI * 3) * 0.004 + Math.cos(t * Math.PI * 7) * 0.002; const v = Math.max(0, start + (c.price - start) * t + c.price * wave); return [now - spanMs + spanMs * t, v]; });
} function memoryRecordSource(record) {
  return String(
    record?.source ||
    record?.market_source ||
    record?.mainSource ||
    record?.snapshot?.source ||
    record?.market?.source ||
    ""
  ).trim();
}

function normalizeChartSourceName(value) {
  const s = String(value || "").toLowerCase();
  if (s.includes("coingecko")) return "CoinGecko";
  if (s.includes("coinlore")) return "CoinLore";
  return value ? String(value).trim() : "";
}

function chartSourceCompatible(recordSource, wantedSource) {
  const a = normalizeChartSourceName(recordSource);
  const b = normalizeChartSourceName(wantedSource);
  if (!b) return true;
  if (!a) return false;
  return a === b;
}

function cleanChartJumps(points, maxJumpPct = 12) {
  if (!Array.isArray(points) || points.length < 3) return points || [];
  const cleaned = [points[0]];
  for (let i = 1; i < points.length; i++) {
    const prev = Number(cleaned[cleaned.length - 1]?.[1]);
    const next = Number(points[i]?.[1]);
    if (!Number.isFinite(prev) || !Number.isFinite(next) || prev <= 0 || next <= 0) continue;
    const jump = Math.abs((next - prev) / prev) * 100;
    if (jump <= maxJumpPct || i === points.length - 1) cleaned.push(points[i]);
  }
  return cleaned.length >= 2 ? cleaned : points;
}

function seriesFromAtlasMemory(c, days, maxPoints = 180, wantedSource = state.mainSource) {
  if (!c) return [];
  const now = Date.now();
  const spanMs = Number(days || 1) * 24 * 60 * 60 * 1000;
  const since = now - spanMs;
  const sourceWanted = normalizeChartSourceName(wantedSource);
  ...
  const records = readAutoMemory()
    .filter(r => {
      ...
      return chartSourceCompatible(memoryRecordSource(r), sourceWanted);
    })
  ...
  const cleaned = cleanChartJumps(points, c?.symbol === "BTC" ? 8 : 15);

  if (cleaned.length > maxPoints) {
    const step = Math.ceil(cleaned.length / maxPoints);
    return cleaned.filter((_, i) => i % step === 0 || i === cleaned.length - 1);
  }
...
function atlasChartCoverageHours(series) {
  ...
  const times = series
    .map(p => Array.isArray(p) ? Number(p[0]) : Number(p?.time || p?.t || p?.timestamp))
    .filter(Number.isFinite)
    .sort((a, b) => a - b);
  if (times.length < 2) return 0;
  return Math.max(0, (times[times.length - 1] - times[0]) / 3600000);
}

function atlasChartFormatCoverage(hours) {
...
function atlasChartMemoryEnoughForPeriod(series, days) {
  const d = Number(days || 1);
  if (!Array.isArray(series) || series.length < 2) return false;
  if (d === 1) return true;
  const coverage = atlasChartCoverageHours(series);
  if (d === 7) return coverage >= 6;
  if (d >= 30) return coverage >= 24;
  return coverage >= 2;
}

function atlasChartIndicativeSeries(c, days, points = 72) {
  if (!c || typeof c.price !== "number" || !Number.isFinite(c.price)) return [];
  const d = Number(days || 1);
  const change = d === 1
  ...
  const now = Date.now();
  const spanMs = d * 24 * 60 * 60 * 1000;
  const waveScale = d === 1 ? 0.004 : d === 7 ? 0.007 : 0.010;
  return Array.from({ length: points }, (_, i) => {
    const t = points <= 1 ? 1 : i / (points - 1);
    const wave = Math.sin(t * Math.PI * 3.2) * waveScale + Math.cos(t * Math.PI * 8.1) * (waveScale * 0.45);
    const v = Math.max(0, start + (c.price - start) * t + c.price * wave);
    return [now - spanMs + spanMs * t, v];
  });
}

function drawChartLoading(canvas, title = "Chargement", detail = "Atlas charge la vraie période demandée.") {
  drawChartBlocked(canvas, title, detail);
}
...
function atlasNormalizeChartPayload(payload) {
  const raw = Array.isArray(payload?.prices) ? payload.prices : Array.isArray(payload?.series) ? payload.series : [];
  return raw
...
async function fetchAtlasChartCache(c, days, options = {}) {
  ...
  if (prices.length < 3) return { ok: false, error: `cache GitHub ${slug} sans assez de points` };
  const age = atlasChartCacheAgeLabel(payload.generated_at);
  return {
    ok: true,
    result: {
      series: prices,
      source: `cache GitHub CoinGecko market_chart réel · ${slug} · ${prices.length} points · généré il y a ${age}`,
      blocked: false,
...
async function fetchChartSeries(c, days, options = {}) {
  if (!c?.id) return { series: [], source: "graphique réel indisponible — aucun actif sélectionné", blocked: true, kind: "empty" };

  const period = Number(days || 1);
  const key = `${c.id}:${period}:coingecko-cache-or-direct`;
  const ttlMs = period === 1 ? 45000 : period === 7 ? 90000 : 120000;
  const cached = state.chartCache[key];
  if (cached && cached.createdAt && Date.now() - cached.createdAt < ttlMs) return cached.result;

  let cacheError = "cache non tenté";
  try {
  ...
  try {
    const response = await fetch(url, { cache: "no-store", signal: options.signal });
    if (!response.ok) {
      return { series: [], source: `${cacheError} · CoinGecko direct HTTP ${response.status}`, blocked: true, kind: "no-real-chart" };
    }
    const data = await response.json();
    const prices = atlasNormalizeChartPayload(data);
    if (prices.length < 3) return { series: [], source: `${cacheError} · CoinGecko direct sans assez de points`, blocked: true, kind: "no-real-chart" };
    const result = { series: prices, source: "CoinGecko market_chart réel direct", blocked: false, kind: "coingecko-direct", pointCount: prices.length };
    state.chartCache[key] = { createdAt: Date.now(), result };
    return result;
  } catch (error) {
    if (error?.name === "AbortError") throw error;
    return { series: [], source: `${cacheError} · CoinGecko direct indisponible · ${String(error?.message || error || "erreur réseau")}`, blocked: true, kind: "no-real-chart" };
  }
}
...
function drawLineChart(canvas, series, label = "") {
  ...
  if (rows.length < 3) {
    drawChartBlocked(canvas, "Graphique réel indisponible", "CoinGecko market_chart n’a pas fourni assez de points.");
    return;
  }
  ...
  if (window.Chart) {
    ...
    const labels = rows.map(r => atlasChartLabelForTime(r.t, period));
    const data = rows.map(r => r.price);
    const ctx = canvas.getContext("2d");
    const gradient = ctx.createLinearGradient(0, 0, 0, canvas.clientHeight || 400);
    gradient.addColorStop(0, "rgba(98,236,255,0.34)");
    gradient.addColorStop(0.55, "rgba(98,236,255,0.10)");
    gradient.addColorStop(1, "rgba(98,236,255,0.018)");
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            titleColor: "#fff0c8",
            bodyColor: "#e8f6ff",
            borderColor: "rgba(98,236,255,0.42)",
            borderWidth: 1,
            padding: 12,
            ...
            callbacks: {
              title(items) {
                ...
                return atlasChartLabelFull(rows[idx]?.t || Date.now());
              },
              label(item) { return `Prix : ${safeMoney(item.raw)}`; },
              afterBody() { return `Source : CoinGecko market_chart réel`; }
            }
          }
        },
        scales: {
          x: {
            grid: { color: "rgba(176,236,255,0.11)", drawTicks: true },
            ticks: { color: "rgba(226,244,255,0.92)", maxTicksLimit: period === 1 ? 9 : 10, autoSkip: true, maxRotation: 0, font: { size: 12, weight: "700" } }
          },
          y: {
            position: "right",
            min: min - yPad,
            max: max + yPad,
            grid: { color: "rgba(176,236,255,0.17)" },
            ticks: { color: "rgba(226,244,255,0.92)", maxTicksLimit: 8, callback(value) { return safeMoney(Number(value)); }, font: { size: 12, weight: "700" }, padding: 8 }
          }
        }
      }
    });
    ...
    if (shell) {
      ...
      shell.dataset.chartSummary = `${label} · ${safeMoney(last)} · ${changePct >= 0 ? "+" : ""}${changePct.toFixed(2)} % · min ${safeMoney(min)} · max ${safeMoney(max)} · ${rows.length} points`;
    }
    return;
  }

  // Fallback canvas interne si Chart.js ne charge pas. Toujours données réelles CoinGecko.
  const ctx = canvas.getContext("2d");
  ...
  const xFor = i => left + (i / (rows.length - 1)) * plotW;
  const yFor = v => top + plotH - ((v - lo) / (hi - lo || 1)) * plotH;
  ctx.strokeStyle = "rgba(255,255,255,0.13)";
  for (let i = 0; i <= 5; i++) { const y = top + (plotH / 5) * i; ctx.beginPath(); ctx.moveTo(left, y); ctx.lineTo(left + plotW, y); ctx.stroke(); }
  ctx.beginPath(); rows.forEach((r, i) => { const x = xFor(i), y = yFor(r.price); if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); });
  ctx.strokeStyle = "rgb(102,242,255)"; ctx.lineWidth = 4; ctx.stroke();
  ctx.fillStyle = "rgba(255,240,200,0.98)"; ctx.font = "900 18px system-ui, sans-serif"; ctx.fillText(`${label} · ${safeMoney(last)}`, left, 24);
  ctx.fillStyle = "rgba(205,220,240,0.86)"; ctx.font = "700 11px system-ui, sans-serif"; ctx.fillText(`min ${safeMoney(min)} · max ${safeMoney(max)} · ${rows.length} points · GRAPHE RÉEL COINGECKO`, left, height - 12);
}

function drawChartBlocked(canvas, title = "Graphique réel indisponible", detail = "CoinGecko market_chart n’a pas répondu pour cette période.") {
  atlasDestroyRealChart();
  if (!canvas) return;
  const ctx = canvas.getContext("2d");
  const rect = canvas.getBoundingClientRect();
  ...
  for (let i = 1; i < 5; i++) { const y = (height / 5) * i; ctx.beginPath(); ctx.moveTo(54, y); ctx.lineTo(width - 84, y); ctx.stroke(); }
  ctx.fillStyle = "rgba(255,240,200,0.98)";
  ctx.font = "900 22px system-ui, sans-serif";
  ctx.fillText(title, 54, 78);
  ctx.fillStyle = "rgba(205,220,240,0.86)";
  ctx.font = "700 13px system-ui, sans-serif";
  ctx.fillText(String(detail || "").slice(0, 150), 54, 110);
  ctx.fillStyle = "rgba(255,214,122,0.92)";
  ctx.font = "800 12px system-ui, sans-serif";
  ctx.fillText("Aucune courbe indicative dans le graphe principal. Cache GitHub puis CoinGecko direct ; comparaison externe : CoinGecko / CoinMarketCap.", 54, height - 30);
}

function sparkSvg(c) { const oldPeriod = state.chartPeriodDays; state.chartPeriodDays = 7; const data = pseudoSeries(c, 18).map(p => p[1]); state.chartPeriodDays = oldPeriod; if (!data.length) return "—"; const min = Math.min(...data); const max = Math.max(...data); const pts = data.map((v, i) => { const x = (i / (data.length - 1)) * 112; const y = 26 - ((v - min) / (max - min || 1)) * 24; return `${x.toFixed(1)},${y.toFixed(1)}`; }).join(" "); return `<svg class="sparkline" viewBox="0 0 112 28" aria-hidden="true"><polyline points="${pts}" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round" opacity=".95"/></svg>`;
  ...
  drawChartLoading(
    ...
    "Chart Engine V2 charge la période demandée, annule l’ancienne requête et mettra à jour la courbe dès réception."
  );

  if (els.chartCaption) {
    els.chartCaption.textContent = `Graphique ${c.symbol} · période ${periodLabel} · chargement Chart Engine V2.`;
  }

  fetchChartSeries(c, period, { signal: controller.signal }).then(result => {
    ...
    if (result.blocked || !Array.isArray(result.series) || result.series.length < 2) {
      drawChartBlocked(
        els.mainChart,
        `${c.symbol} ${periodLabel} · indisponible`,
        result.source || "Aucune donnée chiffrée suffisante pour dessiner cette période."
      );
    } else {
    ...
    if (els.chartCaption) {
      const summary = Array.isArray(result.series) && result.series.length >= 3 ? `${atlasChartSeriesSummary(result.series)} · ` : "";
      els.chartCaption.textContent = `Graphique ${c.symbol} · période ${periodLabel} · ${summary}source : ${result.source}.`;
    }
  }).catch(error => {
    ...
    drawChartBlocked(
      els.mainChart,
      `${c.symbol} ${periodLabel} · erreur graphique`,
      String(error?.message || error || "Erreur inconnue")
    );
    if (els.chartCaption) {
      els.chartCaption.textContent = `Graphique ${c.symbol} · période ${periodLabel} · erreur Chart Engine V2.`;
    }
  }).finally(() => {
    if (renderToken === state.chartRenderToken && Number(state.chartPeriodDays || 1) === period) {
...
} function situationPayload() { return { version: "V1.1-alpha.26.30", active_now: [ "public_market_observation", "charts", "source_diagnostic", "human_readable_tests", "local_paper_trading", "solo_beginner_profile_100_eur", "briefing_questions" ], prepared_only: [ "private_backend", "remote_access", "kraken_read_only", "physical_security_layer" ], locked: [ "real_wallet_connection", "private_key", "withdraw_key", "real_order", "automatic_trading" ], current_step: "collect_information_before_private_backend" };
...
} function backendBlueprintPayload() { return { version: "V1.1-alpha.26.30", principle: "separate_public_frontend_from_private_backend", public_layer: { host: "GitHub Pages", allowed: [ "market_observation", "charts", "watchlist", "command_layer_observation", "local_paper_trading" ], forbidden: [ "private_api_keys", "withdraw_keys", "real_orders", "admin_remote_access", "wallet_connection" ] }, private_layer_future: { host: "private_machine_or_secure_local_server", allowed: [ "encrypted_api_secrets", "Kraken_read_only_client", "server_side_paper_trading", "logs", "kill_switch", "restricted_remote_access" ], access: ["authorized_operator_1", "authorized_operator_2"] }, exchange_layer_future: { primary: "Kraken", first_mode: "read_only", later_modes_locked: [ "paper_trading_server", "human_validated_order", "real_micro_transaction" ], never_allowed_initially: [ "withdraw_permission", "fully_autonomous_trading" ] } };
...
/* =========================================================
   V1.1-alpha.26.30 — Atlas Decision Board
...
} function renderRiskGrid() { if (!els.riskGrid) return; els.riskGrid.innerHTML = ` <div class="risk ${state.liveOk ? "ok" : "wait"}"><span>Marché</span><b>${state.liveOk ? "Source live OK" : "Non récupéré"}</b></div> <div class="risk warn"><span>Sécurité</span><b>Non vérifiée V1.1-alpha.26.30</b></div> <div class="risk warn"><span>Social</span><b>Non vérifié</b></div> <div class="risk warn"><span>On-chain</span><b>Non vérifié</b></div>`;
...
} function simulationDataSnapshot() { if (!state.sim) loadSimulation(); const totals = getSimulationTotals(); return { generated_at: new Date().toISOString(), version: "V1.1-alpha.26.30", public_only: true, warning: "Données publiques et simulation locale uniquement. Aucun compte réel, aucune clé API, aucun wallet.", profile: getSimulationProfileStatus(), simulation: simulationPayload(), totals: { cash_eur: state.sim.cash, positions_value_eur: totals.positionsValue, total_value_eur: totals.total, pnl_eur: totals.pnl }, market_snapshot: publicMarketSnapshot() };
...
} function makeCollectorRecord() { const snapshot = simulationDataSnapshot(); const logs = snapshot?.simulation?.logs || []; const marketAssets = snapshot?.market_snapshot?.assets || []; return { id: `snapshot_${Date.now()}`, saved_at: new Date().toISOString(), version: "V1.1-alpha.26.30", public_only: true, source: snapshot?.market_snapshot?.source || "source live", live_ok: !!snapshot?.market_snapshot?.live_ok, symbols: marketAssets.map(a => a.symbol), learning_tags: collectorRecordReasonFromLogs(logs), snapshot };
...
} function downloadCollectorJSON() { const records = readCollectorMemory(); const payload = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.30", public_only: true, warning: "Export mémoire locale public-compatible. Aucun compte réel, aucune clé API, aucun wallet.", count: records.length, records }; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.json`, "application/json", JSON.stringify(payload, null, 2) ); showCollectorMemory();
} function downloadCollectorJSONL() { const records = readCollectorMemory(); const header = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.30", public_only: true, type: "agent_crypto_collector_memory_jsonl_header" }; const lines = [JSON.stringify(header), ...records.map(r => JSON.stringify(r))]; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.jsonl`, "application/x-ndjson", lines.join("\n") ); showCollectorMemory();
...
} function buildMemoryReportMarkdown() { const records = readCollectorMemory(); const lines = [ "# RAPPORT MÉMOIRE LOCALE — Agent-Crypto @erith.IA", "", "Version : V1.1-alpha.26.30", `Date : ${new Date().toISOString()}`, "", "## Statut sécurité", "", "- Mémoire locale navigateur uniquement.", "- Données public-compatible.", "- Aucun compte réel.", "- Aucune clé API.", "- Aucun wallet.", "- Aucun ordre réel.", "", "## Lecture mémoire", "", exploreMemoryText(records), "", "## Comparaison premier / dernier", "", compareMemoryText(records), "", "## Refus de sécurité", "", refusalSummaryText(records), "", "## Conclusion", "", "L’explorateur transforme les snapshots en lecture pédagogique. La prochaine vraie étape sera de déplacer ce principe vers une base locale plus solide sur PC Ryzen 7." ]; return lines.join("\n");
...
} /* ========================================================= V1.1-alpha.26.30 — Atlas Auto Reader Ouverture page -> Livecheck auto -> snapshots -> lecture marché. ========================================================= */ const AUTO_MEMORY_KEY = "agent_crypto_erith_ia_auto_reader_v1_1_alpha_13";
...
} function makeAutoSnapshot() { const collectorId = getCollectorId(); const wanted = new Set(state.watchIds || []); const watch = state.coins.filter(c => wanted.has(c.id)); const leaders = state.coins.slice(0, 20); const merged = [...leaders, ...watch].filter((c, i, arr) => arr.findIndex(x => x.id === c.id) === i); const created = new Date().toISOString(); return { id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, snapshot_id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, collector_id: collectorId, collector_type: "local_browser", saved_at: created, version: "V1.1-alpha.26.30", source: state.mainSource || null, source_time: state.timestamp || null, live_ok: !!state.liveOk, cadence_ms: state.auto?.intervalMs || 60000, global: { market_cap_eur: state.global?.total_market_cap?.eur ?? null, volume_24h_eur: state.global?.total_volume?.eur ?? null, btc_dominance_pct: state.global?.market_cap_percentage?.btc ?? null }, assets: merged.map(compactCoinForAuto) };
...
} /* Atlas-10 Crypto — Math Core intégré V1.1-alpha.26.30 Source: modules .md Atlas Math. Exécution: traduction JS condensée. Lecture seule : aucun ordre réel, aucune clé API, aucun capital engagé. */
