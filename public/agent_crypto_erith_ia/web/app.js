  ...
  for (const key of keys) {
    try {
      ...
      if (key !== MARKET_CACHE_KEY) {
        try { localStorage.setItem(MARKET_CACHE_KEY, JSON.stringify({ ...parsed, version: "V1.1-alpha.26.41", migrated_from: key })); } catch {}
      }
      return parsed;
    } catch {}
  }
...
async function refreshMarketOnly() {
  ...
  try {
    ...
    try {
      ...
      setTableDecision(`${state.coins.length} actifs EUR directs · USD indisponible`, "warn");      renderAll();
    }
  } catch (error) {
    state.sourceStatus = [
      { key: "coingecko-eur", name: "CoinGecko EUR", kind: "Top 50 marché", status: "ÉCHEC", detail: cleanError(error) },
...
const ATLAS_CHART_SELECTION_AUTO_RETRIES = 1;
const ATLAS_CHART_SELECTION_RETRY_DELAY_MS = 3200;
...
function atlasScheduleChartAutoRetry(c, period, reason = "réponse réseau tardive") {
  ...
  if (els.chartCaption) {
    els.chartCaption.textContent = `Graphique ${c.symbol} · période ${atlasChartPeriodLabel(period)} · nouvelle tentative automatique dans ${Math.round(ATLAS_CHART_SELECTION_RETRY_DELAY_MS / 1000)} s.`;
  }
  ...
  }, ATLAS_CHART_SELECTION_RETRY_DELAY_MS);
  return true;
}
...
async function renderAnalystPanel(options = {}) {
  ...
  try {
    ...
    if (result.blocked || !Array.isArray(result.series) || !result.series.length) {
      ...
      atlasTrackAudience("chart_error", { asset: c.id, symbol: c.symbol, days: period, reason: String(error?.message || error) });
      }
      return;
    }
    ...
    if (renderToken === state.chartRenderToken && Number(state.chartPeriodDays || 1) === period && !atlasChartRetryPending(chartKey)) atlasChartSetPeriodButtons(period, false);
    atlasRenderBrokerStrip();
  }
}
...
} function situationPayload() { return { version: "V1.1-alpha.26.41", active_now: [ "public_market_observation", "charts", "source_diagnostic", "human_readable_tests", "local_paper_trading", "solo_beginner_profile_100_eur", "briefing_questions" ], prepared_only: [ "private_backend", "remote_access", "kraken_read_only", "physical_security_layer" ], locked: [ "real_wallet_connection", "private_key", "withdraw_key", "real_order", "automatic_trading" ], current_step: "collect_information_before_private_backend" };
...
} function backendBlueprintPayload() { return { version: "V1.1-alpha.26.41", principle: "separate_public_frontend_from_private_backend", public_layer: { host: "GitHub Pages", allowed: [ "market_observation", "charts", "watchlist", "command_layer_observation", "local_paper_trading" ], forbidden: [ "private_api_keys", "withdraw_keys", "real_orders", "admin_remote_access", "wallet_connection" ] }, private_layer_future: { host: "private_machine_or_secure_local_server", allowed: [ "encrypted_api_secrets", "Kraken_read_only_client", "server_side_paper_trading", "logs", "kill_switch", "restricted_remote_access" ], access: ["authorized_operator_1", "authorized_operator_2"] }, exchange_layer_future: { primary: "Kraken", first_mode: "read_only", later_modes_locked: [ "paper_trading_server", "human_validated_order", "real_micro_transaction" ], never_allowed_initially: [ "withdraw_permission", "fully_autonomous_trading" ] } };
...
/* =========================================================
   V1.1-alpha.26.41 — Atlas Decision Board
...
} function renderRiskGrid() { if (!els.riskGrid) return; els.riskGrid.innerHTML = ` <div class="risk ${state.liveOk ? "ok" : "wait"}"><span>Marché</span><b>${state.sourceLock?.valid ? "Source Lock CoinGecko" : "Source canonique absente"}</b></div> <div class="risk warn"><span>Sécurité</span><b>Non vérifiée V1.1-alpha.26.41</b></div> <div class="risk warn"><span>Social</span><b>Non vérifié</b></div> <div class="risk warn"><span>On-chain</span><b>Non vérifié</b></div>`;
...
} function simulationDataSnapshot() { if (!state.sim) loadSimulation(); const totals = getSimulationTotals(); return { generated_at: new Date().toISOString(), version: "V1.1-alpha.26.41", public_only: true, warning: "Données publiques et simulation locale uniquement. Aucun compte réel, aucune clé API, aucun wallet.", profile: getSimulationProfileStatus(), simulation: simulationPayload(), totals: { cash_eur: state.sim.cash, positions_value_eur: totals.positionsValue, total_value_eur: totals.total, pnl_eur: totals.pnl }, market_snapshot: publicMarketSnapshot() };
...
} function makeCollectorRecord() { const snapshot = simulationDataSnapshot(); const logs = snapshot?.simulation?.logs || []; const marketAssets = snapshot?.market_snapshot?.assets || []; return { id: `snapshot_${Date.now()}`, saved_at: new Date().toISOString(), version: "V1.1-alpha.26.41", public_only: true, source: snapshot?.market_snapshot?.source || "source live", live_ok: !!snapshot?.market_snapshot?.live_ok, symbols: marketAssets.map(a => a.symbol), learning_tags: collectorRecordReasonFromLogs(logs), snapshot };
...
} function downloadCollectorJSON() { const records = readCollectorMemory(); const payload = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.41", public_only: true, warning: "Export mémoire locale public-compatible. Aucun compte réel, aucune clé API, aucun wallet.", count: records.length, records }; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.json`, "application/json", JSON.stringify(payload, null, 2) ); showCollectorMemory();
} function downloadCollectorJSONL() { const records = readCollectorMemory(); const header = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.41", public_only: true, type: "agent_crypto_collector_memory_jsonl_header" }; const lines = [JSON.stringify(header), ...records.map(r => JSON.stringify(r))]; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.jsonl`, "application/x-ndjson", lines.join("\n") ); showCollectorMemory();
...
} function buildMemoryReportMarkdown() { const records = readCollectorMemory(); const lines = [ "# RAPPORT MÉMOIRE LOCALE — Agent-Crypto @erith.IA", "", "Version : V1.1-alpha.26.41", `Date : ${new Date().toISOString()}`, "", "## Statut sécurité", "", "- Mémoire locale navigateur uniquement.", "- Données public-compatible.", "- Aucun compte réel.", "- Aucune clé API.", "- Aucun wallet.", "- Aucun ordre réel.", "", "## Lecture mémoire", "", exploreMemoryText(records), "", "## Comparaison premier / dernier", "", compareMemoryText(records), "", "## Refus de sécurité", "", refusalSummaryText(records), "", "## Conclusion", "", "L’explorateur transforme les snapshots en lecture pédagogique. La prochaine vraie étape sera de déplacer ce principe vers une base locale plus solide sur PC Ryzen 7." ]; return lines.join("\n");
...
} /* ========================================================= V1.1-alpha.26.41 — Atlas Auto Reader Ouverture page -> Livecheck auto -> snapshots -> lecture marché. ========================================================= */ const AUTO_MEMORY_KEY = "agent_crypto_erith_ia_auto_reader_v1_1_alpha_13";
...
} function makeAutoSnapshot() { const collectorId = getCollectorId(); const wanted = new Set(state.watchIds || []); const watch = state.coins.filter(c => wanted.has(c.id)); const leaders = state.coins.slice(0, 20); const merged = [...leaders, ...watch].filter((c, i, arr) => arr.findIndex(x => x.id === c.id) === i); const created = new Date().toISOString(); return { id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, snapshot_id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, collector_id: collectorId, collector_type: "local_browser", saved_at: created, version: "V1.1-alpha.26.41", source: state.mainSource || null, source_time: state.timestamp || null, live_ok: !!state.liveOk, cadence_ms: state.auto?.intervalMs || ATLAS_MARKET_REFRESH_MS, global: { market_cap_eur: state.global?.total_market_cap?.eur ?? null, volume_24h_eur: state.global?.total_volume?.eur ?? null, btc_dominance_pct: state.global?.market_cap_percentage?.btc ?? null }, assets: merged.map(compactCoinForAuto) };
...
/* =========================================================
   V1.1-alpha.26.41 — Mesure d’audience configurable
   Endpoint public attendu : window.ERITH_AUDIENCE_ENDPOINT
   ========================================================= */
const ATLAS_AUDIENCE_ENDPOINT_KEY = "agent_crypto_erith_ia_audience_endpoint_v1";
const ATLAS_AUDIENCE_MEMBER_KEY = "agent_crypto_erith_ia_audience_member_v1";
const ATLAS_AUDIENCE_VISITOR_KEY = "agent_crypto_erith_ia_audience_visitor_v1";
const ATLAS_AUDIENCE_SESSION_KEY = "agent_crypto_erith_ia_audience_session_v1";
const ATLAS_AUDIENCE_BOOT_AT = Date.now();
function atlasAudienceConfigureFromUrl() {
  try {
    const params = new URLSearchParams(location.search);
    const endpoint = String(params.get("audience_endpoint") || "").trim();
    const member = String(params.get("member") || "").trim().slice(0, 80);
    if (endpoint && /^https:\/\//i.test(endpoint)) localStorage.setItem(ATLAS_AUDIENCE_ENDPOINT_KEY, endpoint);
    if (member) localStorage.setItem(ATLAS_AUDIENCE_MEMBER_KEY, member);
  } catch {}
}
atlasAudienceConfigureFromUrl();
const ATLAS_AUDIENCE_ENDPOINT = String(window.ERITH_AUDIENCE_ENDPOINT || localStorage.getItem(ATLAS_AUDIENCE_ENDPOINT_KEY) || "").trim();
const ATLAS_AUDIENCE_MEMBER = String(window.ERITH_AUDIENCE_MEMBER || localStorage.getItem(ATLAS_AUDIENCE_MEMBER_KEY) || "").trim();
function atlasAudienceVisitorId() {
  try {
    ...
    if (!id) {
      ...
      localStorage.setItem(ATLAS_AUDIENCE_VISITOR_KEY, id);
    }
    return id;
  } catch { return null; }
}
function atlasAudienceSessionId() {
  try {
    ...
    if (!id) {
      ...
      sessionStorage.setItem(ATLAS_AUDIENCE_SESSION_KEY, id);
    }
    return id;
...
function atlasAudiencePayload(eventName, detail = {}) {
  return {
    schema: "erith.audience.event.v1",
    ...
    member_id: ATLAS_AUDIENCE_MEMBER || null,
    ...
    referrer_origin: (() => { try { return document.referrer ? new URL(document.referrer).origin : null; } catch { return null; } })(),
    detail
  };
}
function atlasTrackAudience(eventName, detail = {}) {
  if (!ATLAS_AUDIENCE_ENDPOINT) return false;
  const body = JSON.stringify(atlasAudiencePayload(eventName, detail));
  try {
    if (navigator.sendBeacon) {
      const sent = navigator.sendBeacon(ATLAS_AUDIENCE_ENDPOINT, new Blob([body], { type: "application/json" }));
      if (sent) return true;
    }
    fetch(ATLAS_AUDIENCE_ENDPOINT, { method: "POST", mode: "cors", keepalive: true, headers: { "content-type": "application/json" }, body }).catch(() => {});
    return true;
  } catch { return false; }
}
function atlasInitAudienceModule() {
  const status = document.getElementById("audienceModuleStatus");
  if (status) status.textContent = ATLAS_AUDIENCE_ENDPOINT ? "Collecteur connecté" : "Collecteur à renseigner";
  atlasTrackAudience("app_open", { title: document.title });
  document.querySelectorAll(".atlas-collapse").forEach(section => {
    section.addEventListener("toggle", () => {
      if (section.open) atlasTrackAudience("section_open", { key: section.dataset.collapseKey || section.id || "section" });
    });
  });
  document.querySelectorAll(".period-btn[data-period]").forEach(button => {
    button.addEventListener("click", () => atlasTrackAudience("chart_period", { asset: state.selectedCoinId || null, days: Number(button.dataset.period) || 1 }));
  });
  window.addEventListener("pagehide", () => atlasTrackAudience("session_close", { duration_seconds: Math.max(0, Math.round((Date.now() - ATLAS_AUDIENCE_BOOT_AT) / 1000)) }));
  window.setInterval(() => atlasTrackAudience("session_ping", { duration_seconds: Math.max(0, Math.round((Date.now() - ATLAS_AUDIENCE_BOOT_AT) / 1000)), asset: state.selectedCoinId || null }), 5 * 60 * 1000);
}
...
} /* Atlas-10 Crypto — Math Core intégré V1.1-alpha.26.41 Source: modules .md Atlas Math. Exécution: traduction JS condensée. Lecture seule : aucun ordre réel, aucune clé API, aucun capital engagé. */
