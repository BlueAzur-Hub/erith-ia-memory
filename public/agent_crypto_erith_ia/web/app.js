/* V2.0-alpha · Build 28.1.97R1 — TARGET CYCLE NON-REGRESSION LOCK · CLEAN HOME · INLINE DATA STATUS · GRAPH THREE-STATE · TOP5 FLOW PERSISTENCE · ADMIN GRAPH TOGGLE · MARKET RECENTER · FORGE PRO BRIDGE
...
*/
const ATLAS_RELEASE = "V2.0-alpha · Build 28.1.97R1";
...
function atlasCoinGeckoDirectQuoteForCoin(coinOrId, now = Date.now()) {
  ...
  if (spotPrice && Number.isFinite(spotTimestamp)) {
    ...
    conservedCandidates.push({ ...spotQuote, kind: "coingecko-spot-conserved", source: "CoinGecko spot conservé" });
    if (
      book.status === "ready"
      && book.mode === "direct"
  ...
  if (marketPrice) {
    ...
    conservedCandidates.push({ ...marketQuote, kind: "coingecko-market-conserved", source: "CoinGecko Market conservé" });
    if (
      state.sourceLock?.mode === "direct"
...
function atlasSetStableDirectMarketStatus() {
  ...
  setLiveStatus("ok", `Marché ${assetCount}/250 classés actualisé`);
  setText(els.sourceName, `CoinGecko direct · ${assetCount}/250 classés EUR`);
...
function atlasRenderComparisonControls() {
  ...
  const requestedPreset = atlasScannerTransaction?.preset || atlasScannerQueuedRequest?.preset || "";
  ...
  [
    ...
    button.classList.remove("is-scanner-failed");
  ...
  if (atlasScannerQueuedRequest) {
    ...
    hint = `<span class="compare-hint compare-hint-progress">${escapeHtml(atlasScannerLabel(atlasScannerTransaction.preset))} · cache ${atlasScannerTransaction.cacheHits || 0} · ${atlasScannerTransaction.validCount}/5 prêtes · ${escapeHtml(atlasScannerDisplayedGraphLabel())} reste affiché</span>`;
...
function atlasSyncTargetTopFiveCycleLabel() {
  ...
  const pending = atlasScannerTransaction || atlasScannerQueuedRequest || null;
  const pendingPreset = String(pending?.preset || "");
  ...
  const currentPreset = pendingPreset || committedPreset;
  ...
  if (!pendingPreset) root.dataset.cyclePreset = committedPreset;
  if (pendingPreset) {
    const ready = Number(atlasScannerTransaction?.validCount || 0);
    const examined = Number(atlasScannerTransaction?.examined || 0);
    label.textContent = `${display.label} · ${ready}/5`;
    root.title = `${display.label} en cours · ${examined} actifs examinés · graphique précédent conservé`;
    root.setAttribute("aria-label", `${display.label} en cours, ${ready} séries sur 5 prêtes`);
  } else {
    label.textContent = display.label;
...
function renderMarketTable() {
  ...
  const note = essential
    ? `${rows.length}/${filtered.length} affichés · univers ${state.coins.length}/250 classés · ${selection.length} sélectionnés · ${state.mainSource} · ${updated}`
    : `${rows.length}/${filtered.length} affichés · univers ${state.coins.length}/250 classés · colonnes ${state.chartViewV2.marketColumns === 'complete' ? 'complètes' : 'essentielles'} · sélection ${selection.length}/${ATLAS_COMPARISON_MAX_SERIES} · filtre ${state.assetFilter} · tri ${state.sortKey} · ${updated}`;
...
els.btnChartGainers?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("gainers", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.1.88R4" });
});
els.btnChartLosers?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("losers", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.1.88R4" });
});
els.btnChartVolume5?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("volume", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.1.88R4" });
});
...
/* =========================================================
   Build 28.1.97R1 — Stable Stack Consolidation + Permanent Inline Atlas/Aerith Synthesis
...
const ATLAS_SCANNER_DIRECT_LIMIT = 12;
const ATLAS_SCANNER_TARGET = 5;
const ATLAS_SCANNER_SPACING_MS = 520;
...
function atlasScannerVisibleFailure(preset, message, detail = "") {
  ...
  console.warn(`${label} non appliqué`, String(message || ""), String(detail || ""));
...
function atlasScannerAligned(entries, period, candidateIds) {
  return atlasBuildAlignedComparisonEntries(entries, period)
...
function atlasScannerProgress(tx, symbol = "") {
  ...
  atlasScannerSetUiState(
    tx.preset,
    "running",
    `${atlasScannerLabel(tx.preset)} · ${tx.validCount}/5 · ${tx.examined}/${tx.candidateIds.length}${symbol ? ` · ${String(symbol).toUpperCase()}` : ""}`
  );
  atlasRenderComparisonControls();
}
...
async function atlasFetchBridgeHistory(c, days, options = {}) {
  ...
  const timer = window.setTimeout(() => {
    timedOut = true;
    controller.abort();
  }, Number(options.timeoutMs || ATLAS_HISTORY_BRIDGE_TIMEOUT_MS));

  const query = new URLSearchParams({
    coin_id: String(c.id),
    symbol,
    days: String(Number(days || 1)),
    allow_binance: uniqueBinanceSymbol ? "1" : "0"
  });
  ...
  try {
    ...
    return {
      series,
      volumeSeries,
      source: `Bridge Ryzen V1.6 · ${String(payload?.source || payload?.provider || "historique EUR")}`,
      blocked: false,
      kind: sourceMode,
      sourceMode,
      periodDays: Number(days || 1),
      ...
      integrity,
      bridgeHistory: {
        version: payload?.bridge_version || "1.6",
...
async function atlasScannerFetchDirect(tx, coin) {
  ...
  try {
    const result = await atlasFetchBridgeHistory(coin, tx.period, {
      ...
      timeoutMs: ATLAS_HISTORY_BRIDGE_TIMEOUT_MS
    });
    const provider = String(result?.bridgeHistory?.provider || "bridge");
    atlasStoreChartResult(coin, tx.period, result, provider === "coingecko" ? "coingecko" : "binance");
    return {
      result,
  ...
  /* CoinGecko navigateur reste le dernier transport direct avant cache daté. */
  try {
...
function atlasScannerCommit(tx, finalEntries, rejected) {
  ...
  try {
    ...
    */
    ...
    console.warn("Transaction scanner 28.1.88R4 annulée :", error);
...
async function atlasScannerRun(tx) {
  ...
  try {
    atlasScannerSetUiState(
      tx.preset,
      "warming",
      `${atlasScannerLabel(tx.preset)} · recherche des historiques`
    );
    ...
    for (const coin of coins) {
      ...
      if (atlasScannerAligned(accepted, tx.period, tx.candidateIds).length >= ATLAS_SCANNER_TARGET) break;
      ...
      /*
        ...
        Il ne décide jamais du classement Market.
      */
      if (stored && Array.isArray(stored.series) && stored.series.length) {
        ...
        tx.validCount = accepted.length;
        atlasScannerProgress(tx, coin.symbol);
        continue;
      }

      if (tx.directAttempts >= tx.directLimit) {
        rejected.push({
          id: coin.id,
          symbol: coin.symbol,
          reason: "aucun historique local et limite réseau atteinte"
        });
        continue;
      }

      tx.directAttempts += 1;
      const fetched = await atlasScannerFetchDirect(tx, coin);
      if (!atlasScannerCurrent(tx)) return false;
      ...
      if (
        ...
        accepted.push(
          atlasScannerDecorateEntry(tx, coin, fetched.result, "direct")
        );
        tx.validCount = accepted.length;
      } else {
        rejected.push({
    ...
    return atlasScannerVisibleFailure(
      tx.preset,
      `${atlasScannerLabel(tx.preset)} indisponible · ${finalEntries.length}/5 historiques trouvés après ${tx.examined} candidats`,
      detail
    );
  } catch (error) {
    if (error?.name === "AbortError" || tx?.controller?.signal?.aborted) return false;
    console.error("Hausses 5 / scanner 28.1.88R4 :", error);
...
function atlasScannerStart(preset = "gainers", limit = 5, options = {}) {
  ...
  try {
    ...
    const tx = {
      ...
      validCount: 0,
      displayedSnapshot,
      /* 16 candidats maximum : assez pour trouver 5 séries, sans boucle réseau interminable. */
      directLimit: Math.min(16, pool.length),
      cancelled: false,
    ...
    atlasScannerSetUiState(
      preset,
      "warming",
      `${label} · classement ${marketBasisLabel} · démarrage`
    );
    atlasScannerProgress(tx);

    void atlasScannerRun(tx).catch(error => {
      if (error?.name === "AbortError") return;
      console.error("Scanner 28.1.88R4 non capturé :", error);
      if (atlasScannerTransaction === tx) atlasScannerTransaction = null;
    ...
    console.error("Démarrage Hausses 5 28.1.88R4 :", error);
    return atlasScannerVisibleFailure(
      preset,
...
function atlasSyncMarketUniverseControls() {
  ...
  if (status) status.textContent = `Univers CoinGecko : ${count}/250 classés`;
}
...
/* =========================================================
   Build 28.1.97R1 — Analytical Truth & Evidence + Target Cycle Non-Regression Lock
...
function atlasAnalyticalTruthSchedule(delay = 260) {
  ...
  atlasAnalyticalTruthState.timer = window.setTimeout(() => {
    ...
    if (atlasAnalyticalTruthIsBusy()) {
      atlasAnalyticalTruthSchedule(850);
      return;
    }
    if (Array.isArray(state.coins) && state.coins.length) {
    ...
    } else atlasAnalyticalTruthRender();
...
/* Non-régression Target/Scanner :
   - aucune empreinte n'est recalculée pendant une transaction ;
   - le cartouche affiche immédiatement le preset demandé et sa progression ;
   - un watchdog restaure toujours le graphique précédent au lieu de rester bloqué. */
const atlasScannerSetUiStateV97 = atlasScannerSetUiState;
atlasScannerSetUiState = function atlasScannerSetUiStateR97R1(preset, status, message = "") {
  const result = atlasScannerSetUiStateV97(preset, status, message);
  atlasSyncTargetTopFiveCycleLabel();
  return result;
};

function atlasTargetCycleClearWatchdog(tx) {
  if (tx?.targetCycleWatchdog) {
    window.clearTimeout(tx.targetCycleWatchdog);
    tx.targetCycleWatchdog = 0;
  }
}

const atlasScannerStartV97 = atlasScannerStart;
atlasScannerStart = function atlasScannerStartR97R1(preset = "gainers", limit = 5, options = {}) {
  const started = atlasScannerStartV97(preset, limit, options);
  const tx = atlasScannerTransaction;
  if (started && tx) {
    atlasTargetCycleClearWatchdog(tx);
    tx.targetCycleWatchdog = window.setTimeout(() => {
      if (!atlasScannerCurrent(tx)) return;
      atlasTargetCycleClearWatchdog(tx);
      try { tx.controller.abort(); } catch {}
      tx.cancelled = true;
      ...
      atlasScannerQueuedRequest = null;
      atlasScannerSetTransactionFlag(false);
      if (tx.displayedSnapshot) atlasScannerRestoreSnapshot(tx.displayedSnapshot);
      atlasScannerVisibleFailure(
        tx.preset,
        `${atlasScannerLabel(tx.preset)} interrompu · délai de sécurité atteint`,
        "Le graphique précédent a été conservé. Relance manuelle possible."
      );
      atlasSyncTargetTopFiveCycleLabel();
      atlasAnalyticalTruthSchedule(400);
    }, 45000);
    atlasSyncTargetTopFiveCycleLabel();
  }
  return started;
...
const atlasScannerCommitV97 = atlasScannerCommit;
atlasScannerCommit = function atlasScannerCommitR97R1(tx, finalEntries, rejected) {
  atlasTargetCycleClearWatchdog(tx);
  const result = atlasScannerCommitV97(tx, finalEntries, rejected);
  atlasSyncTargetTopFiveCycleLabel();
  atlasAnalyticalTruthSchedule(350);
  return result;
};

const atlasScannerCancelV97 = atlasScannerCancel;
atlasScannerCancel = function atlasScannerCancelR97R1(reason = "annulé", options = {}) {
  atlasTargetCycleClearWatchdog(atlasScannerTransaction);
  const result = atlasScannerCancelV97(reason, options);
  atlasSyncTargetTopFiveCycleLabel();
  atlasAnalyticalTruthSchedule(350);
  return result;
};

const atlasScannerVisibleFailureV97 = atlasScannerVisibleFailure;
atlasScannerVisibleFailure = function atlasScannerVisibleFailureR97R1(preset, message, detail = "") {
  atlasTargetCycleClearWatchdog(atlasScannerTransaction);
  const result = atlasScannerVisibleFailureV97(preset, message, detail);
  atlasSyncTargetTopFiveCycleLabel();
  atlasAnalyticalTruthSchedule(350);
  return result;
};

const atlasActivateTargetTopFiveCycleV97 = atlasActivateTargetTopFiveCycle;
atlasActivateTargetTopFiveCycle = function atlasActivateTargetTopFiveCycleR97R1() {
  const result = atlasActivateTargetTopFiveCycleV97();
  atlasSyncTargetTopFiveCycleLabel();
  if (!atlasScannerTransaction && !atlasScannerQueuedRequest) atlasAnalyticalTruthSchedule(350);
  return result;
};
