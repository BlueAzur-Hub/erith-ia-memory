...
async function renderAnalystPanel() {
  ...
  // Affichage immédiat compact : jamais de grand bloc vide.
  drawLineChart(els.mainChart, fallbackSeries, `${c.symbol} ${periodLabel}`);

  if (els.chartCaption) {
    els.chartCaption.textContent = `Graphique ${c.symbol} · période ${periodLabel} · stabilisé sur le tableau actif.`;
  }

  const series = await fetchChartSeries(c, period);

  // Anti-saut visuel : si une ancienne requête graphique revient après un nouveau rendu,
  // elle ne doit pas redessiner le canvas ni changer brutalement l’affichage.
...
function renderRiskGrid() {
  ...
  els.riskGrid.innerHTML = `
    ...
    <div class="risk warn"><span>Sécurité</span><b>Non vérifiée V1.1-alpha.25</b></div>
...
function renderColdRead(live = false) {
  ...
  if (live) {
    els.coldRead.textContent =
      ...
      `Lecture froide : prix, volumes et market cap sont disponibles, mais sécurité contrat, social et on-chain restent non validés par cette interface V1.1-alpha.25.`;
  } else {
    els.coldRead.textContent =
...
function buildWakePlanText() {
  ...
  return [
    ...
    "2. Faire Ctrl + F5.",
    "3. Vérifier : GitHub Pack V1.1-alpha.25.",
    "4. Lancer Livecheck.",
...
/* =========================================================
   V1.1-alpha.25 — Atlas Auto Reader
...
function renderAutoReader(snapshot = null, previous = null) {
  ...
  if (els.autoReaderOutput) {
    ...
    els.autoReaderOutput.textContent = [
      "ATLAS AUTO READER — V1.1-alpha.25",
      "",
...
function renderSharedMemory() {
  ...
  if (els.sharedMemoryOutput) {
    ...
    els.sharedMemoryOutput.textContent = [
      "ATLAS SHARED MARKET MEMORY — V1.1-alpha.25",
      "",
      configured
        ? `✅ Machine configurée : ${id}`
  ...
  try {
    ...
    if (showMessages && els.githubMemoryOutput) {
      els.githubMemoryOutput.textContent = [
        ...
        "Étape active : alpha.25 installe GitHub Action Collector pour créer data/latest.json automatiquement."
      ].join("\n");
    }
...
/* Atlas-10 Crypto — Math Core intégré V1.1-alpha.25
