...
const ATLAS_SPOT_REFRESH_MS = 5 * 60 * 1000;
...
function atlasBrokerModeLabel(mode) {
  ...
  if (mode === "github-cache") return "CoinGecko · archive GitHub";
  if (mode === "local-cache") return "CoinGecko · archive locale";
...
function atlasRenderMarketAccessNotice() {
  ...
  els.offlineNotice.style.display = "block";
  els.offlineNotice.innerHTML = `<strong>MODE ARCHIVE COINGECKO</strong><p>Dernière lecture directe conservée depuis ${origin} · ${escapeHtml(atlasBrokerAgeLabel(market.timestamp))}. Prix consultables ; scores et simulation suspendus jusqu’au retour du flux marché EUR direct.</p>`;
}
...
const SourceAdapter = {
  async coingeckoTop50Eur() {
    ...
    const rows = await fetchJsonWithRetry(url, {}, 14000, 2);
  ...
  async coingeckoUsdForIds(ids) {
    ...
    const payload = await fetchJsonWithRetry(url, {}, 11000, 2);
  ...
  for (const key of keys) {
    try {
      ...
      if (key !== MARKET_CACHE_KEY) {
        try { localStorage.setItem(MARKET_CACHE_KEY, JSON.stringify({ ...parsed, version: "V1.1-alpha.26.46.1", migrated_from: key })); } catch {}
      }
      return parsed;
    } catch {}
  }
...
function atlasRestorePreviousSnapshot(previous, error, context = "Actualisation") {
  ...
  state.mainSource = previous.mainSource || "CoinGecko · dernière lecture directe";
  state.liveOk = true;
  const age = atlasMarketAgeMs();
  const recent = age <= ATLAS_DIRECT_GRACE_MS;
  atlasSetSourceLock(recent ? "direct" : "local-cache", state.timestamp, `${context} différée : ${cleanError(error)}`, true, previous.snapshotId || previous.sourceLock?.snapshotId || null);
  atlasBrokerCommitMarket({ coins: state.coins, global: state.global, timestamp: state.timestamp, snapshotId: state.sourceLock.snapshotId }, recent ? "direct" : "local-cache");
  if (recent) {
    setLiveStatus("warn", "Top 50 conservé · réseau différé");
    setText(els.sourceName, "CoinGecko direct · dernier snapshot récent");
    setTableDecision(`Top 50 récent conservé · nouvelle tentative dans 5 min`, "warn");
  } else {
    setLiveStatus("warn", "Mode archive");
    setText(els.sourceName, "CoinGecko · archive locale");
    setTableDecision("Archive CoinGecko · analyses suspendues", "warn");
  }
...
async function runLivecheck() {
  ...
  const previousDirect = state.sourceLock?.mode === "direct" && atlasCanonicalSnapshot(state.coins)
    ? { coins: state.coins, global: state.global, mainSource: state.mainSource, timestamp: state.timestamp, sourceLock: state.sourceLock }
    : null;
  ...
  try {
    ...
    const result = await SourceAdapter.coingeckoTop50Eur();
    ...
    setLiveStatus("ok", "Livecheck Top 50 EUR OK");
    atlasTrackAudience("market_loaded", { assets: state.coins.length, mode: "direct" });
    setText(els.sourceName, state.mainSource);
    setText(els.sourceTime, new Date(state.timestamp).toLocaleString("fr-FR"));
    setTableDecision(`${state.coins.length} actifs EUR directs · Top 50 affiché`, "ok");
    renderSourceGrid();
    updateSourceMetric(1);
    atlasRenderMarketAccessNotice();
    renderAll({ refreshChart: false });
    renderTrustLock(true);
    atlasAfterLivecheck();
    atlasStartSelectedChart(160, true);
    await atlasDelay(260);
    await atlasWaitForChartIdle(6500);

    try {
      const usdStartedAt = performance.now();
      const usdResult = await SourceAdapter.coingeckoUsdForIds(state.coins.map(coin => coin.id));
      ...
      state.sourceStatus = [
        ...
        { key: "coingecko-usd", name: "CoinGecko USD", kind: "Enrichissement Top 50", status: "OK", ms: usdLatencyMs, detail: `${enriched}/${state.coins.length} prix USD` }
      ];
      setText(els.sourceName, "CoinGecko direct · Top 50 EUR + USD");
      setTableDecision(`${state.coins.length} actifs directs · EUR + USD`, "ok");
      renderAll({ refreshChart: false });
    } catch (usdError) {
      state.sourceStatus = [
        state.sourceStatus.find(item => item.key === "coingecko-eur") || { ...result.leg },
        { key: "coingecko-usd", name: "CoinGecko USD", kind: "Enrichissement Top 50", status: "INDISPONIBLE", detail: cleanError(usdError) }
      ];
      setLiveStatus("warn", "Top 50 EUR OK · USD indisponible");
      setTableDecision(`${state.coins.length} actifs EUR directs · USD indisponible`, "warn");
      renderAll({ refreshChart: false });
    }
    renderSourceGrid();
    ...
    state.sourceStatus = [
      { key: "coingecko-eur", name: "CoinGecko EUR", kind: "Top 50 marché", status: "ÉCHEC", ms: eurStartedAt ? Math.round(performance.now() - eurStartedAt) : null, detail: cleanError(error) },
      { key: "coingecko-usd", name: "CoinGecko USD", kind: "Enrichissement Top 50", status: "NON LANCÉ", detail: "Le marché EUR requis a échoué" }
    ];
    if (previousDirect && atlasRestorePreviousSnapshot({ ...previousDirect, snapshotId: previousDirect.sourceLock?.snapshotId || null }, error, "Livecheck")) {
      // La dernière lecture directe récente reste utilisable ; une erreur réseau transitoire ne vide plus Atlas.
...
async function refreshMarketOnly() {
  ...
  state.auto.lastStartedAt = new Date().toISOString();
  state.auto.lastRunMs = Date.now();
  const previous = state.coins.length ? { coins: state.coins, global: state.global, timestamp: state.timestamp, snapshotId: state.sourceLock?.snapshotId } : null;
  let refreshed = false;
  let eurStartedAt = 0;
  try {
    eurStartedAt = performance.now();
    ...
    setText(els.sourceName, state.mainSource);
    setText(els.sourceTime, new Date(state.timestamp).toLocaleString("fr-FR"));
    setTableDecision(`${state.coins.length} actifs EUR directs`, "ok");
    atlasRenderMarketAccessNotice();
    renderAll({ refreshChart: false });
    if (!state.chartEngineV2?.realChart && state.dataBroker.chart?.status !== "loading") atlasStartSelectedChart(160, true);
    ...
    try {
      ...
      const usdLatencyMs = Math.round(performance.now() - usdStartedAt);
      const enriched = atlasApplyUsdEnrichment(usdResult);
      state.sourceStatus.push({ key: "coingecko-usd", name: "CoinGecko USD", kind: "Enrichissement Top 50", status: "OK", ms: usdLatencyMs, detail: `${enriched}/${state.coins.length} prix USD` });
      setText(els.sourceName, "CoinGecko direct · Top 50 EUR + USD");
      setTableDecision(`${state.coins.length} actifs directs · EUR + USD`, "ok");
      renderAll({ refreshChart: false });
    } catch (usdError) {
      state.sourceStatus.push({ key: "coingecko-usd", name: "CoinGecko USD", kind: "Enrichissement Top 50", status: "INDISPONIBLE", detail: cleanError(usdError) });
      setLiveStatus("warn", "Top 50 EUR actualisé · USD indisponible");
      setTableDecision(`${state.coins.length} actifs EUR directs · USD indisponible`, "warn");
      renderAll({ refreshChart: false });
    }
  } catch (error) {
    ...
    if (previous && atlasRestorePreviousSnapshot({ ...previous, mainSource: "CoinGecko · dernière lecture directe" }, error, "Rafraîchissement")) {
      // Conservation non destructive du dernier snapshot valide.
    } else if (!applyMarketCache()) {
      clearMarketDisplay("Aucun snapshot CoinGecko EUR direct disponible");
    }
  } finally {
    ...
    atlasRenderMarketAccessNotice();
    if (refreshed) atlasAfterLivecheck();
    else { renderAutoReader(); scheduleAutoRead(ATLAS_MARKET_REFRESH_MS); }
  }
}
...
function whyDecision(c) {
  ...
  if (typeof c.change24h === "number") bits.push(`Variation marché 24 h : ${fmtPct(c.change24h)}.`);
...
function atlasComparisonIds() {
  ...
  const primary = validIds.has(state.selectedCoinId) ? state.selectedCoinId : ids[0] || state.coins?.[0]?.id || null;
  if (primary && !ids.includes(primary)) ids.unshift(primary);
  if (!ids.length && primary) ids.push(primary);
...
function atlasSetComparisonIds(ids, primaryId = null) {
  const validIds = new Set((state.coins || []).map(coin => coin.id));
  const unique = [];
  for (const id of ids || []) {
  ...
  const primary = validIds.has(primaryId) ? primaryId : validIds.has(state.selectedCoinId) ? state.selectedCoinId : unique[0] || state.coins?.[0]?.id || null;
  if (primary) {
    const index = unique.indexOf(primary);
    if (index >= 0) unique.splice(index, 1);
    unique.unshift(primary);
  }
  if (!unique.length && primary) unique.push(primary);
...
function atlasRenderComparisonControls() {
  ...
  if (els.btnChartSolo) els.btnChartSolo.classList.toggle("active", !active);
  if (els.btnChartTop3) els.btnChartTop3.classList.toggle("active", active && coins.length === 3);
  if (els.btnChartTop5) els.btnChartTop5.classList.toggle("active", active && coins.length === 5);
  if (!els.comparisonSelection) return;
  if (!coins.length) {
    els.comparisonSelection.innerHTML = '<span class="compare-hint">Livecheck requis avant comparaison</span>';
    return;
  }
  const chips = coins.map((coin, index) => `
    <button class="compare-chip ${index === 0 ? "is-primary" : ""}" type="button" data-compare-primary="${escapeHtml(coin.id)}" title="${index === 0 ? "Actif principal" : "Définir comme actif principal"}">
      <span>${escapeHtml(coin.symbol)}</span><small>${index === 0 ? "principal" : "comparé"}</small>
    </button>`).join("");
  const hint = active
    ? `<span class="compare-hint">Base 100 · clique une ligne pour ajouter ou retirer · ${coins.length}/${ATLAS_COMPARISON_MAX_SERIES}</span>`
    : '<span class="compare-hint">Clique une autre ligne du MARKET SNAPSHOT pour l’ajouter au graphe</span>';
  ...
  els.comparisonSelection.querySelectorAll("[data-compare-primary]").forEach(button => {
    button.addEventListener("click", event => {
      ...
      atlasSetComparisonIds([id, ...ids], id);
      atlasBrokerSeedSpot(coin);
      renderScore(coin);
      renderMarketTable();
      renderDecisionBoard();
...
function atlasResetComparison(coin = getSelectedCoin()) {
  if (!coin?.id) return;
  atlasSetComparisonIds([coin.id], coin.id);
...
function atlasSelectTopComparison(limit = 3) {
  ...
  const current = getSelectedCoin();
  const ids = current && top.some(coin => coin.id === current.id)
    ? [current.id, ...top.filter(coin => coin.id !== current.id).map(coin => coin.id)]
    : top.map(coin => coin.id);
  atlasSetComparisonIds(ids, ids[0]);
...
function atlasToggleComparisonCoin(coin) {
  ...
  if (!ids.length) ids = [current.id];
  if (coin.id === current.id && ids.length === 1) {
    atlasRenderComparisonControls();
    return;
  }
  if (ids.includes(coin.id)) {
    if (coin.id === current.id) return;
  ...
  atlasTrackAudience("chart_comparison_changed", { ids, period: Number(state.chartPeriodDays || 1) });
  atlasSetComparisonIds(ids, current.id);
  renderMarketTable();
  renderDecisionBoard();
  renderMultiHorizon();
  requestAnimationFrame(() => { void renderAnalystPanel({ comparisonToggle: true }); });
}
...
function renderMultiHorizon() {
  ...
  if (!state.liveOk || !primary) {
    ...
    setText(els.multiHorizonStatus, "Livecheck requis");
    els.multiHorizonStatus.className = "pill warn";
    setText(els.multiHorizonSummary, "Atlas expliquera ensemble les mouvements 24 h, 7 jours et 30 jours sans produire de conseil financier.");
    for (const [valueEl, labelEl] of [[els.multiHorizon24Value, els.multiHorizon24Label], [els.multiHorizon7Value, els.multiHorizon7Label], [els.multiHorizon30Value, els.multiHorizon30Label]]) {
      setText(valueEl, "—"); setText(labelEl, "En attente");
    }
    setHTML(els.multiHorizonMeta, "<span>Spot : —</span><span>Écart courbe : —</span><span>Source : —</span>");
    setText(els.multiHorizonTechnical, "Les détails apparaîtront après le Livecheck.");
    return;
  }

  const activeComparison = coins.length > 1;
  const horizons = [
    ["change24h", els.multiHorizon24Value, els.multiHorizon24Label, "24 h"],
    ["change7d", els.multiHorizon7Value, els.multiHorizon7Label, "7 j"],
    ["change30d", els.multiHorizon30Value, els.multiHorizon30Label, "30 j"]
  ];
  ...
  if (activeComparison) {
    const best24 = atlasBestComparisonCoin(coins, "change24h");
    const best7 = atlasBestComparisonCoin(coins, "change7d");
    const best30 = atlasBestComparisonCoin(coins, "change30d");
    setText(els.multiHorizonSummary, `Sur 24 heures, ${best24?.symbol || "—"} résiste le mieux. Sur 7 jours, ${best7?.symbol || "—"} mène la comparaison. Sur 30 jours, ${best30?.symbol || "—"} possède la progression la plus forte parmi les actifs sélectionnés. Le graphe Base 100 compare les trajectoires, pas les prix absolus.`);
    for (const [field, valueEl, labelEl, periodLabel] of horizons) {
      const best = atlasBestComparisonCoin(coins, field);
      setText(valueEl, best ? `${best.symbol} ${fmtPct(atlasFiniteMetric(best[field]))}` : "—");
      setText(labelEl, best ? `Meilleur sur ${periodLabel}` : "Donnée manquante");
    }
    setHTML(els.multiHorizonMeta, `<span>Mode : Base 100</span><span>Actifs : ${escapeHtml(coins.map(coin => coin.symbol).join(" · "))}</span><span>Source : CoinGecko</span>`);
    const rows = coins.map(coin => `${coin.symbol} · 24 h ${fmtPct(coin.change24h)} · 7 j ${fmtPct(coin.change7d)} · 30 j ${fmtPct(coin.change30d)}`);
    setText(els.multiHorizonTechnical, `Comparaison déterministe des variations du snapshot marché. ${rows.join(" | ")}. Les prix absolus ne sont pas additionnés ni moyennés.`);
    return;
  }

  setText(els.multiHorizonSummary, atlasMovementSentence(primary));
  for (const [field, valueEl, labelEl] of horizons) {
    const value = atlasFiniteMetric(primary[field]);
    const classification = atlasClassifyHorizonChange(value);
    setText(valueEl, value !== null ? fmtPct(value) : "—");
    setText(labelEl, classification.label);
  ...
  setHTML(els.multiHorizonMeta, `<span>Spot : ${escapeHtml(atlasFormatEUR(primary.priceEur ?? primary.price))}</span><span>Écart courbe : ${escapeHtml(gapLabel)} · ${escapeHtml(gapQuality)}</span><span>Source : ${escapeHtml(state.mainSource || "CoinGecko")}</span>`);
  const result = state.dataBroker.chart?.result;
  const metrics = result?.integrity?.metrics || {};
  const technical = [
    `24 h ${fmtPct(primary.change24h)}`,
    `7 j ${fmtPct(primary.change7d)}`,
    `30 j ${fmtPct(primary.change30d)}`,
    Number.isFinite(metrics.firstPrice) ? `ouverture série active ${atlasFormatEUR(metrics.firstPrice)}` : null,
    Number.isFinite(metrics.lastPrice) ? `dernier point ${atlasFormatEUR(metrics.lastPrice)}` : null,
    Number.isFinite(metrics.minPrice) ? `minimum ${atlasFormatEUR(metrics.minPrice)}` : null,
    Number.isFinite(metrics.maxPrice) ? `maximum ${atlasFormatEUR(metrics.maxPrice)}` : null,
    Number.isFinite(metrics.pointCount) ? `${metrics.pointCount} points` : null,
    result?.source || null
  ].filter(Boolean);
  setText(els.multiHorizonTechnical, technical.join(" · "));
}
...
const ATLAS_CHART_SELECTION_AUTO_RETRIES = 2;
const ATLAS_CHART_SELECTION_RETRY_DELAYS_MS = [1800, 5200];
  ...
  try {
    ...
    if (stored) {
      return {
        ...stored,
        blocked: false,
        refreshWarning: "Actualisation directe indisponible. Dernière série CoinGecko directe conservée dans ce navigateur.",
...
const ATLAS_DETAIL_PANEL_STORAGE_KEY = "agent_crypto_erith_ia_detail_panel_collapsed_v1";

const ATLAS_LUXE_CHART_PLUGIN = {
  id: "atlasLuxeMarketLens",
  beforeDatasetDraw(chart, args) {
    const dataset = chart?.data?.datasets?.[args.index] || {};
    const ctx = chart.ctx;
    ctx.save();
    ctx.shadowBlur = args.index === 0 ? 18 : 10;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;
    ctx.shadowColor = typeof dataset.borderColor === "string" ? dataset.borderColor : "rgba(112,244,255,.78)";
  },
  ...
  beforeDatasetsDraw(chart, _args, options) {
    ...
    if (!Number.isFinite(baseline) || !scale || !area) return;
    ...
    ctx.setLineDash([6, 7]);
    ctx.lineWidth = 1;
    ctx.strokeStyle = "rgba(255,216,136,.34)";
    ctx.beginPath();
    ctx.moveTo(area.left, y);
    ctx.lineTo(area.right, y);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = "rgba(255,232,174,.78)";
    ctx.font = "800 10px system-ui, sans-serif";
    ctx.textAlign = "left";
    ctx.textBaseline = "bottom";
    ctx.fillText(options?.baselineLabel || "Ouverture", area.left + 8, y - 5);
    ctx.restore();
  },
  afterDatasetsDraw(chart, _args, options) {
    if (!options?.showLastPrice) return;
    const meta = chart.getDatasetMeta(0);
    const element = meta?.data?.[meta.data.length - 1];
    const area = chart?.chartArea;
    const text = String(options?.lastLabel || "");
    if (!element || !area || !text) return;
    const ctx = chart.ctx;
    ctx.save();
    ...
    const boxH = 25;
    const x = Math.min(area.right - boxW - 4, Math.max(area.left + 4, element.x + 12));
    const y = Math.min(area.bottom - boxH - 4, Math.max(area.top + 4, element.y - boxH / 2));
    const radius = 9;
    ctx.beginPath();
    if (typeof ctx.roundRect === "function") ctx.roundRect(x, y, boxW, boxH, radius);
    else {
      ctx.moveTo(x + radius, y); ctx.lineTo(x + boxW - radius, y); ctx.quadraticCurveTo(x + boxW, y, x + boxW, y + radius);
      ctx.lineTo(x + boxW, y + boxH - radius); ctx.quadraticCurveTo(x + boxW, y + boxH, x + boxW - radius, y + boxH);
      ctx.lineTo(x + radius, y + boxH); ctx.quadraticCurveTo(x, y + boxH, x, y + boxH - radius);
      ctx.lineTo(x, y + radius); ctx.quadraticCurveTo(x, y, x + radius, y);
    }
    ctx.fillStyle = "rgba(4,18,29,.94)";
    ctx.strokeStyle = "rgba(112,244,255,.72)";
    ctx.lineWidth = 1.2;
    ctx.shadowColor = "rgba(112,244,255,.35)";
    ctx.shadowBlur = 14;
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.stroke();
  ...
  afterDraw(chart) {
    ...
    const ctx = chart.ctx;
    ctx.save();
    ctx.strokeStyle = "rgba(255,236,191,.30)";
    ctx.lineWidth = 1;
    ctx.setLineDash([3, 5]);
    ctx.beginPath();
...
function atlasSetAnalystChange(value, prefix = "24 h") {
  if (!els.analystChange24) return;
  const numeric = Number(value);
  els.analystChange24.textContent = Number.isFinite(numeric) ? `${prefix} ${fmtPct(numeric)}` : `${prefix} —`;
  els.analystChange24.classList.remove("positive", "negative", "neutral");
  els.analystChange24.classList.add(!Number.isFinite(numeric) ? "neutral" : numeric > 0.0001 ? "positive" : numeric < -0.0001 ? "negative" : "neutral");
}

function atlasUpdateAnalystHero(coin, mode = "single", entries = []) {
  if (mode === "comparison") {
    const symbols = entries.map(entry => entry?.coin?.symbol).filter(Boolean);
    if (els.analystPriceEur) els.analystPriceEur.textContent = "BASE 100";
    if (els.analystPriceUsd) els.analystPriceUsd.textContent = `${symbols.length} actifs · prix réels au survol`;
    const ranked = entries.map(entry => Number(entry?.result?.integrity?.metrics?.changePct)).filter(Number.isFinite);
    const spread = ranked.length ? Math.max(...ranked) - Math.min(...ranked) : null;
    atlasSetAnalystChange(spread, "Dispersion");
    if (els.analystSourceMeta) els.analystSourceMeta.textContent = `Comparaison normalisée · ${symbols.join(" · ")} · CoinGecko EUR direct`;
    return;
  }
  if (!coin) {
    if (els.analystPriceEur) els.analystPriceEur.textContent = "—";
    if (els.analystPriceUsd) els.analystPriceUsd.textContent = "USD —";
    atlasSetAnalystChange(null);
    if (els.analystSourceMeta) els.analystSourceMeta.textContent = "Livecheck requis";
    return;
  }
  const spot = atlasSelectedSpotFor(coin);
  const eur = Number.isFinite(Number(spot?.eur)) ? Number(spot.eur) : Number(coin?.priceEur ?? coin?.price);
  const usd = Number.isFinite(Number(spot?.usd)) ? Number(spot.usd) : Number(coin?.priceUsd);
  if (els.analystPriceEur) els.analystPriceEur.textContent = Number.isFinite(eur) ? atlasFormatEUR(eur) : "EUR —";
  if (els.analystPriceUsd) els.analystPriceUsd.textContent = Number.isFinite(usd) ? atlasFormatUSD(usd) : "USD non fourni";
  atlasSetAnalystChange(coin?.change24h);
  const stamp = spot?.timestamp || coin?.lastUpdated || state.timestamp || null;
  const freshness = stamp ? new Date(stamp).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "heure —";
  if (els.analystSourceMeta) els.analystSourceMeta.textContent = `CoinGecko direct · ${freshness} · EUR historique verrouillé`;
}

function atlasUpdateLuxeIntegrity(c, period, result, mode) {
  if (!els.luxeIntegrityCopy) return;
  if (mode === "blocked") {
    els.luxeIntegrityCopy.textContent = "Série indisponible : aucune courbe n’est inventée. Le spot et le marché restent indépendants.";
    return;
  }
  if (mode !== "valid" || !result?.integrity?.metrics) {
    els.luxeIntegrityCopy.textContent = "Atlas vérifie l’actif, la devise EUR, la période, la couverture et le dernier point avant validation.";
    return;
  }
  const metrics = result.integrity.metrics;
  const pointCount = Number.isFinite(metrics.pointCount) ? metrics.pointCount : "—";
  const gap = Number.isFinite(metrics.priceGapPct) ? `${metrics.priceGapPct.toFixed(2)} %` : "calcul spot/courbe disponible dans la lecture";
  els.luxeIntegrityCopy.textContent = `${c?.symbol || "Actif"} · ${atlasChartPeriodLabel(period)} · ${pointCount} points réels · CoinGecko EUR · écart ${gap} · intégrité validée.`;
}

function atlasSetDetailPanelCollapsed(collapsed, persist = true) {
  const deck = document.getElementById("analyste");
  if (!deck) return;
  const isCollapsed = !!collapsed;
  deck.classList.toggle("detail-collapsed", isCollapsed);
  els.detailPanelToggle?.setAttribute("aria-expanded", String(!isCollapsed));
  if (els.detailPanelToggleLabel) els.detailPanelToggleLabel.textContent = isCollapsed ? "Afficher ▶" : "Réduire ◀";
  if (persist) {
    try { localStorage.setItem(ATLAS_DETAIL_PANEL_STORAGE_KEY, isCollapsed ? "1" : "0"); } catch {}
  }
  const resize = () => {
...
function initAtlasDetailPanel() {
  let collapsed = false;
  try { collapsed = localStorage.getItem(ATLAS_DETAIL_PANEL_STORAGE_KEY) === "1"; } catch {}
  atlasSetDetailPanelCollapsed(collapsed, false);
  els.detailPanelToggle?.addEventListener("click", () => atlasSetDetailPanelCollapsed(!document.getElementById("analyste")?.classList.contains("detail-collapsed")));
  els.detailPanelRail?.addEventListener("click", () => atlasSetDetailPanelCollapsed(false));
}
...
function drawLineChart(canvas, series, label = "", result = {}, chartKey = "") {
  ...
  const sourceMode = result?.sourceMode === "browser-cache" ? "historique navigateur" : "direct";
  ...
  if (window.Chart) {
    ...
    state.chartEngineV2.realChart = new Chart(ctx, {
      type: "line",
      data: { datasets: [{ label: `${label} · CoinGecko EUR`, data, parsing: false, borderWidth: 2.9, pointRadius: 0, pointHoverRadius: 4.5, pointHoverBorderWidth: 2, pointHoverBackgroundColor: "#fff4d4", pointHoverBorderColor: "rgba(112,244,255,.95)", pointHitRadius: 18, tension: 0.06, fill: true, backgroundColor: gradient, borderColor: "rgba(112,244,255,0.98)", borderCapStyle: "round", borderJoinStyle: "round", segment: { borderColor(context) { const current = Number(context?.p1?.parsed?.y); return Number.isFinite(current) && current < first ? "rgba(255,123,151,.96)" : "rgba(112,244,255,.98)"; } } }] },
      options: {
        responsive: true, maintainAspectRatio: false, animation: { duration: 280, easing: "easeOutQuart" }, normalized: true, parsing: false,
        interaction: { mode: "nearest", intersect: false, axis: "x" },
        layout: { padding: { top: 42, right: 28, bottom: 24, left: 12 } },
        plugins: {
          atlasLuxeMarketLens: { baseline: first, baselineLabel: "Ouverture", showLastPrice: true, lastLabel: atlasFormatEUR(last) },
          legend: { display: false },
          tooltip: {
            enabled: true, displayColors: false, backgroundColor: "rgba(2,10,19,0.96)", titleColor: "#fff0c8", bodyColor: "#eefaff", borderColor: "rgba(112,244,255,0.48)", borderWidth: 1, padding: 12, cornerRadius: 10, caretPadding: 12, titleFont: { size: 12, weight: "900" }, bodyFont: { size: 12, weight: "700" },
            callbacks: {
              title(items) { return atlasChartLabelFull(items?.[0]?.parsed?.x || Date.now()); },
              label(item) { return `Prix EUR · ${atlasFormatEUR(item.parsed.y)}`; },
              afterLabel(item) { const pct = first ? (Number(item.parsed.y) / first - 1) * 100 : 0; return `${pct >= 0 ? "+" : ""}${pct.toFixed(2)} % depuis l’ouverture`; },
              afterBody() { return `CoinGecko market_chart EUR · ${sourceMode}`; }
            }
          }
        },
        scales: {
          x: {
            ...
            grid: { color: "rgba(176,236,255,0.055)", drawTicks: false },
            border: { color: "rgba(176,236,255,.14)" },
            ticks: { color: "rgba(234,247,255,0.90)", maxTicksLimit: period === 1 ? 9 : 10, autoSkip: true, maxRotation: 0, font: { size: 12, weight: "800" }, callback(value) { return atlasChartLabelForTime(Number(value), period); } }
          },
          y: {
            position: "right", min: min - yPad, max: max + yPad,
            grid: { color: "rgba(176,236,255,0.070)" },
            border: { color: "rgba(176,236,255,.14)" },
            ticks: { color: "rgba(241,250,255,0.94)", maxTicksLimit: 7, callback(value) { return atlasFormatEUR(Number(value)); }, font: { size: 12, weight: "850" }, padding: 10 }
          }
        }
      },
      plugins: [ATLAS_LUXE_CHART_PLUGIN]
    });
    return;
  }
  const ctx = canvas.getContext("2d");
...
function drawComparisonChart(canvas, entries, period, chartKey = "") {
  ...
  const normalizedEntries = entries.map((entry, index) => {
    const rows = (entry.result?.series || []).map(point => ({ t: Number(point[0]), price: Number(point[1]) })).filter(point => Number.isFinite(point.t) && Number.isFinite(point.price) && point.price > 0).sort((a, b) => a.t - b.t);
    const first = rows[0]?.price;
    const data = rows.map(row => ({ x: row.t, y: first ? row.price / first * 100 : 100, rawPrice: row.price }));
    return { ...entry, index, rows, data, first };
  }).filter(entry => entry.rows.length >= atlasChartRules(period).minPoints && Number.isFinite(entry.first));
  ...
  const startTime = Math.min(...normalizedEntries.map(entry => entry.rows[0].t));
  const endTime = Math.max(...normalizedEntries.map(entry => entry.rows[entry.rows.length - 1].t));
  const summary = `Base 100 · ${atlasComparisonSummary(normalizedEntries)} · ${normalizedEntries.length} actifs · CoinGecko EUR`;
  atlasDestroyRealChart();
  atlasSetChartShellState(canvas, "valid", summary, "fresh");
  state.chartEngineV2.lastRenderedKey = chartKey;
  ...
  if (window.Chart) {
    ...
    state.chartEngineV2.realChart = new Chart(ctx, {
      ...
      options: {
        responsive: true, maintainAspectRatio: false, animation: { duration: 280, easing: "easeOutQuart" }, normalized: true, parsing: false,
        interaction: { mode: "nearest", intersect: false, axis: "x" },
        layout: { padding: { top: 60, right: 28, bottom: 24, left: 12 } },
        plugins: {
          atlasLuxeMarketLens: { baseline: 100, baselineLabel: "Base 100", showLastPrice: false },
          legend: { display: true, position: "top", align: "end", labels: { color: "rgba(240,250,255,0.94)", usePointStyle: true, pointStyle: "line", boxWidth: 28, padding: 16, font: { size: 11, weight: "850" } } },
          tooltip: {
            enabled: true, displayColors: true, backgroundColor: "rgba(2,10,19,0.96)", titleColor: "#fff0c8", bodyColor: "#eefaff", borderColor: "rgba(112,244,255,0.48)", borderWidth: 1, padding: 12, cornerRadius: 10, caretPadding: 12, titleFont: { size: 12, weight: "900" }, bodyFont: { size: 12, weight: "700" },
            callbacks: {
              title(items) { return atlasChartLabelFull(items?.[0]?.parsed?.x || Date.now()); },
              label(item) {
                const rawPrice = Number(item?.raw?.rawPrice);
                const relative = Number(item.parsed.y) - 100; return `${item.dataset.label} · ${Number(item.parsed.y).toFixed(2)} · ${relative >= 0 ? "+" : ""}${relative.toFixed(2)} % · ${Number.isFinite(rawPrice) ? atlasFormatEUR(rawPrice) : "prix —"}`;
              },
              afterBody() { return "Comparaison normalisée : chaque série commence à 100."; }
            }
          }
        },
        scales: {
          x: {
            type: "linear", min: startTime, max: endTime,
            grid: { color: "rgba(176,236,255,0.075)", drawTicks: true },
            ticks: { color: "rgba(226,244,255,0.84)", maxTicksLimit: Number(period) === 1 ? 9 : 10, autoSkip: true, maxRotation: 0, font: { size: 11, weight: "700" }, callback(value) { return atlasChartLabelForTime(Number(value), period); } }
          },
          y: {
            position: "right", min: min - yPad, max: max + yPad,
            grid: { color: "rgba(176,236,255,0.105)" },
            ticks: { color: "rgba(226,244,255,0.86)", maxTicksLimit: 7, callback(value) { return Number(value).toFixed(1); }, font: { size: 11, weight: "700" }, padding: 8 },
            title: { display: true, text: "Base 100", color: "rgba(255,240,200,0.78)", font: { size: 10, weight: "800" } }
          }
        }
      },
      plugins: [ATLAS_LUXE_CHART_PLUGIN]
  ...
  canvas.width = Math.floor(width * dpr); canvas.height = Math.floor(height * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, width, height);
  const left = 52, right = 100, top = 60, bottom = 42;
  const plotW = width - left - right, plotH = height - top - bottom;
  const xFor = time => left + ((time - startTime) / (endTime - startTime || 1)) * plotW;
  const yFor = value => top + plotH - ((value - (min - yPad)) / ((max + yPad) - (min - yPad) || 1)) * plotH;
  normalizedEntries.forEach((entry, index) => {
    ctx.beginPath();
    entry.data.forEach((point, pointIndex) => { const x = xFor(point.x), y = yFor(point.y); if (pointIndex === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); });
    ctx.strokeStyle = ATLAS_COMPARISON_COLORS[index % ATLAS_COMPARISON_COLORS.length]; ctx.lineWidth = index === 0 ? 2.8 : 2.2; ctx.stroke();
  });
}
...
function atlasRenderComparisonDetail(entries, period) {
  ...
  if (els.assetDetailGrid) {
    els.assetDetailGrid.innerHTML = `
      <div class="detail-hero-cell"><b>Mode</b><span>Comparaison Base 100</span></div>
      <div class="detail-hero-cell"><b>Principal</b><span>${escapeHtml(primary?.name || "—")} · ${escapeHtml(primary?.symbol || "—")}</span></div>
      <div class="detail-wide-cell"><b>Actifs</b><span>${escapeHtml(coins.map(coin => coin.symbol).join(" · "))}</span></div>
      <div><b>Période</b><span>${escapeHtml(atlasChartPeriodLabel(period))}</span></div>
      <div><b>Meilleure</b><span>${ranked[0] ? `${escapeHtml(ranked[0].coin.symbol)} · ${fmtPct(ranked[0].change)}` : "—"}</span></div>
      <div><b>Plus faible</b><span>${ranked.at(-1) ? `${escapeHtml(ranked.at(-1).coin.symbol)} · ${fmtPct(ranked.at(-1).change)}` : "—"}</span></div>
      <div><b>Échelle</b><span>Chaque actif commence à 100</span></div>
      <div class="detail-decision-cell"><b>Décision</b><span>Observation comparative uniquement</span></div>`;
  }
  if (els.assetDetailWhy) els.assetDetailWhy.textContent = "La normalisation Base 100 rend comparables des actifs dont les prix absolus sont très différents. Elle mesure la trajectoire relative, sans classer la qualité d’investissement.";
  atlasUpdateAnalystHero(primary, "comparison", entries);
  if (els.luxeIntegrityCopy) els.luxeIntegrityCopy.textContent = `${entries.length} séries réelles CoinGecko EUR · normalisation Base 100 · prix absolus conservés dans les infobulles.`;
  atlasRenderBrokerStrip();
}
...
async function renderComparisonAnalystPanel(options = {}) {
  ...
  const fetched = await atlasMapComparisonWithConcurrency(coins, 2, async coin => {
    const stored = atlasGetStoredChartResult(coin, period);
    const result = await fetchChartSeries(coin, period, { signal: controller.signal, fallback: stored });
    if (!result?.blocked && Array.isArray(result?.series) && result.series.length) atlasStoreChartResult(coin, period, result);
    return { coin, result };
  });
  if (renderToken !== state.comparisonRenderToken || controller.signal.aborted || !atlasComparisonActive()) return;
  const entries = fetched.filter(item => item?.coin && item?.result && !item.result.blocked && Array.isArray(item.result.series) && item.result.series.length);
  const failures = fetched.filter(item => item?.error || item?.result?.blocked);
  if (entries.length < 2) {
    ...
    drawComparisonChart(els.mainChart, entries, period, chartKey);
    const latestTimestamp = Math.max(...entries.map(entry => Number(entry.result?.integrity?.metrics?.lastTimestamp || 0)));
    const pointCount = entries.reduce((sum, entry) => sum + Number(entry.result?.series?.length || 0), 0);
    const comparisonResult = { comparison: true, entries, periodDays: period, sourceMode: "comparison-base100", source: "CoinGecko market_chart EUR · Base 100" };
    ...
    state.dataBroker.chart = { status: "ready", coinId: state.selectedCoinId, period, source: ATLAS_CANONICAL_MARKET_SOURCE, mode: "comparison-base100", timestamp: Number.isFinite(latestTimestamp) && latestTimestamp > 0 ? new Date(latestTimestamp).toISOString() : null, pointCount, latencyMs: null, retryCount: 0, result: comparisonResult, error: failures.length ? `${failures.length} série(s) indisponible(s)` : null };
    atlasRenderComparisonDetail(entries, period);
    if (els.chartCaption) els.chartCaption.textContent = `Comparaison ${periodLabel} · Base 100 · ${atlasComparisonSummary(entries)} · ${entries.length}/${coins.length} séries valides · source CoinGecko EUR${failures.length ? ` · ${failures.length} indisponible(s)` : ""}.`;
...
function atlasRenderAssetDetail(c, period, result = null, mode = "loading") {
  ...
  if (els.assetDetailGrid) {
    els.assetDetailGrid.innerHTML = `
      <div class="detail-hero-cell"><b>Actif</b><span>${escapeHtml(c.name)} · ${escapeHtml(c.symbol)}</span></div>
      <div class="detail-hero-cell"><b>Type</b><span>${escapeHtml(classifyAsset(c))}</span></div>
      <div class="detail-price-cell"><b>Prix EUR</b><span>${atlasFormatEUR(spotEur)}</span></div>
      <div class="detail-price-cell"><b>Prix USD</b><span>${spotUsd ? atlasFormatUSD(spotUsd) : "Non fourni"}</span></div>
      <div><b>24 h</b><span class="${clsPct(c?.change24h)}">${Number.isFinite(Number(c?.change24h)) ? fmtPct(Number(c.change24h)) : "—"}</span></div>
      <div><b>7 jours</b><span class="${clsPct(c?.change7d)}">${Number.isFinite(Number(c?.change7d)) ? fmtPct(Number(c.change7d)) : "—"}</span></div>
      <div><b>30 jours</b><span class="${clsPct(c?.change30d)}">${Number.isFinite(Number(c?.change30d)) ? fmtPct(Number(c.change30d)) : "—"}</span></div>
      <div><b>Vol/Cap</b><span>${ratio}</span></div>
      <div class="detail-decision-cell"><b>Décision</b><span>${escapeHtml(beginnerDecision(c))}</span></div>
      <div><b>Série ${periodLabel}</b><span>${Number.isFinite(metrics.changePct) ? fmtPct(metrics.changePct) : "—"}</span></div>`;
  }
  atlasUpdateAnalystHero(c, "single");
  atlasUpdateLuxeIntegrity(c, period, result, mode);
...
function atlasPrepareChartSelection(coin, period = 1, options = {}) {
  ...
  if (!options.preserveComparison) atlasSetComparisonIds([coin.id], coin.id);
...
function atlasSelectMarketCoin(coin) {
  ...
  atlasPrepareChartSelection(coin, 1);
  renderScore(coin);
  renderMarketTable();
  renderDecisionBoard();
  renderMultiHorizon();
...
} function situationPayload() { return { version: "V1.1-alpha.26.46.1", active_now: [ "public_market_observation", "charts", "source_diagnostic", "human_readable_tests", "local_paper_trading", "solo_beginner_profile_100_eur", "briefing_questions" ], prepared_only: [ "private_backend", "remote_access", "kraken_read_only", "physical_security_layer" ], locked: [ "real_wallet_connection", "private_key", "withdraw_key", "real_order", "automatic_trading" ], current_step: "collect_information_before_private_backend" };
...
/* =========================================================
   V1.1-alpha.26.46.1 — Atlas Decision Board
...
function decisionFromScore(score) { if (score === null || score === undefined) return "Analyse suspendue"; if (score <= 40) return "Données fragiles"; if (score <= 55) return "Lecture prudente"; if (score <= 65) return "Mouvement modéré"; if (score <= 75) return "Mouvement marqué"; return "Volatilité élevée";
...
} function renderRiskGrid() { if (!els.riskGrid) return; els.riskGrid.innerHTML = ` <div class="risk ${state.liveOk ? "ok" : "wait"}"><span>Marché</span><b>${state.sourceLock?.valid ? "Source Lock CoinGecko" : "Source canonique absente"}</b></div> <div class="risk warn"><span>Sécurité</span><b>Non vérifiée V1.1-alpha.26.46.1</b></div> <div class="risk warn"><span>Social</span><b>Non vérifié</b></div> <div class="risk warn"><span>On-chain</span><b>Non vérifié</b></div>`;
  ...
  return [
    ...
    `État : ${tone.label}.`,
    `Snapshot marché : CoinGecko Top 50 EUR ; USD en enrichissement optionnel · ${new Date(state.timestamp).toLocaleString("fr-FR")}.`,
...
} function simulationDataSnapshot() { if (!state.sim) loadSimulation(); const totals = getSimulationTotals(); return { generated_at: new Date().toISOString(), version: "V1.1-alpha.26.46.1", public_only: true, warning: "Données publiques et simulation locale uniquement. Aucun compte réel, aucune clé API, aucun wallet.", profile: getSimulationProfileStatus(), simulation: simulationPayload(), totals: { cash_eur: state.sim.cash, positions_value_eur: totals.positionsValue, total_value_eur: totals.total, pnl_eur: totals.pnl }, market_snapshot: publicMarketSnapshot() };
...
} function makeCollectorRecord() { const snapshot = simulationDataSnapshot(); const logs = snapshot?.simulation?.logs || []; const marketAssets = snapshot?.market_snapshot?.assets || []; return { id: `snapshot_${Date.now()}`, saved_at: new Date().toISOString(), version: "V1.1-alpha.26.46.1", public_only: true, source: snapshot?.market_snapshot?.source || "source live", live_ok: !!snapshot?.market_snapshot?.live_ok, symbols: marketAssets.map(a => a.symbol), learning_tags: collectorRecordReasonFromLogs(logs), snapshot };
...
} function downloadCollectorJSON() { const records = readCollectorMemory(); const payload = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.46.1", public_only: true, warning: "Export mémoire locale public-compatible. Aucun compte réel, aucune clé API, aucun wallet.", count: records.length, records }; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.json`, "application/json", JSON.stringify(payload, null, 2) ); showCollectorMemory();
} function downloadCollectorJSONL() { const records = readCollectorMemory(); const header = { exported_at: new Date().toISOString(), version: "V1.1-alpha.26.46.1", public_only: true, type: "agent_crypto_collector_memory_jsonl_header" }; const lines = [JSON.stringify(header), ...records.map(r => JSON.stringify(r))]; downloadTextFile( `agent_crypto_collector_memory_${new Date().toISOString().slice(0, 10)}.jsonl`, "application/x-ndjson", lines.join("\n") ); showCollectorMemory();
...
} function buildMemoryReportMarkdown() { const records = readCollectorMemory(); const lines = [ "# RAPPORT MÉMOIRE LOCALE — Agent-Crypto @erith.IA", "", "Version : V1.1-alpha.26.46.1", `Date : ${new Date().toISOString()}`, "", "## Statut sécurité", "", "- Mémoire locale navigateur uniquement.", "- Données public-compatible.", "- Aucun compte réel.", "- Aucune clé API.", "- Aucun wallet.", "- Aucun ordre réel.", "", "## Lecture mémoire", "", exploreMemoryText(records), "", "## Comparaison premier / dernier", "", compareMemoryText(records), "", "## Refus de sécurité", "", refusalSummaryText(records), "", "## Conclusion", "", "L’explorateur transforme les snapshots en lecture pédagogique. La prochaine vraie étape sera de déplacer ce principe vers une base locale plus solide sur PC Ryzen 7." ]; return lines.join("\n");
...
} /* ========================================================= V1.1-alpha.26.46.1 — Atlas Auto Reader Ouverture page -> Livecheck auto -> snapshots -> lecture marché. ========================================================= */ const AUTO_MEMORY_KEY = "agent_crypto_erith_ia_auto_reader_v1_1_alpha_13";
...
} function makeAutoSnapshot() { const collectorId = getCollectorId(); const wanted = new Set(state.watchIds || []); const watch = state.coins.filter(c => wanted.has(c.id)); const leaders = state.coins.slice(0, 20); const merged = [...leaders, ...watch].filter((c, i, arr) => arr.findIndex(x => x.id === c.id) === i); const created = new Date().toISOString(); return { id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, snapshot_id: `${collectorId}_${created.replace(/[:.]/g, "-")}`, collector_id: collectorId, collector_type: "local_browser", saved_at: created, version: "V1.1-alpha.26.46.1", source: state.mainSource || null, source_time: state.timestamp || null, live_ok: !!state.liveOk, cadence_ms: state.auto?.intervalMs || ATLAS_MARKET_REFRESH_MS, global: { market_cap_eur: state.global?.total_market_cap?.eur ?? null, volume_24h_eur: state.global?.total_volume?.eur ?? null, btc_dominance_pct: state.global?.market_cap_percentage?.btc ?? null }, assets: merged.map(compactCoinForAuto) };
...
} function startAutoReader() { state.auto.livecheckBusy = false; loadWatchIds(); if (els.autoCadenceSelect) state.auto.cadence = els.autoCadenceSelect.value || "adaptive"; const restored = applyMarketCache("Dernier snapshot CoinGecko restauré pendant la vérification directe."); renderAutoReader(); if (state.auto.countdownTimer) clearInterval(state.auto.countdownTimer); state.auto.countdownTimer = setInterval(updateAutoCountdown, 1000); if (state.auto.enabled) { setTimeout(() => runLivecheck(), restored ? 1800 : 900); }
...
} function setAutoCadence(value) { state.auto.cadence = String(value || "adaptive"); state.auto.intervalMs = state.auto.cadence === "900" ? 900000 : ATLAS_MARKET_REFRESH_MS; renderAutoReader(); if (state.auto.enabled) scheduleAutoRead(state.auto.intervalMs);
...
$("btnAddWatch")?.addEventListener("click", addWatch); document.querySelectorAll(".period-btn[data-period]").forEach(btn => { btn.addEventListener("click", () => {
  const period = Number(btn.dataset.period) || 1;
  const coin = getSelectedCoin();
  if (!coin) return;
...
els.btnChartSolo?.addEventListener("click", () => atlasResetComparison(getSelectedCoin()));
...
/* =========================================================
   V1.1-alpha.26.46.1 — Audience V2 moteur de session
...
} /* Atlas-10 Crypto — Math Core intégré V1.1-alpha.26.46.1 Source: modules .md Atlas Math. Exécution: traduction JS condensée. Lecture seule : aucun ordre réel, aucune clé API, aucun capital engagé. */
