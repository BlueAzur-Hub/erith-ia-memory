/* Market Core V2.0-Alpha · Build 28.2.9 — SCANNER RECOVERY FULL STACK LOCK · ANALYTICAL TRUTH & EVIDENCE · CLEAN HOME · INLINE DATA STATUS · GRAPH THREE-STATE · TOP5 FLOW PERSISTENCE · ADMIN GRAPH TOGGLE · MARKET RECENTER · FORGE PRO BRIDGE
...
*/
const ATLAS_RELEASE = "Market Core V2.0-Alpha · Build 28.2.9";
...
/* =========================================================
   Market Core V2.0-Alpha · Build 28.2.9
...
function atlasActivateTargetTopFiveCycle() {
  ...
  if (next === "gainers" || next === "losers" || next === "volume") {
    return atlasScannerStart(next, 5, {
      period,
      source: "target-top5-cycle-28.2.9"
    });
  }
...
els.btnChartGainers?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("gainers", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.2.9" });
});
els.btnChartLosers?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("losers", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.2.9" });
});
els.btnChartVolume5?.addEventListener("click", event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  atlasScannerStart("volume", 5, { period: Number(state.chartPeriodDays || 1), source: "button-28.2.9" });
});
...
function atlasSyncReleaseLabels() {
  ...
  setText(
    ...
    `Agent-Crypto @erith.IA · Market Core · Build 28.2.9`
  );
}
...
function atlasScannerCommit(tx, finalEntries, rejected) {
  ...
  try {
    ...
    */
    ...
    console.warn("Transaction scanner 28.2.9 annulée :", error);
...
async function atlasScannerRun(tx) {
  ...
  try {
    ...
    */
    ...
    console.error("Scanner 28.2.9 :", error);
...
function atlasScannerStart(preset = "gainers", limit = 5, options = {}) {
  ...
  try {
    ...
    void atlasScannerRun(tx).catch(error => {
      if (error?.name === "AbortError") return;
      console.error("Scanner 28.2.9 non capturé :", error);
      if (atlasScannerTransaction === tx) atlasScannerTransaction = null;
    ...
    console.error("Démarrage scanner 28.2.9 :", error);
    return atlasScannerVisibleFailure(
      preset,
