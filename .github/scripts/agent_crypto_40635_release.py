...
build_truth = json.loads(BUILD.read_text(encoding='utf-8'))
require(build_truth.get('build') == PARENT, f'parent build drift: {build_truth.get("build")}')
require(build_truth.get('engine') == ENGINE, 'Market Core drift')
...
/*
   Design contract:
   - the paid Observatory artwork is the visible cockpit and owns cards, icons, borders and ornament;
   - live HTML owns only changing information;
   - baked values are covered with small local masks, never with replacement cards;
   - no SVG, no generated image, no second dashboard skin;
   - the 40.6.34 single 16:9 stage remains the sole coordinate system.
*/
@media (min-width:901px){
  /* ---------- GLANCE: retire the replacement-card skin ---------- */
  ...
  /* The artwork already prints these semantic labels. Keep DOM values, not duplicate headings. */
  ...
  /* Base live typography: values sit inside the card faces already painted in the artwork. */
  ...
  #atlasAetherStatusPanel4084 [data-aether-status-grid-406030] article > b{
    ...
    font-size:clamp(10px,.84vw,13.4px)!important;
  ...
  #atlasAetherStatusPanel4084 [data-aether-status-grid-406030] article > small{
    ...
    display:block!important;
    margin:0!important;
    color:#65efc5!important;
    font-size:clamp(8.3px,.64vw,10.2px)!important;
  ...
  /* ---------- EXACT ARTWORK ANCHORS ---------- */
  #atlasAetherStatusPanel4084 .aether-first-glance-grid-406030 > article:has([data-aether-row-4084="convergence"]){left:37.55%!important;top:5.0%!important;width:25.25%!important;height:14.8%!important}
  ...
  /* ---------- LOCAL MASK LANGUAGE ----------
     Each mask covers ONLY baked data, preserving the paid card, iconography and ornament. */
  ...
  #atlasAetherStatusPanel4084 [data-aether-weather-406030]::before{
    ...
    box-shadow:0 0 18px rgba(0,0,0,.18)!important;
    pointer-events:none!important;
  }

  /* Convergence: preserve network icon left and signal bars right. */
  ...
  /* Divergence: preserve violet icon and blue signal bars. */
  ...
  /* System: preserve gear icon and the yellow load motif. */
  ...
  /* Sources: preserve book/check iconography, replace only readiness text. */
  ...
  /* Atlas has baked correlations. Two small masks retire stale values while preserving globe + card shell. */
  ...
  #atlasAetherStatusPanel4084 [data-aether-status-grid-406030] article:has([data-aether-row-4084="atlas"])::after{
    ...
    border-radius:9px!important;background:rgba(2,12,28,.94)!important;box-shadow:0 0 16px rgba(0,0,0,.16)!important;
  }
  ...
  /* Market: mask stale breadth and baked asset-price row separately; keep icon + sparkline. */
  ...
  #atlasAetherStatusPanel4084 .aether-first-glance-grid-406030 > article:has([data-aether-row-4084="market"])::after{
    ...
    background:rgba(2,12,28,.95)!important;box-shadow:0 0 18px rgba(0,0,0,.17)!important;
  }
  ...
  /* Oracle: preserve gold cube + right signal bars; live Oracle occupies the painted text lane. */
  ...
  /* Weather: preserve cloud icon left and rain glyph right; live five-day text owns the center lane. */
  ...
  #atlasAetherStatusPanel4084 [data-aether-weather-406030] > small{left:31%!important;top:70%!important;width:56%!important;height:20%!important;color:#8ddfff!important;font-size:clamp(7.7px,.60vw,9.2px)!important;line-height:1.12!important}

  /* Events: keep violet icon/title; replace the baked sample events with the live session timeline. */
  ...
  #atlasAetherStatusPanel4084 [data-aether-timeline-card-406027]::after{
    content:""!important;position:absolute!important;z-index:2!important;pointer-events:none!important;border-radius:8px!important;
  ...
  /* ---------- CENTER: values only, lotus/rings remain visible ---------- */
  #atlasAetherStatusPanel4084 [data-aether-level-406030]{
    left:50%!important;top:46.25%!important;width:21.5%!important;height:9.8%!important;
    transform:translate(-50%,-50%)!important;padding:0!important;border:0!important;border-radius:0!important;
    background:transparent!important;box-shadow:none!important;overflow:visible!important;text-align:center!important;
  }
  #atlasAetherStatusPanel4084 [data-aether-level-406030]::before{
    content:""!important;position:absolute!important;z-index:2!important;left:10%!important;right:10%!important;top:13%!important;bottom:9%!important;
    border-radius:14px!important;background:linear-gradient(90deg,rgba(2,11,27,.28),rgba(2,11,27,.97) 24%,rgba(2,11,27,.97) 76%,rgba(2,11,27,.28))!important;
    box-shadow:0 0 20px rgba(0,0,0,.18)!important;
  }
  ...
  #atlasAetherStatusPanel4084 [data-aether-level-406030] [data-aether-row-4084="level"]{
    left:11%!important;top:24%!important;width:78%!important;height:63%!important;z-index:4!important;
    display:flex!important;align-items:center!important;justify-content:center!important;text-align:center!important;
    color:#ffd66d!important;font-size:clamp(10px,.90vw,13.8px)!important;line-height:1.13!important;font-weight:880!important;
    text-transform:none!important;text-shadow:0 0 10px rgba(255,200,74,.30),0 1px 2px rgba(0,0,0,.95)!important;
  }

  #atlasAetherStatusPanel4084 [data-aether-action-406030]{
    left:50%!important;top:64.1%!important;width:31.8%!important;height:13.9%!important;transform:translate(-50%,-50%)!important;
    padding:0!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;overflow:visible!important;text-align:center!important;
  }
  #atlasAetherStatusPanel4084 [data-aether-action-406030]::before{
    content:""!important;position:absolute!important;z-index:2!important;left:8%!important;right:8%!important;top:3%!important;bottom:5%!important;
    border-radius:12px!important;background:linear-gradient(90deg,rgba(2,11,27,.24),rgba(2,11,27,.95) 19%,rgba(2,11,27,.95) 81%,rgba(2,11,27,.24))!important;
    box-shadow:0 0 18px rgba(0,0,0,.17)!important;
  }
  #atlasAetherStatusPanel4084 [data-aether-action-406030] > b{
    left:10%!important;top:12%!important;width:80%!important;height:45%!important;z-index:4!important;
    display:flex!important;align-items:center!important;justify-content:center!important;text-align:center!important;
    color:#f4e4a0!important;font-size:clamp(8.8px,.70vw,10.8px)!important;line-height:1.17!important;font-weight:790!important;
  }
  #atlasAetherStatusPanel4084 [data-aether-action-406030] > small{
    position:absolute!important;left:12%!important;top:58%!important;width:76%!important;height:32%!important;z-index:4!important;margin:0!important;
    display:block!important;color:#8eeaff!important;font-size:clamp(7.7px,.60vw,9.2px)!important;line-height:1.12!important;font-weight:720!important;
    text-shadow:0 1px 2px rgba(0,0,0,.95)!important;overflow:hidden!important;
  }

  /* ---------- CONTROLS: disappear into the artwork instead of forming another panel ---------- */
  #atlasAetherStatusPanel4084 [data-aether-close-4084]{
    right:.7%!important;top:1.1%!important;width:29px!important;height:29px!important;
    color:#bdeeff!important;background:rgba(2,12,25,.58)!important;border:1px solid rgba(92,216,255,.38)!important;
    box-shadow:none!important;backdrop-filter:blur(2px)!important;-webkit-backdrop-filter:blur(2px)!important;
    opacity:.76!important;
  }
  #atlasAetherStatusPanel4084 [data-aether-close-4084]:hover,
  #atlasAetherStatusPanel4084 [data-aether-close-4084]:focus-visible{opacity:1!important;background:rgba(2,12,25,.90)!important}

  #atlasAetherStatusPanel4084 .aether-focus-toolbar-406030{
    right:1.35%!important;bottom:1.35%!important;z-index:50!important;
    display:flex!important;justify-content:flex-end!important;padding:0!important;gap:4px!important;
  ...
  #atlasAetherStatusPanel4084 [data-aether-focus-button-406030]{
    min-height:25px!important;padding:4px 8px!important;border-radius:8px!important;
    border:1px solid rgba(91,210,255,.27)!important;background:rgba(2,12,25,.66)!important;color:#d8efff!important;
    font-size:9.5px!important;line-height:1!important;box-shadow:none!important;backdrop-filter:blur(2px)!important;-webkit-backdrop-filter:blur(2px)!important;
  }
  ...
  /* Secondary modes remain interactive and bounded; the embroidery rules apply only to glance. */
  #atlasAetherStatusPanel4084 [data-aether-focus-view-406030="history"],
  #atlasAetherStatusPanel4084 [data-aether-focus-view-406030="details"]{pointer-events:auto!important}
  #atlasAetherStatusPanel4084 [data-aether-focus-view-406030="history"] article,
  #atlasAetherStatusPanel4084 [data-aether-focus-view-406030="details"] article{
    pointer-events:auto!important;border:1px solid rgba(81,210,255,.26)!important;border-radius:12px!important;background:rgba(4,20,38,.82)!important;padding:9px!important;overflow:hidden!important;
  }
}
'''

CSS.write_text(css_text + css_406035.rstrip() + '\n', encoding='utf-8')
...
## Operator intent
Use the paid Observatory artwork as the cockpit itself. Do not recreate its cards in CSS. The live layer must behave like embroidery: only changing values are written over the card faces already present in the background.

## Correction
- Keeps the 40.6.34 single 16:9 stage unchanged.
- Removes the large replacement-card backgrounds, borders, shadows and blur from Glance.
- Hides duplicate DOM headings because the artwork already owns the semantic labels.
- Adds small local dark masks only where baked sample values would conflict with live truth.
- Preserves card shells, icons, lotus, globe, orbital rings and decorative motifs from the artwork.
- Positions live Convergence, Divergence, Market, System, Sources, Atlas, Oracle, Weather and recent Events inside their painted card lanes.
- Keeps the center lotus/rings visible while replacing only the attention value and current watch text.
- Keeps History and Details as bounded interactive overlays.
...
## Firefox acceptance
Ctrl+F5 → Aether Attention → verify that the artwork cards remain visually intact and that only live values appear over them. Check: Convergence, System, Sources, Atlas, Divergence, Market, Events, Oracle, Weather, center Attention, then Historique → Détails → Vue → close.
''', encoding='utf-8')

# Anti-destruction gates.
...
require(MARKER in css_final, '40.6.35 CSS marker missing')
require(PARENT_MARKER in css_final, '40.6.34 stage marker lost')
require('background-first UI'.lower().replace(' ', '') not in '', 'sanity')
index_final = INDEX.read_text(encoding='utf-8')
require('content="40.6.35"' in index_final, 'index build token missing')
require('admin-ribbons.css?v=administrator-build-40.6.35' in index_final, 'CSS cache token missing')
for path in (BUILD, ADMIN_VERSION, VERSION):
    data = json.loads(path.read_text(encoding='utf-8'))
