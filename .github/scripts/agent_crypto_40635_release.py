...
PATCH = Path('.github/scripts/aether_40635_live_embroidery.css')
...
require(PATCH.exists(), 'embroidery patch missing')

css = CSS.read_text(encoding='utf-8').rstrip()
patch = PATCH.read_text(encoding='utf-8').strip()
require(PARENT_MARKER in css, '40.6.34 stage lock missing')
require(MARKER not in css, '40.6.35 marker already present')
require(patch.startswith(MARKER), 'embroidery patch marker mismatch')
CSS.write_text(css + '\n\n' + patch + '\n', encoding='utf-8')
...
## Intent
The Observatory artwork is the cockpit itself. CSS does not redraw its cards. Live HTML is embroidered into the artwork: only changing values replace baked sample values.
