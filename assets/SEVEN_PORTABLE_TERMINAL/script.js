/* =========================================================
   SEVEN PORTABLE TERMINAL — SCRIPT LOCK
   Fix priorité : script.js
   Base : Celestial Atlas 2.9 Golden Glass / Recovery Hero
   Objectif :
   - remplacer le script empilé par un script unique ;
   - supprimer les boots destructeurs ;
   - garder l'Atlas, les backgrounds, le Hero, les options ;
   - ajouter mode Transparent / Lisibilité sombre ;
   - ne pas toucher aux images.
========================================================= */
...
const CELESTIAL_ATLAS_ITEMS = BACKGROUNDS;
...
const STORAGE = {
  ...
  atlasUrl: "seven-atlas-last-url",
  ambiance: "seven-ambiance",
  favorite: "seven-favorite-state-v4",
...
function fixBrokenLinks() {
  $all("a.tile").forEach(link => {
    const label = link.getAttribute("aria-label") || link.textContent.trim();

    if (/ERITH\.IA Auto-Agent/i.test(label)) link.href = ERITH_AUTO_AGENT_LINK;
    if (/ERITH Memory/i.test(label)) link.href = ERITH_MEMORY_LINK;
    if (/GitHub public/i.test(label)) link.href = "https://github.com/BlueAzur-Hub/erith-ia-memory";
    if (/Blue Azur/i.test(label)) link.href = BLUE_AZUR_CHANNEL_LINK;
    if (/Facebook/i.test(label)) link.href = FACEBOOK_SEARCH_LINK;
    if (/X \/ Twitter/i.test(label)) link.href = X_SEARCH_LINK;
  });
}
...
function ensureVisibilityControls() {
  const strip = $(".home-sober-toolbar") || $(".sky-control-strip");
  if (!strip) return;

  let readabilityBtn = byId("readabilityShieldBtn");

  if (!byId("heroFocusBtn")) {
    const hero = document.createElement("button");
    hero.className = "mini-control";
    hero.id = "heroFocusBtn";
    hero.type = "button";
    hero.textContent = "🎯 Hero";
    hero.onclick = () => toggleHeroFocusPanel();
    strip.insertBefore(hero, readabilityBtn || null);
  }

  if (!byId("transparentModeBtn")) {
    const transparent = document.createElement("button");
    transparent.className = "mini-control active";
    transparent.id = "transparentModeBtn";
    transparent.type = "button";
    transparent.textContent = "🫧 Transparent";
    transparent.onclick = () => setSevenVisibilityMode("transparent");
    strip.insertBefore(transparent, readabilityBtn || null);
  }

  if (!readabilityBtn) {
    readabilityBtn = document.createElement("button");
    readabilityBtn.className = "mini-control";
    readabilityBtn.id = "readabilityShieldBtn";
    readabilityBtn.type = "button";
    readabilityBtn.textContent = "🛡️ Lisibilité";
    readabilityBtn.onclick = () => setSevenVisibilityMode("dark");
    strip.appendChild(readabilityBtn);
  } else {
    readabilityBtn.textContent = "🛡️ Lisibilité";
    readabilityBtn.onclick = () => setSevenVisibilityMode("dark");
  }

  if (!byId("commandDotTopBtn")) {
    const dot = document.createElement("button");
    dot.className = "mini-control command-dot-top";
    dot.id = "commandDotTopBtn";
    dot.type = "button";
    dot.textContent = "?";
    dot.onclick = () => toggleCommandPalette();
...
function applyAtlasItem(url, name = "Fond Atlas", family = "Atlas", variant = "fond") {
  ...
  safeStorageSet(STORAGE.atlasUrl, url);
...
function applyBackground(index) {
  ...
  safeStorageSet(STORAGE.atlasUrl, bg.url);
...
function setAmbiance(mode) {
  ...
  document.body.dataset.uiMode = mode === "night" ? "night" : mode === "crystal" ? "crystal" : "sky";
...
/* -------------------------------------------------------
   Interactive help / palette / selected state
------------------------------------------------------- */
...
function boot() {
  ...
  setStatus("Seven Terminal prêt — script lock actif.");
}
