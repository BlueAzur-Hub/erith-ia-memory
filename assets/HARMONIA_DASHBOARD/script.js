...
async function load(p,f){
  try{
    const r=await fetch(p+"?v=20260601-toggle-v2",{cache:"no-store"});
...
function render(activeIds){
  ...
  $("cards").innerHTML=scenarios.map(s=>{
    ...
    return `<div class="card ${state}" data-id="${s.id}" title="Clic : actif → exclu → réactivé"><b>${s.id}</b><br><small>${s.name}${label}</small></div>`;
  }).join("");

  document.querySelectorAll(".card").forEach(card=>{
    card.onclick=()=>{
      const id=card.dataset.id;
      const currentlyActive=activeSet.has(id)||forced.has(id);
...
function analyze(){
  ...
  const m=metrics();
  const active=detect(text,m);
  const primary=active.slice().sort((a,b)=>a.prio-b.prio)[0];
  const secondaries=active.filter(s=>!primary||s.id!==primary.id);
  const levelValue=level(active,m);
  const decisionText=decision(primary,levelValue,m);
  ...
  const msg=(templates[levelValue]||templates.Tension)
